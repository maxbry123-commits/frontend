...
![Pipeline permintaan (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)

> Sumber: [diagrams/request-pipeline.mmd](../diagrams/request-pipeline.mmd)

![Model ketahanan 3 lapis](../diagrams/exported/resilience-3layers.svg)
...
flowchart LR
    subgraph Clients[Klien Pengembang]
        ...
        C4[Klien khusus yang kompatibel dengan OpenAI]
        BROWSER[Dasbor Browser]
    end

    subgraph Router[Proses Lokal OmniRoute]
        API[API Kompatibilitas V1\n/v1/*]
        DASH[Dasbor + API Manajemen\n/api/*]
        CORE[Inti SSE + Penerjemahan\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(tabel penggunaan + artefak log)]
    end

    subgraph Upstreams[Penyedia Hulu]
        P1[Penyedia OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Penyedia Kunci API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Node Kompatibel\nKompatibel dengan OpenAI / kompatibel dengan Anthropic]
    end

    subgraph Cloud[Sinkronisasi Cloud Opsional]
        CLOUD[Endpoint Sinkronisasi Cloud\nNEXT_PUBLIC_CLOUD_URL]
    end
...
```mermaid
flowchart TD
    A[String model yang masuk] --> B{Apakah nama kombo?}
    B -- Ya --> C[Muat urutan model kombo]
    ...
    H -- Tidak --> I[Kembalikan status penyedia tidak tersedia]
    H -- Ya --> J[Jalankan permintaan]

    J --> K{Berhasil?}
    K -- Ya --> L[Kembalikan respons]
    K -- Tidak --> M{Error memenuhi syarat fallback?}

    M -- Tidak --> N[Kembalikan error]
    M -- Ya --> O[Tandai akun tidak tersedia selama masa tunggu]
    O --> P{Ada akun lain untuk penyedia?}
    P -- Ya --> G
    P -- Tidak --> Q{Berada dalam kombo dengan model berikutnya?}
    Q -- Ya --> E
    Q -- Tidak --> R[Kembalikan status semua tidak tersedia]
```
...
sequenceDiagram
    ...
    participant ProvAuth as Server Autentikasi Penyedia
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Eksekutor Penyedia

    UI->>OAuth: GET otorisasi atau kode perangkat
    OAuth->>ProvAuth: buat alur autentikasi/perangkat
    ProvAuth-->>OAuth: URL autentikasi atau payload kode perangkat
    OAuth-->>UI: data alur

    UI->>OAuth: POST pertukaran atau polling
    OAuth->>ProvAuth: pertukaran/polling token
    ...
    Exec-->>Test: info token valid atau yang telah disegarkan
    Test->>DB: perbarui status/token/error
...
sequenceDiagram
    autonumber
    participant UI as UI Halaman Endpoint
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Sinkronisasi Cloud Eksternal
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: tetapkan cloudEnabled=true
    Sync->>DB: pastikan kunci API tersedia
    Sync->>Cloud: POST /sync/{machineId} (penyedia/alias/kombo/kunci)
    Cloud-->>Sync: hasil sinkronisasi
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: diaktifkan + status verifikasi

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: data jarak jauh
    Sync->>DB: perbarui token/status lokal yang lebih baru
    Sync-->>UI: tersinkronisasi

    UI->>Sync: POST action=disable
    Sync->>DB: tetapkan cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: alihkan ANTHROPIC_BASE_URL kembali ke lokal (jika diperlukan)
    Sync-->>UI: dinonaktifkan
```
