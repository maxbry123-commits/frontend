...
![Pipeline požiadavky (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Klienti pre vývojárov]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Vlastné klienty kompatibilné s OpenAI]
        BROWSER[Ovládací panel v prehliadači]
    end

    subgraph Router[Lokálny proces OmniRoute]
        API[Kompatibilné API V1\n/v1/*]
        DASH[Ovládací panel + API na správu\n/api/*]
        CORE[Jadro SSE + prekladu\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(tabuľky používania + artefakty protokolov)]
    end

    subgraph Upstreams[Poskytovatelia vyššej úrovne]
        P1[Poskytovatelia OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Poskytovatelia s kľúčom API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Kompatibilné uzly\nKompatibilné s OpenAI / kompatibilné s Anthropic]
    end

    subgraph Cloud[Voliteľná synchronizácia s cloudom]
        CLOUD[Koncový bod synchronizácie s cloudom\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    ...
    participant Model as Prekladač modelov
    participant Auth as Výber poverení
    participant Exec as Vykonávateľ poskytovateľa
    participant Prov as Nadradený poskytovateľ
    participant Stream as Prekladač prúdu
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: analyzovať/vyriešiť model alebo kombináciu

    alt Kombinovaný model
        Chat->>Chat: iterovať cez modely kombinácie (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: aktívny účet + tokeny/kľúč API

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: zistiť zdrojový formát
    Core->>Core: preložiť požiadavku do cieľového formátu
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: volanie nadradeného API
    Prov-->>Exec: odpoveď SSE/JSON
    Exec-->>Core: odpoveď + metadáta

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: aktualizované tokeny
        Core->>Exec: zopakovať požiadavku
    end

    Core->>Stream: preložiť/normalizovať prúd do formátu klienta
    Stream-->>Client: bloky SSE / odpoveď JSON

    Stream->>Usage: extrahovať údaje o používaní + uložiť históriu/protokol
```
...
```mermaid
flowchart TD
    A[Prichádzajúci reťazec modelu] --> B{Je to názov kombinácie?}
    B -- Áno --> C[Načítať postupnosť modelov kombinácie]
    B -- Nie --> D[Cesta jedného modelu]

    C --> E[Vyskúšať model N]
    E --> F[Určiť poskytovateľa/model]
    D --> F

    F --> G[Vybrať prihlasovacie údaje účtu]
    G --> H{Sú prihlasovacie údaje dostupné?}
    H -- Nie --> I[Vrátiť nedostupnosť poskytovateľa]
    H -- Áno --> J[Vykonať požiadavku]

    J --> K{Úspešné?}
    K -- Áno --> L[Vrátiť odpoveď]
    K -- Nie --> M{Chyba vhodná na použitie záložnej možnosti?}

    M -- Nie --> N[Vrátiť chybu]
    M -- Áno --> O[Označiť účet ako dočasne nedostupný]
    O --> P{Ďalší účet poskytovateľa?}
    P -- Áno --> G
    P -- Nie --> Q{Je v kombinácii ďalší model?}
    Q -- Áno --> E
    Q -- Nie --> R[Vrátiť, že všetky možnosti sú nedostupné]
```
...
sequenceDiagram
    autonumber
    participant UI as Používateľské rozhranie panela
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Autorizačný server poskytovateľa
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Vykonávací modul poskytovateľa

    UI->>OAuth: GET authorize alebo device-code
    OAuth->>ProvAuth: vytvoriť autorizačný tok/tok zariadenia
    ProvAuth-->>OAuth: autorizačná URL alebo dáta kódu zariadenia
    OAuth-->>UI: údaje toku

    UI->>OAuth: POST exchange alebo poll
    OAuth->>ProvAuth: výmena tokenu/dopytovanie
    ProvAuth-->>OAuth: prístupové/obnovovacie tokeny
    OAuth->>DB: createProviderConnection(oauth data)
    OAuth-->>UI: úspech + ID pripojenia

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: overiť prihlasovacie údaje / voliteľné obnovenie
    Exec-->>Test: platné alebo obnovené údaje tokenu
    Test->>DB: aktualizovať stav/tokeny/chyby
    Test-->>UI: výsledok overenia
```
...
sequenceDiagram
    autonumber
    participant UI as Používateľské rozhranie stránky koncového bodu
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Externá cloudová synchronizácia
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: nastaviť cloudEnabled=true
    Sync->>DB: zabezpečiť existenciu kľúča API
    Sync->>Cloud: POST /sync/{machineId} (poskytovatelia/aliasy/kombinácie/kľúče)
    Cloud-->>Sync: výsledok synchronizácie
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: zapnuté + stav overenia

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: vzdialené údaje
    Sync->>DB: aktualizovať novšie lokálne tokeny/stav
    Sync-->>UI: synchronizované

    UI->>Sync: POST action=disable
    Sync->>DB: nastaviť cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: prepnúť ANTHROPIC_BASE_URL späť na lokálnu adresu (ak je to potrebné)
    Sync-->>UI: vypnuté
```
...
```mermaid
erDiagram
    SETTINGS ||--o{ PROVIDER_CONNECTION : riadi
    PROVIDER_NODE ||--o{ PROVIDER_CONNECTION : podporuje_kompatibilného_poskytovateľa
    PROVIDER_CONNECTION ||--o{ USAGE_ENTRY : zaznamenáva_použitie
...
```mermaid
flowchart LR
    subgraph LocalHost[Hostiteľské prostredie vývojára]
        CLI[Nástroje CLI]
        Browser[Prehliadač ovládacieho panela]
    end

    subgraph ContainerOrProcess[Runtime OmniRoute]
        Next[Server Next.js\nPORT=20128]
        Core[Jadro SSE + vykonávacie moduly]
        MainDB[(storage.sqlite)]
        UsageDB[(tabuľky používania + artefakty protokolov)]
    end

    subgraph External[Externé služby]
        Providers[Poskytovatelia AI]
        SyncCloud[Služba cloudovej synchronizácie]
    end
