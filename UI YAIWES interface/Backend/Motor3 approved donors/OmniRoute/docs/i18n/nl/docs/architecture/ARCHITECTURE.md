...
![Aanvraagpijplijn (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)

> Bron: [diagrams/request-pipeline.mmd](../diagrams/request-pipeline.mmd)

![Drielaags weerbaarheidsmodel](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Clients voor ontwikkelaars]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Aangepaste OpenAI-compatibele clients]
        BROWSER[Browserdashboard]
    end

    subgraph Router[Lokaal OmniRoute-proces]
        API[V1-compatibiliteits-API\n/v1/*]
        DASH[Dashboard + beheer-API\n/api/*]
        CORE[SSE + vertaalkern\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(gebruikstabellen + logartefacten)]
    end

    subgraph Upstreams[Upstreamproviders]
        P1[OAuth-providers\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API-sleutelproviders\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Compatibele knooppunten\nOpenAI-compatibel / Anthropic-compatibel]
    end

    subgraph Cloud[Optionele cloudsynchronisatie]
        CLOUD[Cloudsynchronisatie-eindpunt\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    ...
    participant Model as Modelresolver
    participant Auth as Referentieselector
    participant Exec as Provideruitvoerder
    participant Prov as Bovenliggende provider
    participant Stream as Streamvertaler
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: model of combo parseren/oplossen

    alt Combomodel
        Chat->>Chat: combomodellen doorlopen (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: actief account + tokens/API-sleutel

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: bronindeling detecteren
    Core->>Core: verzoek naar doelindeling vertalen
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: bovenliggende API-aanroep
    Prov-->>Exec: SSE-/JSON-respons
    Exec-->>Core: respons + metagegevens

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: bijgewerkte tokens
        Core->>Exec: verzoek opnieuw proberen
    end

    Core->>Stream: stream naar clientindeling vertalen/normaliseren
    Stream-->>Client: SSE-fragmenten / JSON-respons

    Stream->>Usage: gebruiksgegevens extraheren + geschiedenis/logboek opslaan
```
...
```mermaid
flowchart TD
    A[Inkomende modeltekenreeks] --> B{Is dit een combonaam?}
    B -- Ja --> C[Laad de reeks combomodellen]
    B -- Nee --> D[Pad voor één model]

    C --> E[Probeer model N]
    E --> F[Los provider/model op]
    D --> F

    F --> G[Selecteer accountreferenties]
    G --> H{Referenties beschikbaar?}
    H -- Nee --> I[Retourneer dat de provider niet beschikbaar is]
    H -- Ja --> J[Voer de aanvraag uit]

    J --> K{Geslaagd?}
    K -- Ja --> L[Retourneer het antwoord]
    K -- Nee --> M{Komt de fout in aanmerking voor fallback?}

    M -- Nee --> N[Retourneer de fout]
    M -- Ja --> O[Markeer account tijdelijk als niet beschikbaar]
    O --> P{Nog een account voor de provider?}
    P -- Ja --> G
    P -- Nee --> Q{In een combo met een volgend model?}
    Q -- Ja --> E
    Q -- Nee --> R[Retourneer dat alles niet beschikbaar is]
```
...
sequenceDiagram
    ...
    participant ProvAuth as Authenticatieserver van provider
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Provider-executor

    UI->>OAuth: GET authorize of device-code
    OAuth->>ProvAuth: maak authenticatie-/apparaatstroom
    ProvAuth-->>OAuth: authenticatie-URL of payload met apparaatcode
    OAuth-->>UI: stroomgegevens

    UI->>OAuth: POST exchange of poll
    OAuth->>ProvAuth: tokenuitwisseling/-polling
    ProvAuth-->>OAuth: toegangs-/vernieuwingstokens
    OAuth->>DB: createProviderConnection(oauth-gegevens)
    OAuth-->>UI: geslaagd + verbindings-id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: valideer referenties / optionele vernieuwing
    Exec-->>Test: geldige of vernieuwde tokeninformatie
    Test->>DB: werk status/tokens/fouten bij
    Test-->>UI: validatieresultaat
```
...
sequenceDiagram
    autonumber
    participant UI as UI van endpointpagina
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Externe cloudsynchronisatie
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: stel cloudEnabled=true in
    Sync->>DB: zorg dat er een API-sleutel bestaat
    Sync->>Cloud: POST /sync/{machineId} (providers/aliases/combos/keys)
    Cloud-->>Sync: synchronisatieresultaat
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: ingeschakeld + verificatiestatus

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: externe gegevens
    Sync->>DB: werk nieuwere lokale tokens/status bij
    Sync-->>UI: gesynchroniseerd

    UI->>Sync: POST action=disable
    Sync->>DB: stel cloudEnabled=false in
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: zet ANTHROPIC_BASE_URL terug naar lokaal (indien nodig)
    Sync-->>UI: uitgeschakeld
```
