...
>
> Để xem các sơ đồ kiến trúc cấp cao và lý do thiết kế đằng sau từng hệ thống con, hãy đọc
...
Tệp này mô tả **những gì hiện có trong kho lưu trữ** để một kỹ sư mới
có thể điều hướng cây thư mục, hiểu cách phân lớp thời gian chạy và biết nơi cần thêm mã
...
**Không có middleware toàn cục** — việc chặn được thực hiện theo từng route.
...
| `oauth/`          | Các mô-đun OAuth/nhập nhà cung cấp (22): `agy`, `antigravity`, `claude`, `cline`, `codebuddy-cn`, `codex`, `cursor`, `devin-desktop`, `ghe-copilot`, `github`, `gitlab-duo`, `grok-cli-oauth`, `grok-cli`, `kilocode`, `kimi-coding`, `kiro`, `openference`, `qoder`, `trae`, `xai-oauth`, `zed-hosted`, `zed`, cùng với `services/`, `utils/` và `constants/oauth.ts`                                                                                                                                                                                                                                                                                                                                  |
...
- `validation/` — `schemas.ts` (~80 schema Zod), `compressionConfigSchemas.ts`,
...
- `utils/` — `circuitBreaker.ts`, `apiAuth.ts`, `apiKey.ts`, `apiKeyPolicy.ts`,
  ...
  `a11yAudit.ts`, cùng với các hook/thành phần của bảng điều khiển trong `services/`, `network/`,
...
> Lưu ý: các nhà cung cấp không được liệt kê tại đây được phục vụ bởi `default.ts` bằng executor chung
...
- `responsesTransformer.ts` — Bộ chuyển đổi Responses API ↔ Chat Completions dựa trên `TransformStream`
  (được sử dụng bởi tuyến bắt tất cả `responses/`).
...
- **110 công cụ duy nhất** được kết nối trong `server.ts` (45 công cụ chuẩn trong `schemas/tools.ts` +
  ...
  kho dữ liệu cục bộ và nén — hợp được đếm bởi `countUniqueMcpTools`).
...
2. Thêm trình thực thi trong `open-sse/executors/` nếu cần logic tùy chỉnh
...
5. Đăng ký các mô hình trong `open-sse/config/providerRegistry.ts` (hoặc registry dành riêng cho định dạng
...
4. Nếu chỉ dành cho quản trị: thêm đường dẫn vào `src/shared/constants/publicApiRoutes.ts`
   (danh sách từ chối đối với bề mặt API công khai).
...
3. Nếu có bảng mới: thêm migration trong `src/lib/db/migrations/`, được đánh số
   tuần tự, có tính lũy đẳng và theo giao dịch.
...
- **ESLint**: `no-eval`, `no-implied-eval`, `no-new-func` = `error` ở mọi nơi;
  `no-explicit-any` = `warn` trong `open-sse/` và `tests/`, lỗi ở những nơi khác.
- **TypeScript**: `strict: false` (trạng thái kế thừa). Ưu tiên kiểu tường minh hơn
  suy luận kiểu tại các ranh giới giữa mô-đun.
- **Cơ sở dữ liệu**: không bao giờ viết SQL thô trong route hoặc handler — luôn thông qua
  các mô-đun `src/lib/db/`. Không bao giờ import từ barrel — hãy sử dụng trực tiếp các mô-đun `src/lib/db/*` cụ thể.
- **Định kiểu thực thể DB (#3512)**: một hàm ghi hoặc đọc cấu trúc hàng của bảng DB
  ...
  của bảng đó, không phải `any` hoặc kiểu ẩn danh nội tuyến tại vị trí gọi. Đặt
  interface cạnh hàm (ví dụ: `export interface UsageEntry` trong
  `src/lib/usage/usageHistory.ts` phía trên `saveRequestUsage`), giữ từng
  trường ở dạng tùy chọn/có thể null khi các trình ghi khác nhau điền dữ liệu vào hàng
  theo từng bước, và ưu tiên `unknown` hơn `any` đối với trường có cấu trúc
  thay đổi giữa các bên gọi (được ghi chú trên trường, ví dụ: `UsageEntry.tokens`
  chấp nhận cả dữ liệu sử dụng thô theo cấu trúc của nhà cung cấp lẫn cấu trúc đã chuẩn hóa). Khi số lượng
  `any` của một tệp giảm xuống bằng không theo cách này, hãy thêm tệp đó vào danh sách cho phép
  `check:any-budget:t11` (`scripts/check/check-t11-any-budget.mjs`,
  `maxAny: 0`) để ngăn tái diễn. Đây là quy ước cho giai đoạn đầu — việc
  dọn dẹp rộng hơn nhằm loại bỏ "`any` ẩn danh" được thực hiện lặp dần trên phần còn lại của
  codebase.
- **Lỗi**: sử dụng try/catch với các kiểu lỗi cụ thể, ghi log kèm ngữ cảnh pino. Không bao giờ
  âm thầm bỏ qua lỗi trong các luồng SSE; sử dụng tín hiệu hủy để dọn dẹp.
- **Bảo mật**: không bao giờ sử dụng `eval()` / `new Function()` / eval ngầm định. Xác thực
  ...
  danh sách từ chối `src/shared/constants/upstreamHeaders.ts` đồng bộ với lớp
  làm sạch/xác thực.
...
- **Husky**: pre-commit chạy `lint-staged` + `check:docs-sync` +
  `check:any-budget:t11`; pre-push chạy `check:any-budget:t11` + `check:tracked-artifacts` (các bước kiểm tra nhanh; loại trừ `test:unit`).
...
- [ARCHITECTURE.md](./ARCHITECTURE.md) — kiến trúc cấp cao và trách nhiệm của
  các mô-đun.
- [API_REFERENCE.md](../reference/API_REFERENCE.md) — tài liệu tham khảo API công khai + quản trị.
- [FEATURES.md](../guides/FEATURES.md) — ma trận tính năng và các điểm nổi bật theo phiên bản.
...
- [MCP-SERVER.md](../frameworks/MCP-SERVER.md) — danh mục công cụ MCP đầy đủ + các phương thức truyền tải.
...
- [ELECTRON_GUIDE.md](../guides/ELECTRON_GUIDE.md) (nếu có), [DOCKER_GUIDE.md](../guides/DOCKER_GUIDE.md), [FLY_IO_DEPLOYMENT_GUIDE.md](../ops/FLY_IO_DEPLOYMENT_GUIDE.md), [VM_DEPLOYMENT_GUIDE.md](../ops/VM_DEPLOYMENT_GUIDE.md), [TERMUX_GUIDE.md](../guides/TERMUX_GUIDE.md), [PWA_GUIDE.md](../guides/PWA_GUIDE.md) — các môi trường triển khai đích.
- [TROUBLESHOOTING.md](../guides/TROUBLESHOOTING.md) — các vấn đề vận hành thường gặp.
- [CONTRIBUTING.md](../../CONTRIBUTING.md) — quy trình làm việc dành cho người đóng góp.
- [CLAUDE.md](../../CLAUDE.md) — các quy tắc của kho lưu trữ dành cho Claude Code (nguồn tham chiếu chính xác
  cho nhiều quy ước ở trên).
- [AGENTS.md](../../AGENTS.md) — tài liệu tham khảo kiến trúc chuyên sâu hơn dành cho các agent.
