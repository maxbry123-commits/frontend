...
![צינור עיבוד הבקשות (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)

> מקור: [diagrams/request-pipeline.mmd](../diagrams/request-pipeline.mmd)

![מודל חוסן בן 3 שכבות](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[לקוחות מפתחים]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[לקוחות מותאמים אישית התואמים ל-OpenAI]
        BROWSER[לוח מחוונים בדפדפן]
    end

    subgraph Router[תהליך OmniRoute מקומי]
        API[API תאימות V1\n/v1/*]
        DASH[לוח מחוונים + API ניהול\n/api/*]
        CORE[ליבת SSE + תרגום\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(טבלאות שימוש + ארטיפקטים של יומנים)]
    end

    subgraph Upstreams[ספקים במעלה הזרם]
        P1[ספקי OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[ספקי מפתחות API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[צמתים תואמים\nתואמי OpenAI / תואמי Anthropic]
    end

    subgraph Cloud[סנכרון ענן אופציונלי]
        CLOUD[נקודת קצה לסנכרון ענן\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as לקוח CLI/SDK
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as פותר המודלים
    participant Auth as בורר פרטי ההזדהות
    participant Exec as מבצע הספק
    participant Prov as ספק במעלה הזרם
    participant Stream as מתרגם הזרם
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: ניתוח/פתרון מודל או שילוב

    alt מודל משולב
        Chat->>Chat: מעבר על מודלי השילוב (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: חשבון פעיל + אסימונים/מפתח API

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: זיהוי פורמט המקור
    Core->>Core: תרגום הבקשה לפורמט היעד
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: קריאת API במעלה הזרם
    Prov-->>Exec: תגובת SSE/JSON
    Exec-->>Core: תגובה + מטא-נתונים

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: אסימונים מעודכנים
        Core->>Exec: ניסיון חוזר של הבקשה
    end

    Core->>Stream: תרגום/נרמול הזרם לפורמט הלקוח
    Stream-->>Client: מקטעי SSE / תגובת JSON

    Stream->>Usage: חילוץ נתוני שימוש + שמירת היסטוריה/יומן
```
...
```mermaid
flowchart TD
    A[מחרוזת מודל נכנסת] --> B{האם זהו שם קומבו?}
    B -- כן --> C[טעינת רצף המודלים של הקומבו]
    B -- לא --> D[נתיב של מודל יחיד]

    C --> E[ניסיון עם מודל N]
    E --> F[זיהוי ספק/מודל]
    D --> F

    F --> G[בחירת פרטי כניסה לחשבון]
    G --> H{האם פרטי הכניסה זמינים?}
    H -- לא --> I[החזרת הודעה שהספק אינו זמין]
    H -- כן --> J[ביצוע הבקשה]

    J --> K{האם הצליח?}
    K -- כן --> L[החזרת התגובה]
    K -- לא --> M{האם השגיאה מאפשרת מעבר חלופי?}

    M -- לא --> N[החזרת השגיאה]
    M -- כן --> O[סימון החשבון כלא זמין למשך תקופת צינון]
    O --> P{האם יש חשבון נוסף עבור הספק?}
    P -- כן --> G
    P -- לא --> Q{האם יש בקומבו מודל נוסף?}
    Q -- כן --> E
    Q -- לא --> R[החזרת הודעה שכולם אינם זמינים]
```
...
sequenceDiagram
    autonumber
    participant UI as ממשק המשתמש של לוח הבקרה
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as שרת האימות של הספק
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as מבצע הספק

    UI->>OAuth: GET לאישור או לקבלת קוד מכשיר
    OAuth->>ProvAuth: יצירת תהליך אימות/מכשיר
    ProvAuth-->>OAuth: כתובת URL לאימות או מטען נתונים של קוד מכשיר
    OAuth-->>UI: נתוני התהליך

    UI->>OAuth: POST להמרה או לתשאול
    OAuth->>ProvAuth: המרת אסימון/תשאול
    ProvAuth-->>OAuth: אסימוני גישה/רענון
    OAuth->>DB: createProviderConnection(נתוני oauth)
    OAuth-->>UI: הצלחה + מזהה חיבור

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: אימות פרטי כניסה / רענון אופציונלי
    Exec-->>Test: מידע על אסימון תקף או מרוענן
    Test->>DB: עדכון סטטוס/אסימונים/שגיאות
    Test-->>UI: תוצאת האימות
```
...
sequenceDiagram
    autonumber
    participant UI as ממשק המשתמש של דף נקודת הקצה
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as סנכרון ענן חיצוני
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: הגדרת cloudEnabled=true
    Sync->>DB: וידוא שקיים מפתח API
    Sync->>Cloud: POST /sync/{machineId} (ספקים/כינויים/קומבואים/מפתחות)
    Cloud-->>Sync: תוצאת הסנכרון
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: מופעל + סטטוס אימות

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: נתונים מרוחקים
    Sync->>DB: עדכון אסימונים/סטטוס מקומיים חדשים יותר
    Sync-->>UI: סונכרן

    UI->>Sync: POST action=disable
    Sync->>DB: הגדרת cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: החזרת ANTHROPIC_BASE_URL לכתובת המקומית (במידת הצורך)
    Sync-->>UI: מושבת
```
