...
![Flujo de procesamiento de solicitudes (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Clientes para desarrolladores]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Clientes personalizados compatibles con OpenAI]
        BROWSER[Panel del navegador]
    end

    subgraph Router[Proceso local de OmniRoute]
        API[API de compatibilidad V1\n/v1/*]
        DASH[Panel + API de administración\n/api/*]
        CORE[Núcleo de SSE + traducción\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(tablas de uso + artefactos de registro)]
    end

    subgraph Upstreams[Proveedores ascendentes]
        P1[Proveedores OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Proveedores de claves de API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Nodos compatibles\nCompatibles con OpenAI / compatibles con Anthropic]
    end

    subgraph Cloud[Sincronización opcional con la nube]
        CLOUD[Endpoint de sincronización con la nube\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    ...
    participant Model as Resolutor de modelos
    participant Auth as Selector de credenciales
    participant Exec as Ejecutor del proveedor
    participant Prov as Proveedor ascendente
    participant Stream as Traductor de flujos
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: analizar/resolver modelo o combo

    alt Modelo combo
        Chat->>Chat: iterar modelos del combo (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: cuenta activa + tokens/clave API

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: detectar formato de origen
    Core->>Core: traducir solicitud al formato de destino
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: llamada a la API ascendente
    Prov-->>Exec: respuesta SSE/JSON
    Exec-->>Core: respuesta + metadatos

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: tokens actualizados
        Core->>Exec: reintentar solicitud
    end

    Core->>Stream: traducir/normalizar el flujo al formato del cliente
    Stream-->>Client: fragmentos SSE / respuesta JSON

    Stream->>Usage: extraer uso + almacenar historial/registro
```
...
```mermaid
flowchart TD
    A[Cadena de modelo entrante] --> B{¿Es un nombre de combinación?}
    B -- Sí --> C[Cargar secuencia de modelos de la combinación]
    B -- No --> D[Ruta de modelo único]

    C --> E[Probar modelo N]
    E --> F[Resolver proveedor/modelo]
    D --> F

    F --> G[Seleccionar credenciales de la cuenta]
    G --> H{¿Hay credenciales disponibles?}
    H -- No --> I[Devolver proveedor no disponible]
    H -- Sí --> J[Ejecutar solicitud]

    J --> K{¿Correcto?}
    K -- Sí --> L[Devolver respuesta]
    K -- No --> M{¿Error apto para respaldo?}

    M -- No --> N[Devolver error]
    M -- Sí --> O[Marcar cuenta como no disponible durante el período de espera]
    O --> P{¿Hay otra cuenta para el proveedor?}
    P -- Sí --> G
    P -- No --> Q{¿Está en una combinación con otro modelo?}
    Q -- Sí --> E
    Q -- No --> R[Devolver que ninguno está disponible]
```
...
sequenceDiagram
    autonumber
    participant UI as Interfaz del panel
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Servidor de autenticación del proveedor
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Ejecutor del proveedor

    UI->>OAuth: GET de autorización o código de dispositivo
    OAuth->>ProvAuth: crear flujo de autenticación/dispositivo
    ProvAuth-->>OAuth: URL de autenticación o carga útil del código de dispositivo
    OAuth-->>UI: datos del flujo

    UI->>OAuth: POST de intercambio o sondeo
    OAuth->>ProvAuth: intercambio/sondeo del token
    ProvAuth-->>OAuth: tokens de acceso/renovación
    OAuth->>DB: createProviderConnection(datos de OAuth)
    OAuth-->>UI: operación correcta + id de conexión

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: validar credenciales / renovación opcional
    Exec-->>Test: información del token válido o renovado
    Test->>DB: actualizar estado/tokens/errores
    Test-->>UI: resultado de la validación
```
...
sequenceDiagram
    autonumber
    participant UI as Interfaz de la página del endpoint
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Sincronización externa en la nube
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: establecer cloudEnabled=true
    Sync->>DB: garantizar que exista una clave de API
    Sync->>Cloud: POST /sync/{machineId} (proveedores/alias/combinaciones/claves)
    Cloud-->>Sync: resultado de la sincronización
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: activado + estado de verificación

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: datos remotos
    Sync->>DB: actualizar tokens/estado locales más recientes
    Sync-->>UI: sincronizado

    UI->>Sync: POST action=disable
    Sync->>DB: establecer cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: restablecer ANTHROPIC_BASE_URL al entorno local (si es necesario)
    Sync-->>UI: desactivado
```
...
erDiagram
    ...
    PROVIDER_NODE ||--o{ PROVIDER_CONNECTION : respalda_proveedor_compatible
...
```mermaid
flowchart LR
    subgraph LocalHost[Host del desarrollador]
        CLI[Herramientas de CLI]
        Browser[Navegador del panel]
    end

    subgraph ContainerOrProcess[Entorno de ejecución de OmniRoute]
        Next[Servidor Next.js\nPORT=20128]
        Core[Núcleo SSE + ejecutores]
        MainDB[(storage.sqlite)]
        UsageDB[(tablas de uso + artefactos de registro)]
    end

    subgraph External[Servicios externos]
        Providers[Proveedores de IA]
        SyncCloud[Servicio de sincronización en la nube]
    end
