...
```mermaid
flowchart LR
    subgraph Clients[ไคลเอนต์สำหรับนักพัฒนา]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[ไคลเอนต์แบบกำหนดเองที่เข้ากันได้กับ OpenAI]
        BROWSER[แดชบอร์ดบนเบราว์เซอร์]
    end

    subgraph Router[โปรเซส OmniRoute ภายในเครื่อง]
        API[API ที่เข้ากันได้กับ V1\n/v1/*]
        DASH[แดชบอร์ด + API การจัดการ\n/api/*]
        CORE[แกนหลัก SSE + การแปล\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(ตารางการใช้งาน + อาร์ติแฟกต์บันทึก)]
    end

    subgraph Upstreams[ผู้ให้บริการต้นทาง]
        P1[ผู้ให้บริการ OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[ผู้ให้บริการ API Key\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[โหนดที่เข้ากันได้\nเข้ากันได้กับ OpenAI / เข้ากันได้กับ Anthropic]
    end

    subgraph Cloud[การซิงค์กับคลาวด์ที่เป็นตัวเลือก]
        CLOUD[เอนด์พอยต์การซิงค์กับคลาวด์\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as ไคลเอนต์ CLI/SDK
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as ตัวแก้ไขโมเดล
    participant Auth as ตัวเลือกข้อมูลรับรอง
    participant Exec as ตัวดำเนินการของผู้ให้บริการ
    participant Prov as ผู้ให้บริการต้นทาง
    participant Stream as ตัวแปลงสตรีม
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: แยกวิเคราะห์/แก้ไขโมเดลหรือคอมโบ

    alt โมเดลคอมโบ
        Chat->>Chat: วนซ้ำโมเดลในคอมโบ (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: บัญชีที่ใช้งานอยู่ + โทเค็น/API key

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: ตรวจหารูปแบบต้นทาง
    Core->>Core: แปลงคำขอเป็นรูปแบบเป้าหมาย
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: เรียก API ต้นทาง
    Prov-->>Exec: การตอบกลับ SSE/JSON
    Exec-->>Core: การตอบกลับ + ข้อมูลเมตา

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: โทเค็นที่อัปเดตแล้ว
        Core->>Exec: ลองส่งคำขออีกครั้ง
    end

    Core->>Stream: แปลง/ปรับสตรีมให้อยู่ในรูปแบบของไคลเอนต์
    Stream-->>Client: ส่วนย่อย SSE / การตอบกลับ JSON

    Stream->>Usage: ดึงข้อมูลการใช้งาน + จัดเก็บประวัติ/บันทึกถาวร
```
...
```mermaid
flowchart TD
    A[สตริงโมเดลขาเข้า] --> B{เป็นชื่อคอมโบหรือไม่?}
    B -- ใช่ --> C[โหลดลำดับโมเดลของคอมโบ]
    B -- ไม่ใช่ --> D[เส้นทางโมเดลเดี่ยว]

    C --> E[ลองใช้โมเดล N]
    E --> F[ระบุผู้ให้บริการ/โมเดล]
    D --> F

    F --> G[เลือกข้อมูลประจำตัวของบัญชี]
    G --> H{มีข้อมูลประจำตัวหรือไม่?}
    H -- ไม่มี --> I[ส่งคืนว่าผู้ให้บริการไม่พร้อมใช้งาน]
    H -- มี --> J[ดำเนินการคำขอ]

    J --> K{สำเร็จหรือไม่?}
    K -- ใช่ --> L[ส่งคืนการตอบกลับ]
    K -- ไม่ใช่ --> M{เป็นข้อผิดพลาดที่สามารถสลับไปใช้รายการสำรองได้หรือไม่?}

    M -- ไม่ใช่ --> N[ส่งคืนข้อผิดพลาด]
    M -- ใช่ --> O[ทำเครื่องหมายบัญชีว่าไม่พร้อมใช้งานในช่วงพัก]
    O --> P{มีบัญชีอื่นสำหรับผู้ให้บริการหรือไม่?}
    P -- ใช่ --> G
    P -- ไม่ใช่ --> Q{อยู่ในคอมโบที่มีโมเดลถัดไปหรือไม่?}
    Q -- ใช่ --> E
    Q -- ไม่ใช่ --> R[ส่งคืนว่าทั้งหมดไม่พร้อมใช้งาน]
```
...
sequenceDiagram
    autonumber
    participant UI as UI แดชบอร์ด
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as เซิร์ฟเวอร์การยืนยันตัวตนของผู้ให้บริการ
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as ตัวดำเนินการของผู้ให้บริการ

    UI->>OAuth: GET authorize หรือ device-code
    OAuth->>ProvAuth: สร้างโฟลว์การยืนยันตัวตน/อุปกรณ์
    ProvAuth-->>OAuth: URL การยืนยันตัวตนหรือเพย์โหลดรหัสอุปกรณ์
    OAuth-->>UI: ข้อมูลโฟลว์

    UI->>OAuth: POST exchange หรือ poll
    OAuth->>ProvAuth: แลกเปลี่ยน/ตรวจสอบโทเค็น
    ProvAuth-->>OAuth: โทเค็นการเข้าถึง/รีเฟรช
    OAuth->>DB: createProviderConnection(ข้อมูล oauth)
    OAuth-->>UI: สำเร็จ + ID การเชื่อมต่อ

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: ตรวจสอบข้อมูลประจำตัว / รีเฟรชเพิ่มเติม
    Exec-->>Test: ข้อมูลโทเค็นที่ถูกต้องหรือได้รับการรีเฟรชแล้ว
    Test->>DB: อัปเดตสถานะ/โทเค็น/ข้อผิดพลาด
    Test-->>UI: ผลการตรวจสอบ
```
...
sequenceDiagram
    autonumber
    participant UI as UI หน้าเอนด์พอยต์
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as บริการซิงค์กับคลาวด์ภายนอก
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: ตั้งค่า cloudEnabled=true
    Sync->>DB: ตรวจสอบให้แน่ใจว่ามีคีย์ API
    Sync->>Cloud: POST /sync/{machineId} (ผู้ให้บริการ/นามแฝง/คอมโบ/คีย์)
    Cloud-->>Sync: ผลการซิงค์
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: เปิดใช้งานแล้ว + สถานะการตรวจสอบ

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: ข้อมูลระยะไกล
    Sync->>DB: อัปเดตโทเค็น/สถานะในเครื่องที่ใหม่กว่า
    Sync-->>UI: ซิงค์แล้ว

    UI->>Sync: POST action=disable
    Sync->>DB: ตั้งค่า cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: เปลี่ยน ANTHROPIC_BASE_URL กลับเป็นภายในเครื่อง (หากจำเป็น)
    Sync-->>UI: ปิดใช้งานแล้ว
```
