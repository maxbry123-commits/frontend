...
![Sorğu emalı xətti (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)
...
```mermaid
flowchart LR
    subgraph Clients[İstifadəçi Klientləri]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Fərdi OpenAI-uyğun klientlər]
        BROWSER[Brauzer İdarəetmə Paneli]
    end

    subgraph Router[OmniRoute Lokal Prosesi]
        API[V1 Uyğunluq API-si\n/v1/*]
        DASH[İdarəetmə Paneli + İdarəetmə API-si\n/api/*]
        CORE[SSE + Tərcümə Nüvəsi\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(istifadə cədvəlləri + jurnal artefaktları)]
    end

    subgraph Upstreams[Yuxarı Axın Provayderləri]
        P1[OAuth Provayderləri\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API Açarı Provayderləri\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Uyğun Qovşaqlar\nOpenAI-uyğun / Anthropic-uyğun]
    end

    subgraph Cloud[İxtiyari Bulud Sinxronizasiyası]
        CLOUD[Bulud Sinxronizasiya Son Nöqtəsi\nNEXT_PUBLIC_CLOUD_URL]
    end
...
```mermaid
flowchart TD
    A[Daxil olan model sətri] --> B{Kombinasiya adıdır?}
    B -- Bəli --> C[Kombinasiya modelləri ardıcıllığını yüklə]
    B -- Xeyr --> D[Tək model yolu]

    C --> E[N-ci modeli sına]
    E --> F[Provayderi/modeli müəyyən et]
    D --> F

    F --> G[Hesabın giriş məlumatlarını seç]
    G --> H{Giriş məlumatları mövcuddur?}
    H -- Xeyr --> I[Provayderin əlçatan olmadığını qaytar]
    H -- Bəli --> J[Sorğunu icra et]

    J --> K{Uğurludur?}
    K -- Bəli --> L[Cavabı qaytar]
    K -- Xeyr --> M{Xəta alternativə keçid üçün uyğundur?}

    M -- Xeyr --> N[Xətanı qaytar]
    M -- Bəli --> O[Hesabı gözləmə müddəti üçün əlçatmaz kimi işarələ]
    O --> P{Provayder üçün başqa hesab var?}
    P -- Bəli --> G
    P -- Xeyr --> Q{Növbəti modeli olan kombinasiyadadır?}
    Q -- Bəli --> E
    Q -- Xeyr --> R[Hamısının əlçatmaz olduğunu qaytar]
```
...
sequenceDiagram
    autonumber
    participant UI as İdarə paneli interfeysi
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Provayderin autentifikasiya serveri
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Provayder icraedicisi

    UI->>OAuth: GET avtorizasiya və ya cihaz kodu
    OAuth->>ProvAuth: autentifikasiya/cihaz axını yarat
    ProvAuth-->>OAuth: autentifikasiya URL-i və ya cihaz kodu məlumatları
    OAuth-->>UI: axın məlumatları

    UI->>OAuth: POST mübadilə və ya sorğulama
    OAuth->>ProvAuth: token mübadiləsi/sorğulaması
    ProvAuth-->>OAuth: giriş/yeniləmə tokenləri
    OAuth->>DB: createProviderConnection(oauth məlumatları)
    OAuth-->>UI: uğur + bağlantı id-si

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: giriş məlumatlarını yoxla / istəyə bağlı yenilə
    Exec-->>Test: etibarlı və ya yenilənmiş token məlumatları
    Test->>DB: statusu/tokenləri/xətaları yenilə
    Test-->>UI: yoxlama nəticəsi
```
...
sequenceDiagram
    autonumber
    participant UI as Son nöqtə səhifəsinin interfeysi
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Xarici bulud sinxronizasiyası
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=true təyin et
    Sync->>DB: API açarının mövcudluğunu təmin et
    Sync->>Cloud: POST /sync/{machineId} (provayderlər/aliaslar/kombinasiyalar/açarlar)
    Cloud-->>Sync: sinxronizasiya nəticəsi
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: aktivləşdirildi + yoxlama statusu

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: uzaq məlumatlar
    Sync->>DB: daha yeni lokal tokenləri/statusu yenilə
    Sync-->>UI: sinxronizasiya edildi

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=false təyin et
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URL-i yenidən lokal ünvana keçir (lazım olduqda)
    Sync-->>UI: deaktivləşdirildi
```
...
```mermaid
erDiagram
    SETTINGS ||--o{ PROVIDER_CONNECTION : idarə_edir
    PROVIDER_NODE ||--o{ PROVIDER_CONNECTION : uyğun_provayderi_dəstəkləyir
    PROVIDER_CONNECTION ||--o{ USAGE_ENTRY : istifadə_məlumatı_yaradır
...
```mermaid
flowchart LR
    subgraph LocalHost[Tərtibatçı Hostu]
        CLI[CLI Alətləri]
        Browser[İdarəetmə Paneli Brauzeri]
    end

    subgraph ContainerOrProcess[OmniRoute İcra Mühiti]
        Next[Next.js Serveri\nPORT=20128]
        Core[SSE Nüvəsi + İcraedicilər]
        MainDB[(storage.sqlite)]
        UsageDB[(istifadə cədvəlləri + jurnal artefaktları)]
    end

    subgraph External[Xarici Xidmətlər]
        Providers[Süni İntellekt Provayderləri]
        SyncCloud[Bulud Sinxronizasiya Xidməti]
    end
