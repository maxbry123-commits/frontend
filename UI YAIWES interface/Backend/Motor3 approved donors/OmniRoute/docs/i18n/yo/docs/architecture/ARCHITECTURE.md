...
![Àwòṣe ìfaradà aláfẹ́lẹ̀ mẹ́ta](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart TD
    A[Okùn àwòṣe tó ń wọlé] --> B{Ṣé orúkọ àkójọpọ̀ ni?}
    B -- Bẹ́ẹ̀ni --> C[Ṣàkójọpọ̀ ìtẹ̀lera àwọn àwòṣe àkójọpọ̀]
    B -- Rárá --> D[Ọ̀nà àwòṣe kan ṣoṣo]

    C --> E[Gbìyànjú àwòṣe N]
    E --> F[Yanjú olùpèsè/àwòṣe]
    D --> F

    F --> G[Yan ẹ̀rí ìdánimọ̀ àkọọ́lẹ̀]
    G --> H{Ṣé ẹ̀rí ìdánimọ̀ wà?}
    H -- Rárá --> I[Dáhùn pé olùpèsè kò sí lárọ̀ọ́wọ́tó]
    H -- Bẹ́ẹ̀ni --> J[Ṣiṣẹ́ ìbéèrè]

    J --> K{Ṣé ó ṣàṣeyọrí?}
    K -- Bẹ́ẹ̀ni --> L[Dá èsì padà]
    K -- Rárá --> M{Ṣé àṣìṣe náà yẹ fún àṣàyàn àfẹ́yìntì?}

    M -- Rárá --> N[Dá àṣìṣe padà]
    M -- Bẹ́ẹ̀ni --> O[Sàmì sí àkọọ́lẹ̀ gẹ́gẹ́ bí èyí tí kò sí lárọ̀ọ́wọ́tó fún àkókò ìsinmi]
    O --> P{Ṣé àkọọ́lẹ̀ mìíràn wà fún olùpèsè?}
    P -- Bẹ́ẹ̀ni --> G
    P -- Rárá --> Q{Ṣé ó wà nínú àkójọpọ̀ pẹ̀lú àwòṣe tó kàn?}
    Q -- Bẹ́ẹ̀ni --> E
    Q -- Rárá --> R[Dáhùn pé gbogbo wọn kò sí lárọ̀ọ́wọ́tó]
```
...
sequenceDiagram
    autonumber
    participant UI as UI Pátákó Ìdarí
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Sẹ́ńfà Ìfọwọ́sí Olùpèsè
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Olùṣiṣẹ́ Olùpèsè

    UI->>OAuth: GET authorize tàbí device-code
    OAuth->>ProvAuth: ṣẹ̀dá ìṣàn ìfọwọ́sí/ẹ̀rọ
    ProvAuth-->>OAuth: URL ìfọwọ́sí tàbí ẹrù dátà kóòdù ẹ̀rọ
    OAuth-->>UI: dátà ìṣàn

    UI->>OAuth: POST exchange tàbí poll
    OAuth->>ProvAuth: pàṣípààrọ̀ tóókì/ṣàyẹ̀wò léraléra
    ProvAuth-->>OAuth: àwọn tóókì ààyè/ìsọ̀tun
    OAuth->>DB: createProviderConnection(dátà oauth)
    OAuth-->>UI: àṣeyọrí + ID àsopọ̀

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: fìdí ẹ̀rí ìdánimọ̀ múlẹ̀ / ìsọ̀tun àṣàyàn
    Exec-->>Test: àlàyé tóókì tó fẹsẹ̀ múlẹ̀ tàbí tí a sọ̀tun
    Test->>DB: ṣe ìmúdójúìwọ̀n ipò/àwọn tóókì/àwọn àṣìṣe
    Test-->>UI: àbájáde ìfọwọ́sí
```
...
sequenceDiagram
    autonumber
    participant UI as UI Ojú-ìwé Endpoint
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Ìmúṣiṣẹ́pọ̀ Àwọsánmà Òde
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: ṣètò cloudEnabled=true
    Sync->>DB: rí i dájú pé kọ́kọ́rọ́ API wà
    Sync->>Cloud: POST /sync/{machineId} (àwọn olùpèsè/orúkọ-àpèjẹ/àkójọpọ̀/kọ́kọ́rọ́)
    Cloud-->>Sync: àbájáde ìmúṣiṣẹ́pọ̀
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: ti ṣiṣẹ́ + ipò ìfọwọ́sí

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: dátà jíjìnnà
    Sync->>DB: ṣe ìmúdójúìwọ̀n àwọn tóókì/ipò abẹ́lẹ̀ tuntun
    Sync-->>UI: ti múṣiṣẹ́pọ̀

    UI->>Sync: POST action=disable
    Sync->>DB: ṣètò cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: yí ANTHROPIC_BASE_URL padà sí ti abẹ́lẹ̀ (bí ó bá yẹ)
    Sync-->>UI: ti mú kúrò
```
...
- **Ìṣọ̀kan ipa** — Yí `developer` → `system` padà fún àwọn àfojúsùn tí kì í ṣe OpenAI; dapọ̀ `system` → `user` fún àwọn mọ́dẹ́lì tí kò gba ipa system (GLM, ERNIE)
...
| `GET /v1/embeddings`                               | Àtòjọ àwọn mọ́dẹ́lì        | Òpópónà API                                                                                      |
| `POST /v1/images/generations`                      | Àwọn Àwòrán OpenAI       | `open-sse/handlers/imageGeneration.ts`                                                           |
| `GET /v1/images/generations`                       | Àtòjọ àwọn mọ́dẹ́lì        | Òpópónà API                                                                                      |
| `POST /v1/providers/{provider}/chat/completions`   | Ìfọ̀rọ̀wérọ̀ OpenAI         | Tí a yà sọ́tọ̀ fún olùpèsè kọ̀ọ̀kan pẹ̀lú ìfàṣẹsí mọ́dẹ́lì                                              |
| `POST /v1/providers/{provider}/embeddings`         | Àwọn Ìfàṣẹ̀wọlé OpenAI    | Tí a yà sọ́tọ̀ fún olùpèsè kọ̀ọ̀kan pẹ̀lú ìfàṣẹsí mọ́dẹ́lì                                              |
| `POST /v1/providers/{provider}/images/generations` | Àwọn Àwòrán OpenAI       | Tí a yà sọ́tọ̀ fún olùpèsè kọ̀ọ̀kan pẹ̀lú ìfàṣẹsí mọ́dẹ́lì                                              |
| `POST /v1/messages/count_tokens`                   | Ìṣírò Token Claude       | Òpópónà API                                                                                      |
| `GET /v1/models`                                   | Àtòjọ àwọn mọ́dẹ́lì OpenAI | Òpópónà API (ìfọ̀rọ̀wérọ̀ + ìfàṣẹ̀wọlé + àwòrán + àwọn mọ́dẹ́lì àkànṣe)                                |
| `GET /api/models/catalog`                          | Àkójọ                    | Gbogbo mọ́dẹ́lì tí a pín sí ẹgbẹ́ gẹ́gẹ́ bí olùpèsè + irú                                             |
...
| `GET/POST/DELETE /api/provider-models`             | Àwọn Mọ́dẹ́lì Olùpèsè      | Métádátà mọ́dẹ́lì olùpèsè tó ń ṣètìlẹ́yìn fún àwọn mọ́dẹ́lì àkànṣe àti àwọn mọ́dẹ́lì tó wà lábẹ́ ìṣàkóso |
