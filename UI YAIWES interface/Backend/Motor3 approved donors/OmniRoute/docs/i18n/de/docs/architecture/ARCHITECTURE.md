...
![Dreischichtiges Resilienzmodell](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Entwickler-Clients]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Benutzerdefinierte OpenAI-kompatible Clients]
        BROWSER[Browser-Dashboard]
    end

    subgraph Router[Lokaler OmniRoute-Prozess]
        API[V1-Kompatibilitäts-API\n/v1/*]
        DASH[Dashboard + Verwaltungs-API\n/api/*]
        CORE[SSE- + Übersetzungskern\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(Nutzungstabellen + Protokollartefakte)]
    end

    subgraph Upstreams[Upstream-Anbieter]
        P1[OAuth-Anbieter\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API-Schlüssel-Anbieter\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Kompatible Knoten\nOpenAI-kompatibel / Anthropic-kompatibel]
    end

    subgraph Cloud[Optionale Cloud-Synchronisierung]
        CLOUD[Cloud-Synchronisierungsendpunkt\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    ...
    participant Model as Modellauflöser
    participant Auth as Zugangsdaten-Auswahl
    participant Exec as Anbieter-Executor
    participant Prov as Vorgelagerter Anbieter
    participant Stream as Stream-Übersetzer
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: Modell oder Kombination parsen/auflösen

    alt Kombinationsmodell
        Chat->>Chat: Kombinationsmodelle durchlaufen (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: aktives Konto + Token/API-Schlüssel

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: Quellformat erkennen
    Core->>Core: Anfrage in Zielformat übersetzen
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: vorgelagerter API-Aufruf
    Prov-->>Exec: SSE-/JSON-Antwort
    Exec-->>Core: Antwort + Metadaten

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: aktualisierte Token
        Core->>Exec: Anfrage erneut versuchen
    end

    Core->>Stream: Stream in Clientformat übersetzen/normalisieren
    Stream-->>Client: SSE-Blöcke / JSON-Antwort

    Stream->>Usage: Nutzungsdaten extrahieren + Verlauf/Protokoll persistieren
```
...
```mermaid
flowchart TD
    A[Eingehende Modellzeichenfolge] --> B{Ist es ein Combo-Name?}
    B -- Ja --> C[Combo-Modellsequenz laden]
    B -- Nein --> D[Pfad für einzelnes Modell]

    C --> E[Modell N ausprobieren]
    E --> F[Anbieter/Modell auflösen]
    D --> F

    F --> G[Kontozugangsdaten auswählen]
    G --> H{Zugangsdaten verfügbar?}
    H -- Nein --> I[Anbieter als nicht verfügbar zurückgeben]
    H -- Ja --> J[Anfrage ausführen]

    J --> K{Erfolgreich?}
    K -- Ja --> L[Antwort zurückgeben]
    K -- Nein --> M{Fallback-fähiger Fehler?}

    M -- Nein --> N[Fehler zurückgeben]
    M -- Ja --> O[Konto für Abklingzeit als nicht verfügbar markieren]
    O --> P{Weiteres Konto für Anbieter vorhanden?}
    P -- Ja --> G
    P -- Nein --> Q{In Combo mit nachfolgendem Modell?}
    Q -- Ja --> E
    Q -- Nein --> R[Alle als nicht verfügbar zurückgeben]
```
...
sequenceDiagram
    autonumber
    participant UI as Dashboard-Benutzeroberfläche
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Authentifizierungsserver des Anbieters
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Anbieter-Executor

    UI->>OAuth: GET für Autorisierung oder Gerätecode
    OAuth->>ProvAuth: Authentifizierungs-/Geräteablauf erstellen
    ProvAuth-->>OAuth: Authentifizierungs-URL oder Gerätecode-Nutzdaten
    OAuth-->>UI: Ablaufdaten

    UI->>OAuth: POST für Austausch oder Abfrage
    OAuth->>ProvAuth: Token-Austausch/-Abfrage
    ProvAuth-->>OAuth: Zugriffs-/Aktualisierungs-Token
    OAuth->>DB: createProviderConnection(oauth-Daten)
    OAuth-->>UI: Erfolg + Verbindungs-ID

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: Zugangsdaten validieren / optional aktualisieren
    Exec-->>Test: gültige oder aktualisierte Token-Informationen
    Test->>DB: Status/Token/Fehler aktualisieren
    Test-->>UI: Validierungsergebnis
```
...
sequenceDiagram
    autonumber
    participant UI as Benutzeroberfläche der Endpunktseite
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Externe Cloud-Synchronisierung
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=true setzen
    Sync->>DB: sicherstellen, dass ein API-Schlüssel vorhanden ist
    Sync->>Cloud: POST /sync/{machineId} (Anbieter/Aliasse/Combos/Schlüssel)
    Cloud-->>Sync: Synchronisierungsergebnis
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: aktiviert + Verifizierungsstatus

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: entfernte Daten
    Sync->>DB: neuere lokale Token/Statusangaben aktualisieren
    Sync-->>UI: synchronisiert

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=false setzen
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URL wieder auf lokal umstellen (falls erforderlich)
    Sync-->>UI: deaktiviert
```
