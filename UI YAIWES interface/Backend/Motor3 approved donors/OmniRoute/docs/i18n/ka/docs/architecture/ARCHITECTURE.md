...
![მოთხოვნის დამუშავების ჯაჭვი (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)

> წყარო: [diagrams/request-pipeline.mmd](../diagrams/request-pipeline.mmd)

![მედეგობის 3-შრიანი მოდელი](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[დეველოპერის კლიენტები]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[OpenAI-სთან თავსებადი მორგებული კლიენტები]
        BROWSER[ბრაუზერის მართვის პანელი]
    end

    subgraph Router[OmniRoute-ის ლოკალური პროცესი]
        API[V1 თავსებადობის API\n/v1/*]
        DASH[მართვის პანელი + მართვის API\n/api/*]
        CORE[SSE + ტრანსლაციის ბირთვი\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(გამოყენების ცხრილები + ჟურნალის არტეფაქტები)]
    end

    subgraph Upstreams[ზედა დონის პროვაიდერები]
        P1[OAuth პროვაიდერები\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API გასაღების პროვაიდერები\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[თავსებადი კვანძები\nOpenAI-სთან თავსებადი / Anthropic-თან თავსებადი]
    end

    subgraph Cloud[არასავალდებულო ღრუბლოვანი სინქრონიზაცია]
        CLOUD[ღრუბლოვანი სინქრონიზაციის საბოლოო წერტილი\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as CLI/SDK კლიენტი
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as მოდელის გადამწყვეტი
    participant Auth as ავტორიზაციის მონაცემების ამრჩევი
    participant Exec as პროვაიდერის შემსრულებელი
    participant Prov as ზედა დონის პროვაიდერი
    participant Stream as ნაკადის გარდამქმნელი
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: მოდელის ან კომბინაციის გარჩევა/გადაწყვეტა

    alt კომბინირებული მოდელი
        Chat->>Chat: კომბინაციის მოდელების იტერაცია (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: აქტიური ანგარიში + ტოკენები/API გასაღები

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: წყაროს ფორმატის ამოცნობა
    Core->>Core: მოთხოვნის სამიზნე ფორმატში გარდაქმნა
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: ზედა დონის API-ის გამოძახება
    Prov-->>Exec: SSE/JSON პასუხი
    Exec-->>Core: პასუხი + მეტამონაცემები

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: განახლებული ტოკენები
        Core->>Exec: მოთხოვნის ხელახლა ცდა
    end

    Core->>Stream: ნაკადის კლიენტის ფორმატში გარდაქმნა/ნორმალიზება
    Stream-->>Client: SSE ფრაგმენტები / JSON პასუხი

    Stream->>Usage: გამოყენების მონაცემების ამოღება + ისტორიის/ჟურნალის შენახვა
```
...
```mermaid
flowchart TD
    A[შემომავალი მოდელის სტრიქონი] --> B{კომბინაციის სახელია?}
    B -- დიახ --> C[კომბინაციის მოდელების თანმიმდევრობის ჩატვირთვა]
    B -- არა --> D[ერთი მოდელის გზა]

    C --> E[მოდელი N-ის ცდა]
    E --> F[პროვაიდერის/მოდელის განსაზღვრა]
    D --> F

    F --> G[ანგარიშის ავტორიზაციის მონაცემების არჩევა]
    G --> H{ავტორიზაციის მონაცემები ხელმისაწვდომია?}
    H -- არა --> I[პროვაიდერის მიუწვდომლობის დაბრუნება]
    H -- დიახ --> J[მოთხოვნის შესრულება]

    J --> K{წარმატებულია?}
    K -- დიახ --> L[პასუხის დაბრუნება]
    K -- არა --> M{შეცდომა სარეზერვო გადართვისთვის დასაშვებია?}

    M -- არა --> N[შეცდომის დაბრუნება]
    M -- დიახ --> O[ანგარიშის დროებით მიუწვდომლად მონიშვნა]
    O --> P{არსებობს სხვა ანგარიში ამ პროვაიდერისთვის?}
    P -- დიახ --> G
    P -- არა --> Q{კომბინაციაში შემდეგი მოდელია?}
    Q -- დიახ --> E
    Q -- არა --> R[ყველას მიუწვდომლობის დაბრუნება]
```
...
sequenceDiagram
    autonumber
    participant UI as მართვის პანელის UI
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as პროვაიდერის ავტორიზაციის სერვერი
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as პროვაიდერის შემსრულებელი

    UI->>OAuth: GET ავტორიზაცია ან მოწყობილობის კოდი
    OAuth->>ProvAuth: ავტორიზაციის/მოწყობილობის ნაკადის შექმნა
    ProvAuth-->>OAuth: ავტორიზაციის URL ან მოწყობილობის კოდის მონაცემები
    OAuth-->>UI: ნაკადის მონაცემები

    UI->>OAuth: POST გაცვლა ან გამოკითხვა
    OAuth->>ProvAuth: ტოკენის გაცვლა/გამოკითხვა
    ProvAuth-->>OAuth: წვდომის/განახლების ტოკენები
    OAuth->>DB: createProviderConnection(oauth მონაცემები)
    OAuth-->>UI: წარმატება + კავშირის id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: ავტორიზაციის მონაცემების ვალიდაცია / არასავალდებულო განახლება
    Exec-->>Test: მოქმედი ან განახლებული ტოკენის ინფორმაცია
    Test->>DB: სტატუსის/ტოკენების/შეცდომების განახლება
    Test-->>UI: ვალიდაციის შედეგი
```
...
sequenceDiagram
    autonumber
    participant UI as საბოლოო წერტილის გვერდის UI
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as გარე ღრუბლოვანი სინქრონიზაცია
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=true-ის დაყენება
    Sync->>DB: API გასაღების არსებობის უზრუნველყოფა
    Sync->>Cloud: POST /sync/{machineId} (პროვაიდერები/ფსევდონიმები/კომბინაციები/გასაღებები)
    Cloud-->>Sync: სინქრონიზაციის შედეგი
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: ჩართულია + ვერიფიკაციის სტატუსი

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: დისტანციური მონაცემები
    Sync->>DB: უფრო ახალი ლოკალური ტოკენების/სტატუსის განახლება
    Sync-->>UI: სინქრონიზებულია

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=false-ის დაყენება
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URL-ის კვლავ ლოკალურზე გადართვა (საჭიროების შემთხვევაში)
    Sync-->>UI: გამორთულია
```
...
```mermaid
flowchart LR
    subgraph LocalHost[დეველოპერის ჰოსტი]
        CLI[CLI ხელსაწყოები]
        Browser[მართვის პანელის ბრაუზერი]
    end

    subgraph ContainerOrProcess[OmniRoute-ის სამუშაო გარემო]
        Next[Next.js სერვერი\nPORT=20128]
        Core[SSE ბირთვი + შემსრულებლები]
        MainDB[(storage.sqlite)]
        UsageDB[(გამოყენების ცხრილები + ჟურნალის არტეფაქტები)]
    end

    subgraph External[გარე სერვისები]
        Providers[AI პროვაიდერები]
        SyncCloud[ღრუბლოვანი სინქრონიზაციის სერვისი]
    end
