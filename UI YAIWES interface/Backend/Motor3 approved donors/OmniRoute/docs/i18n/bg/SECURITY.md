...
## Констатации на скенери за веригата за доставки (Socket.dev / Snyk / подобни)

Публикуваният npm артефакт `omniroute` включва компилацията на Next.js с `output: "standalone"`, което означава, че всеки обработчик на маршрути — включително документираните привилегировани функционалности (MITM, импортиране от Zed, Cloud Sync, вграден надзорник на услуги) — попада в минифицирани фрагменти `.next/server/*.js`. Евристичните скенери за веригата за доставки често съпоставят тези фрагменти със сигнатури на зловреден софтуер.

Конфигурацията на скенера, която използваме, се намира в [`socket.yml`](socket.yml) в корена на хранилището (формат v2 на GitHub App на Socket.dev — вижте <https://docs.socket.dev/docs/socket-yml>). Тя изрично изключва директории, които не се разпространяват (`tests/`, `_tasks/`, `_references/`, `_ideia/`, `_mono_repo/`, `docs/` и др.), така че скенерът да отчита само пътища в кода, които действително достигат до потребителите на публикувания пакет — самото сканиране се извършва от GitHub App на Socket, която прочита този файл, а не от работен процес в това хранилище.

За всяка категория констатации поддържаме декларация от отговорника за всяка отделна констатация:

- **[`docs/security/SOCKET_DEV_FINDINGS.md`](docs/security/SOCKET_DEV_FINDINGS.md)** —
  съпоставка за всяка констатация: файл с изходен код ↔ маркиран фрагмент ↔ поведение ↔ мярка за ограничаване, приложена във v3.8.6.
- Блокове `SECURITY-AUDITOR-NOTE:` в изходния код при всяка маркирана функция, които препращат към същия документ.

За потребители, чиито конвейери не позволяват предупреждението да бъде смекчено: компилирайте с `OMNIROUTE_BUILD_PROFILE=minimal npm run build`. Това заменя четирите чувствителни модула със заглушки, които по време на изпълнение връщат HTTP 503 `feature-disabled`, така че привилегированите пътища в кода физически да отсъстват от пакета. Вижте [`docs/security/SOCKET_DEV_FINDINGS.md`](docs/security/SOCKET_DEV_FINDINGS.md) за процедурата за публикуване.
