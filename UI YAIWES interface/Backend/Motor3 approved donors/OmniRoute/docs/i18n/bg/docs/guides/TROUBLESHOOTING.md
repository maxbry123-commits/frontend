...
Когато изпълните `npm install -g omniroute`, може да видите множество предупреждения като `npm warn ERESOLVE`, известия за peer зависимости и съобщения `deprecated`. **Те са очаквани и безвредни.** Инсталацията е успешна, ако видите `added <N> packages` в изхода.

Предупрежденията идват от остарели диапазони за peer зависимости в пакети на трети страни, които OmniRoute не контролира:

1. **`marked-terminal` изисква `marked >=1 <16`, но е намерен `marked@18`** — на практика работи без проблеми; диапазонът за peer зависимост в upstream пакета просто е остарял.
2. **`deprecated prebuild-install@7.1.3`** — транзитивен помощен инструмент за изтегляне на нативни двоични файлове. Той не се
   използва за инсталиране на фиксирания транспортен binding `wreq-js` и не означава, че настройването на транспорта
   на доставчика за уеб бисквитки е неуспешно.

**Не е необходимо действие** — предупрежденията не могат да бъдат напълно скрити без създаване на fork на upstream пакетите.
