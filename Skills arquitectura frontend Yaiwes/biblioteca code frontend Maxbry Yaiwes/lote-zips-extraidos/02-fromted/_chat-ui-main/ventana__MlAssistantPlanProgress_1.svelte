...
<div class="flex min-w-0 items-center gap-[14px]">
			...
			{#each steps as step, index (index)}
				...
				<span role="listitem" class="contents">
					...
					>
						...
						>
							{#if step.status === "done"}
								<span class="ml-step-glyph grid place-items-center rounded-full bg-[#2563eb]">
							...
							{:else if step.status === "skipped"}
								<span
									class="ml-step-glyph grid place-items-center rounded-full border-[1.5px] border-[#2563eb]"
								>
									<span
										class="hidden h-[1.5px] w-[7px] rounded-[1px] bg-[#2563eb] @min-[340px]:block"
									></span>
								</span>
							...
							{:else}
								<span
									class="ml-step-glyph rounded-full border-[1.5px] border-[#ffffff] dark:border-[#141417]"
								></span>
							{/if}
						</Tooltip.Trigger>
						<Tooltip.Portal>
	...
	<span
		...
		class={[
			...
			complete
				? "font-semibold text-[#2563eb] dark:text-[#2563eb]"
				: "font-medium text-[#a1a1aa] dark:text-[#a1a1aa]",
		]}
	>
		{statusText}
	</span>
...
<style>
	...
	:global(.ml-step-tooltip) {
		...
		line-height: 1.35;
		white-space: nowrap;
		box-shadow: 0 4px 14px rgba(10,10,13,0.18);
