...
(function(){
  ...
  window.addEventListener('message', function(ev){
    ...
    try {
      ...
      if (rootBackground === 'transparent' || rootBackground === 'rgba(10,10,13,0)') rootBackground = '';
...
export function buildHtmlSrcdoc(content: string, channel: string): string {
	...
	if (svgPattern.test(trimmed)) {
		...
		const svgBackground = "<style>html { background: #ffffff; }</style>";
...
<style>html, body { margin: 0; min-height: 100%; background: #ffffff; }</style>
</head>
<body>
...
(function () {
	...
	function fail(message) {
		...
		if (el) {
			...
			pre.style.cssText = "color:#2563eb;font:12px/1.5 ui-monospace,monospace;white-space:pre-wrap;padding:16px;margin:0";
