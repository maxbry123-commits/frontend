...
<div class="ml-strip-collapse @container" class:is-open={visible} inert={!visible}>
	...
	<div
		class="ml-strip flex h-[44px] items-center gap-[10px] border-b border-[#ffffff] bg-white pr-2 pl-[14px] @min-[340px]:gap-[14px] @min-[340px]:pr-[10px] @min-[340px]:pl-[20px] dark:border-[#141417] dark:bg-[#0a0a0d]"
	>
		<span class="flex-none">
			<!-- Two spellings, one accessible name: the narrow one is hidden by CSS,
			     not removed, so both would otherwise be read out. -->
			<span aria-hidden="true" class="block size-2 rounded-[2px] bg-[#2563eb] @min-[240px]:hidden"
			></span>
			<span
				aria-hidden="true"
				class="hidden text-[13px] leading-none font-semibold text-[#2563eb] @min-[240px]:inline dark:text-[#2563eb]"
			>
				ML Intern
			</span>
		...
		{:else}
			<span class="min-w-0 truncate text-[13px] leading-none text-[#a1a1aa] dark:text-[#a1a1aa]">
		...
		{#if dashboard}
			<button
				...
				class={[
					...
					"text-[13px] leading-none font-medium text-[#141417] dark:text-[#a1a1aa]",
					dashboardLive
						? "cursor-pointer hover:bg-black/5 hover:text-[#141417] dark:hover:bg-white/[.07] dark:hover:text-[#ffffff]"
		...
		{#if budget}
			{#if dashboard}
				...
				<span
					aria-hidden="true"
					class="hidden h-[14px] w-px flex-none bg-[#ffffff] @min-[340px]:mx-[-6px] @min-[340px]:block dark:bg-[#141417]"
				></span>
			{/if}
			{#if editingBudget}
				...
				<span
					class="ml-budget-pill flex flex-none items-center gap-px rounded-[6px] px-2 py-[5px] font-mono text-[13px] leading-none font-medium text-[#2563eb] tabular-nums dark:text-[#2563eb]"
					onkeydown={(e) => {
				...
				>
					...
					<input
						...
						class="ml-budget-input min-w-[1ch] border-0 bg-transparent p-0 pb-px text-right font-mono text-[13px] font-medium text-[#141417] tabular-nums outline-none dark:text-[#ffffff]"
						aria-label="Session budget in dollars, Enter to save"
					/>
					<span class="pl-1 text-[#a1a1aa] dark:text-[#a1a1aa]">left</span>
				</span>
			{:else}
				<button
					type="button"
