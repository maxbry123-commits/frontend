...
`@assistant-ui/react-langchain` exposes subagents as discovery; AG-UI exposes them as structure. `useLangChainSubagents()` returns a namespace-keyed map and leaves message fetching to the caller (see [subagent and subgraph views](/docs/runtimes/langchain#subagent-and-subgraph-views)), while this runtime nests the messages directly and offers no discovery hook. Neither runtime provides the other's shape, and the split follows the protocols rather than taste: AG-UI carries an explicit `parentToolCallId` on the lifecycle event, a precise join that LangChain's namespace scheme cannot give, so nesting is available here and not there.
