...
#### Claim Anonymous Threads After Sign-In

When an anonymous visitor signs in, read the browser's refresh token with `readAnonymousRefreshToken(baseUrl)` and send it to your server. The server can claim those threads with an API-key client scoped to the signed-in user's workspace. The response reports how many threads moved.
