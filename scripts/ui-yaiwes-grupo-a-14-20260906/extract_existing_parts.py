...
def source_metadata(item):
    ...
    meta = {'type': 'file', 'sha': item['source_blob'], 'size': item['source_size'], 'mode': item['source_mode']}
...
def prepare_item(root, item, target):
    ...
    cache = Path(os.environ['RDC_SOURCE_ROOT']) / item['component']
    if subprocess.check_output(['git', '-C', str(cache), 'rev-parse', 'HEAD'], text=True).strip() != item['source_commit']:
        raise RuntimeError('SOURCE_COMMIT_MISMATCH')
    target.write_bytes(subprocess.check_output(['git', '-C', str(cache), 'cat-file', 'blob', meta['sha']]))
    if meta['mode'] == '120000':
        link = target.read_text()
        from posixpath import normpath, dirname, join
        resolved = normpath(join(dirname(item['path']), link))
        if not link or link.startswith('/') or '\\' in link or resolved == '..' or resolved.startswith('../'):
            raise RuntimeError('UNSAFE_SOURCE_SYMLINK')
    elif meta['mode'] not in ('100644', '100755'):
        raise RuntimeError('SOURCE_MODE_GAP')
    check_payload(target, meta)
    return target, 'DOWNLOAD_PINNED_GIT_BLOB', meta
...
def publish_batch(batch, plan, number, temp):
    evidence_path = f"{plan['destination_root']}/_adquisicion/{plan['task_id']}/batch-{number:03d}.json"
    ...
    candidates = [(row['destination'], Path(row['staged']), row['blob'], row.get('source_mode', '100644')) for row in batch]
    candidates.append((evidence_path, evidence, git('hash-object', '--no-filters', str(evidence)), '100644'))
    for attempt in range(3):
        git('fetch', 'origin', 'main')
        git('reset', '--hard', 'origin/main')
        for path, src, expected, mode in candidates:
            ...
            git('update-index', '--add', '--cacheinfo', mode, actual, path)
        if subprocess.run(['git', 'diff', '--cached', '--quiet']).returncode == 0:
            return git('rev-parse', 'HEAD')
        git('-c', 'core.hooksPath=/dev/null', 'commit', '-m', f"download: {plan['task_id']} batch {number:03d}")
...
def repair_queue(plan_path):
    ...
    report['expected_components'] = len(plan['components'])
    report['metadata'] = []
    seen = set()
    with tempfile.TemporaryDirectory(prefix='repair-payload-') as td:
        ...
        try:
            ...
            for component in plan['components']:
                component_rows = [row for row in report['files'] if row['component'] == component['name']]
                if len(component_rows) != component['expected_files'] or any('sha256' not in row for row in component_rows):
                    report['blocked'].append({'component': component['name'], 'reason': 'COMPONENT_NOT_COMPLETE_FOR_METADATA'})
                    continue
                source_url = component['source_url']
                source_commit = component['source_commit']
                license_name = '\n'.join(source_url + '/blob/' + source_commit + '/' + path for path in component['license_files'])
                documents = {
                    'SOURCE_URL.txt': source_url.rstrip('/') + '\n',
                    'SOURCE_COMMIT.txt': source_commit + '\n',
                    'SOURCE_LICENSE.txt': license_name + '\n',
                }
                sums = {row['path']: row['sha256'] for row in component_rows if row.get('source_mode') != '120000'}
                for filename, content in documents.items():
                    sums[filename] = hashlib.sha256(content.encode('utf-8')).hexdigest()
                documents['SOURCE_SHA256SUMS.txt'] = ''.join(f'{digest}  {path}\n' for path, digest in sorted(sums.items()))
                for filename, content in documents.items():
                    staged = temp / ('metadata-' + str(len(report['metadata'])))
                    staged.write_text(content, encoding='utf-8')
                    row = dict(component=component['name'], path=filename,
                               destination=(root / component['name'] / filename).as_posix(),
                               source_url=source_url, source_commit=source_commit, source_mode='100644',
                               operation='TRACE_METADATA', status='PREPARED',
                               staged=str(staged), blob=git('hash-object', '--no-filters', str(staged)),
                               sha256=sha256(staged), bytes=staged.stat().st_size)
                    batch.append(row)
                    report['metadata'].append(row)
            if batch:
                commit = publish_batch(batch, plan, len(report['batches']) + 1, temp)
                report['batches'].append(commit)
                for row in batch:
            ...
            for row in report['files'] + report['metadata']:
                row.pop('staged', None)
