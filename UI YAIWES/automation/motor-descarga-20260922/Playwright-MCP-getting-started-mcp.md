...
-   With `--shared-browser-context`, the timer spans all clients: a client that is idle while others keep working keeps its tabs and the shared browser is closed only once every client has been idle for the timeout. 
