...
static LRESULT CALLBACK subWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (uMsg == WM_PAINT) {
        auto user_data = (SubWindowUserData*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
        if (user_data && user_data->repaint_callback) {
            user_data->repaint_callback(user_data->repaint_callback_param);
        }
    } else if (uMsg == WM_NCDESTROY) {
        SubWindowUserData* user_data = (SubWindowUserData*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
        delete user_data;
    }
...
EGLNativeWindowType createSubWindow(FBNativeWindowType p_window, int x, int y, int width,
    ...
    EGLNativeWindowType ret = CreateWindowExA(
        ...
        className, "sub", WS_CHILD | WS_DISABLED, x, y, width, height, p_window, NULL, NULL, NULL);
    ...
    if (!hideWindow) ShowWindow(ret, SW_SHOW);
    return ret;
}

void destroySubWindow(EGLNativeWindowType win) { PostMessage(win, WM_CLOSE, 0, 0); }
...
int moveSubWindow(FBNativeWindowType p_parent_window, EGLNativeWindowType p_sub_window, int x,
    ...
    BOOL ret = MoveWindow(p_sub_window, x * dpr, y * dpr, width * dpr, height * dpr, TRUE);
    return ret;
}
