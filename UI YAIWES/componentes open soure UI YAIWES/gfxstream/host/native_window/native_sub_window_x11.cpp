...
EGLNativeWindowType createSubWindow(FBNativeWindowType p_window, int x, int y, int width,
    ...
    Window win =
        x11->XCreateWindow(s_display, p_window, x * dpr, y * dpr, width * dpr, height * dpr, 0,
                           CopyFromParent, CopyFromParent, CopyFromParent, CWEventMask, &wa);
    if (!hideWindow) {
...
int moveSubWindow(FBNativeWindowType p_parent_window, EGLNativeWindowType p_sub_window, int x,
    ...
    if (x == attrs.x && y == attrs.y && width == attrs.width && height == attrs.height) {
    ...
    int ret = x11->XMoveResizeWindow(s_display, p_sub_window, x, y, width, height);
...
void* getNativeDisplay() { return s_display; }
