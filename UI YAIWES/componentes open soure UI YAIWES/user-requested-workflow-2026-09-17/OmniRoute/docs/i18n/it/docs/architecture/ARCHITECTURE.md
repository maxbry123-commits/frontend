...
```mermaid
flowchart LR
    subgraph Clients[Client per Sviluppatori]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Client personalizzati compatibili con OpenAI]
        BROWSER[Dashboard del Browser]
    end

    subgraph Router[Processo Locale OmniRoute]
    ...
    subgraph Upstreams[Provider Upstream]
        ...
        P2[Provider con Chiave API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Nodi Compatibili\nCompatibili con OpenAI / compatibili con Anthropic]
    end
...
```mermaid
flowchart TD
    A[Stringa del modello in ingresso] --> B{È il nome di una combo?}
    B -- Sì --> C[Carica la sequenza di modelli della combo]
    B -- No --> D[Percorso per modello singolo]
    ...
    H -- Sì --> J[Esegui la richiesta]

    J --> K{Operazione riuscita?}
    K -- Sì --> L[Restituisci la risposta]
    K -- No --> M{Errore idoneo al fallback?}

    M -- No --> N[Restituisci l'errore]
    M -- Sì --> O[Contrassegna l'account come non disponibile per il periodo di attesa]
    O --> P{Un altro account per il provider?}
    P -- Sì --> G
    P -- No --> Q{In una combo con un modello successivo?}
    Q -- Sì --> E
    Q -- No --> R[Restituisci tutti come non disponibili]
```
...
sequenceDiagram
    autonumber
    participant UI as Interfaccia della dashboard
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Server di autenticazione del provider
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Esecutore del provider

    UI->>OAuth: GET per autorizzazione o codice dispositivo
    OAuth->>ProvAuth: crea il flusso di autenticazione/dispositivo
    ProvAuth-->>OAuth: URL di autenticazione o payload del codice dispositivo
    OAuth-->>UI: dati del flusso

    UI->>OAuth: POST per scambio o polling
    OAuth->>ProvAuth: scambio/polling del token
    ProvAuth-->>OAuth: token di accesso/aggiornamento
    OAuth->>DB: createProviderConnection(dati OAuth)
    OAuth-->>UI: esito positivo + ID connessione

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: convalida le credenziali / aggiornamento facoltativo
    Exec-->>Test: informazioni sul token valido o aggiornato
    Test->>DB: aggiorna stato/token/errori
    Test-->>UI: risultato della convalida
```
...
sequenceDiagram
    autonumber
    participant UI as Interfaccia della pagina degli endpoint
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Sincronizzazione cloud esterna
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: imposta cloudEnabled=true
    Sync->>DB: verifica che esista una chiave API
    Sync->>Cloud: POST /sync/{machineId} (provider/alias/combo/chiavi)
    Cloud-->>Sync: risultato della sincronizzazione
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: attivata + stato della verifica

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: dati remoti
    Sync->>DB: aggiorna i token/stati locali più recenti
    Sync-->>UI: sincronizzata

    UI->>Sync: POST action=disable
    Sync->>DB: imposta cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ripristina ANTHROPIC_BASE_URL all'indirizzo locale (se necessario)
    Sync-->>UI: disattivata
```
