...
```mermaid
flowchart LR
    subgraph Clients[개발자 클라이언트]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[사용자 지정 OpenAI 호환 클라이언트]
        BROWSER[브라우저 대시보드]
    end

    subgraph Router[OmniRoute 로컬 프로세스]
        API[V1 호환성 API\n/v1/*]
        DASH[대시보드 + 관리 API\n/api/*]
        CORE[SSE + 변환 코어\nopen-sse + src/sse]
        DB[(storage.sqlite)]
    ...
    subgraph Upstreams[업스트림 제공자]
        P1[OAuth 제공자\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API 키 제공자\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[호환 노드\nOpenAI 호환 / Anthropic 호환]
    end

    subgraph Cloud[선택적 클라우드 동기화]
        CLOUD[클라우드 동기화 엔드포인트\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as CLI/SDK 클라이언트
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as 모델 리졸버
    participant Auth as 자격 증명 선택기
    participant Exec as 제공자 실행기
    participant Prov as 업스트림 제공자
    participant Stream as 스트림 변환기
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: 모델 또는 콤보 파싱/리졸브

    alt 콤보 모델
        Chat->>Chat: 콤보 모델 순회 (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: 활성 계정 + 토큰/API 키

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: 소스 형식 감지
    Core->>Core: 요청을 대상 형식으로 변환
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: 업스트림 API 호출
    Prov-->>Exec: SSE/JSON 응답
    Exec-->>Core: 응답 + 메타데이터
    ...
    Core->>Stream: 스트림을 클라이언트 형식으로 변환/정규화
    Stream-->>Client: SSE 청크 / JSON 응답

    Stream->>Usage: 사용량 추출 + 기록/로그 영속화
```
...
flowchart TD
    ...
    F --> G[계정 자격 증명 선택]
    G --> H{자격 증명을 사용할 수 있는가?}
    H -- 아니요 --> I[공급자 사용 불가 반환]
    H -- 예 --> J[요청 실행]
    ...
    M -- 아니요 --> N[오류 반환]
    M -- 예 --> O[계정을 쿨다운 상태로 사용 불가 처리]
    O --> P{해당 공급자의 다른 계정이 있는가?}
    P -- 예 --> G
    P -- 아니요 --> Q{콤보에 다음 모델이 있는가?}
    Q -- 예 --> E
    Q -- 아니요 --> R[모두 사용 불가 반환]
```
...
sequenceDiagram
    ...
    participant ProvAuth as 공급자 인증 서버
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as 공급자 실행기

    UI->>OAuth: GET authorize 또는 device-code
    OAuth->>ProvAuth: 인증/기기 흐름 생성
    ProvAuth-->>OAuth: 인증 URL 또는 기기 코드 페이로드
    OAuth-->>UI: 흐름 데이터

    UI->>OAuth: POST exchange 또는 poll
    OAuth->>ProvAuth: 토큰 교환/폴링
    ProvAuth-->>OAuth: 액세스/갱신 토큰
    OAuth->>DB: createProviderConnection(OAuth 데이터)
    OAuth-->>UI: 성공 + 연결 ID

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: 자격 증명 검증 / 선택적 갱신
    Exec-->>Test: 유효한 토큰 또는 갱신된 토큰 정보
    Test->>DB: 상태/토큰/오류 업데이트
    Test-->>UI: 검증 결과
```
...
sequenceDiagram
    autonumber
    participant UI as 엔드포인트 페이지 UI
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as 외부 클라우드 동기화
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=true 설정
    Sync->>DB: API 키 존재 확인
    Sync->>Cloud: POST /sync/{machineId} (공급자/별칭/콤보/키)
    Cloud-->>Sync: 동기화 결과
    ...
    Cloud-->>Sync: 원격 데이터
    Sync->>DB: 더 최신인 로컬 토큰/상태 업데이트
    Sync-->>UI: 동기화됨

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=false 설정
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URL을 로컬로 다시 전환(필요한 경우)
    Sync-->>UI: 비활성화됨
```
...
```mermaid
erDiagram
    SETTINGS ||--o{ PROVIDER_CONNECTION : 제어
    PROVIDER_NODE ||--o{ PROVIDER_CONNECTION : 호환_프로바이더_지원
    PROVIDER_CONNECTION ||--o{ USAGE_ENTRY : 사용량_생성
...
```mermaid
flowchart LR
    subgraph LocalHost[개발자 호스트]
        CLI[CLI 도구]
        Browser[대시보드 브라우저]
    end

    subgraph ContainerOrProcess[OmniRoute 런타임]
        Next[Next.js 서버\nPORT=20128]
        Core[SSE 코어 + 실행기]
        MainDB[(storage.sqlite)]
        UsageDB[(사용량 테이블 + 로그 산출물)]
    end

    subgraph External[외부 서비스]
        Providers[AI 프로바이더]
        SyncCloud[클라우드 동기화 서비스]
    end
