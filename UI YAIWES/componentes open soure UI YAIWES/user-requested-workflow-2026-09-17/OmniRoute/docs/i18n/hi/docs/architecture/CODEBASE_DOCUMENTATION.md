...
App Router डैशबोर्ड UI और सार्वजनिक/प्रबंधन HTTP API, दोनों उपलब्ध कराता है।
यहाँ **कोई ग्लोबल मिडलवेयर नहीं है** — इंटरसेप्शन प्रत्येक रूट पर अलग से किया जाता है।

`src/app/` के अंतर्गत शीर्ष-स्तरीय सेगमेंट:

| पथ                                                                            | उद्देश्य                                        |
| ----------------------------------------------------------------------------- | ----------------------------------------------- |
| `api/`                                                                        | सभी HTTP API रूट (नीचे विवरण देखें)             |
| `a2a/`                                                                        | A2A JSON-RPC 2.0 एंडपॉइंट (`POST /a2a`)         |
| `.well-known/agent.json/`                                                     | A2A Agent Card डिस्कवरी दस्तावेज़               |
| `(dashboard)/`                                                                | डैशबोर्ड UI (रूट समूह, कोई URL प्रीफ़िक्स नहीं) |
| `auth/`, `login/`, `forgot-password/`, `callback/`                            | प्रमाणीकरण फ़्लो                                |
| `landing/`                                                                    | मार्केटिंग/लैंडिंग पेज                          |
| `docs/`                                                                       | एम्बेडेड API दस्तावेज़ व्यूअर                   |
| `status/`, `maintenance/`, `offline/`                                         | परिचालन पेज                                     |
| `privacy/`, `terms/`                                                          | कानूनी पेज                                      |
| `400/`, `401/`, `403/`, `408/`, `429/`, `500/`, `502/`, `503/`                | स्थिर त्रुटि पेज                                |
| `error.tsx`, `global-error.tsx`, `not-found.tsx`, `forbidden/`, `loading.tsx` | फ़्रेमवर्क त्रुटि/लोडिंग सीमाएँ                 |
| `layout.tsx`, `page.tsx`, `globals.css`, `manifest.ts`                        | रूट शेल                                         |
...
डेटा, सिंक, OAuth, कौशल, मेमोरी आदि को हमेशा इन्हीं मॉड्यूल के माध्यम से इम्पोर्ट करें।
तालिका वास्तविक डायरेक्टरियों और उल्लेखनीय शीर्ष-स्तरीय फ़ाइलों को समूहित करती है।

| मॉड्यूल           | उद्देश्य                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `a2a/`            | A2A प्रोटोकॉल सर्वर: `taskManager.ts`, `streaming.ts`, `taskExecution.ts`, `routingLogger.ts`, `skills/` (6 कौशल: लागत विश्लेषण, स्वास्थ्य रिपोर्ट, प्रदाता खोज, कोटा प्रबंधन, स्मार्ट रूटिंग, क्षमताओं की सूची)                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `acp/`            | Agent-Control-Protocol: `index.ts`, `manager.ts`, `registry.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `api/`            | आंतरिक API सहायक: `requireManagementAuth.ts`, `requireCliToolsAuth.ts`, `errorResponse.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `auth/`           | `managementPassword.ts` (पासवर्ड रीसेट / हैशिंग)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `batches/`        | OpenAI Batches API सेवा (`service.ts`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `catalog/`        | OpenRouter कैटलॉग सिंक (`openrouterCatalog.ts`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `cloudAgent/`     | क्लाउड एजेंट रजिस्ट्री: `api.ts`, `baseAgent.ts`, `db.ts`, `index.ts`, `registry.ts`, `types.ts`, `agents/{codex, devin, jules}.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `combos/`         | कॉम्बो समाधान सहायक                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `compliance/`     | ऑडिट + प्रदाता ऑडिट: `index.ts`, `providerAudit.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `config/`         | रनटाइम कॉन्फ़िगरेशन संयोजन                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `db/`             | SQLite डोमेन मॉड्यूल (§3.2.1 देखें)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `display/`        | API प्रतिक्रियाओं द्वारा उपयोग किए जाने वाले UI/डिस्प्ले सहायक                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `embeddings/`     | एम्बेडिंग सेवा रजिस्ट्री                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `env/`            | एनवायरनमेंट लोडिंग + आत्मनिरीक्षण                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `evals/`          | मूल्यांकन रनटाइम                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `guardrails/`     | `piiMasker.ts`, `promptInjection.ts`, `visionBridge.ts`, `visionBridgeHelpers.ts`, `registry.ts`, `base.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `jobs/`           | बैकग्राउंड जॉब्स (`autoUpdate.ts`, …)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `oauth/`          | OAuth/आयात प्रदाता मॉड्यूल (22): `agy`, `antigravity`, `claude`, `cline`, `codebuddy-cn`, `codex`, `cursor`, `devin-desktop`, `ghe-copilot`, `github`, `gitlab-duo`, `grok-cli-oauth`, `grok-cli`, `kilocode`, `kimi-coding`, `kiro`, `openference`, `qoder`, `trae`, `xai-oauth`, `zed-hosted`, `zed`, साथ ही `services/`, `utils/`, और `constants/oauth.ts`                                                                                                                                                                                                                                                                                                                            |
| `plugins/`        | प्लगइन लोडर (`index.ts`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `promptCache/`    | `prefixAnalyzer.ts`, `index.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `providerModels/` | प्रबंधित मॉडल जीवनचक्र: `modelDiscovery.ts`, `managedModelImport.ts`, `managedAvailableModels.ts`, `cursorAgent.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `providers/`      | प्रदाता सहायक: `catalog.ts`, `validation.ts`, `imageValidation.ts`, `claudeExtraUsage.ts`, `codexConnectionDefaults.ts`, `codexFastTier.ts`, `webCookieAuth.ts`, `managedAvailableModels.ts`, `requestDefaults.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `resilience/`     | `settings.ts` — सर्किट ब्रेकर, कूलडाउन और लॉकआउट की सेटिंग्स                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `runtime/`        | रनटाइम फ़ीचर पहचान                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `search/`         | `executeWebSearch.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `services/`       | एम्बेडेड सेवा फ़्रेमवर्क: `ServiceSupervisor.ts` (ऑपरेशन लॉक, रिंग बफ़र और स्वास्थ्य जाँचकर्ता वाला सामान्य चाइल्ड-प्रोसेस पर्यवेक्षक), `bootstrap.ts` (प्रोसेस-स्तरीय पंजीकरण और स्वतः-प्रारंभ), `registry.ts` (टूल → पर्यवेक्षक मैप), `apiKey.ts` (AES-256-GCM कुंजी संग्रह), `modelSync.ts` (आवधिक मॉडल सिंक), `ringBuffer.ts` (5 MB सर्कुलर लॉग बफ़र), `healthCheck.ts` (HTTP स्वास्थ्य जाँच), `types.ts`, `embedWsProxy.ts` (WebSocket प्रॉक्सी), `installers/{ninerouter,cliproxy}.ts`। `docs/frameworks/EMBEDDED-SERVICES.md` देखें                                                                                                                                               |
| `agentSkills/`    | एजेंट कौशल कैटलॉग + जनरेटर: `catalog.ts` (getCatalog/getSkillById/filterCatalog/computeCoverage), `generator.ts` (generateAgentSkills → `skills/{id}/SKILL.md` लिखता है), `openapiParser.ts` (OpenAPI विनिर्देश से REST एंडपॉइंट निकालता है), `cliRegistryParser.ts` (bin/cli-registry से CLI उपकमांड निकालता है), `schemas.ts` (Zod: AgentSkillSchema, SkillCoverageSchema, ListQuerySchema, GenerateBodySchema), `types.ts` (AgentSkill, SkillCoverage, SkillMarkdown, GeneratorReport)। REST रूट्स (`/api/agent-skills/*`), MCP टूल्स (`omniroute_agent_skills_*`) और A2A कौशल `list-capabilities` द्वारा उपयोग किया जाता है। [AGENT-SKILLS.md](../frameworks/AGENT-SKILLS.md) देखें। |
| `skills/`         | कौशल फ़्रेमवर्क: `registry.ts`, `executor.ts`, `interception.ts`, `injection.ts`, `sandbox.ts`, `custom.ts`, `hybrid.ts`, `builtins.ts`, `a2a.ts`, `providerSettings.ts`, `schemas.ts`, `skillssh.ts`, `types.ts`, साथ ही `builtin/browser.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `spend/`          | `batchWriter.ts` (राइट-बिहाइंड बफ़र)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `sync/`           | `bundle.ts`, `tokens.ts` (क्लाउड सिंक)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `system/`         | सिस्टम-स्तरीय सहायक                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `translator/`     | शीर्ष-स्तरीय अनुवादक संयोजन (`open-sse/translator/` को प्रत्यायोजित करता है)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `usage/`          | उपयोग लेखांकन: `costCalculator.ts`, `tokenAccounting.ts`, `usageHistory.ts`, `aggregateHistory.ts`, `usageStats.ts`, `callLogs.ts`, `callLogArtifacts.ts`, `fetcher.ts`, `providerLimits.ts`, `migrations.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `versionManager/` | स्वतः-अपडेट + संस्करण मैनिफ़ेस्ट                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `ws/`             | WebSocket ब्रिज                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `zed-oauth/`      | Zed एडिटर OAuth प्रवाह                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |

`src/lib/` में शीर्ष-स्तरीय फ़ाइलें:

- पुराना `localDb.ts` बैरल हटा दिया गया था — उपभोक्ता विशिष्ट `src/lib/db/*` मॉड्यूल को सीधे इम्पोर्ट करते हैं।
...
![डेटाबेस स्कीमा का अवलोकन (चयनित मुख्य टेबल)](../diagrams/exported/db-schema-overview.svg)
...
| फ़ाइल                                      | उद्देश्य                                            |
| ------------------------------------------ | --------------------------------------------------- |
| `policyEngine.ts`                          | शीर्ष-स्तरीय पॉलिसी रिज़ॉल्वर                       |
| `fallbackPolicy.ts`                        | फ़ॉलबैक निर्णय ट्री                                 |
| `costRules.ts`                             | लागत गणना के नियम                                   |
| `lockoutPolicy.ts`                         | मॉडल लॉकआउट संबंधी निर्णय                           |
| `tagRouter.ts`                             | टैग-आधारित रूटिंग                                   |
| `comboResolver.ts`                         | अनुरोध → लक्ष्य सूची से कॉम्बो रिज़ॉल्यूशन          |
| `connectionModelRules.ts`                  | प्रति-कनेक्शन मॉडल फ़िल्टर                          |
| `modelAvailability.ts`                     | मॉडल उपलब्धता की जाँच                               |
| `degradation.ts`                           | डिग्रेडेड-मोड ट्रांज़िशन                            |
| `providerExpiration.ts`                    | समाप्त हो चुके अकाउंट/कुंजी की पहचान                |
| `quotaCache.ts`                            | कैश किए गए कोटा संबंधी निर्णय                       |
| `responses.ts`, `omnirouteResponseMeta.ts` | रिस्पॉन्स संरचना के सहायक                           |
| `configAudit.ts`                           | कॉन्फ़िगरेशन परिवर्तन का ऑडिट                       |
| `assessment/`                              | मॉडल आकलन (RFC के अनुसार, आंशिक रूप से कार्यान्वित) |
| `types.ts`                                 | साझा डोमेन प्रकार                                   |
...
- `constants/` — `providers.ts` (Zod द्वारा सत्यापित प्रदाता कैटलॉग), `models.ts`,
  `modelSpecs.ts`, `modelCompat.ts`, `pricing.ts`, `cliTools.ts`,
  `cliCompatProviders.ts`, `routingStrategies.ts`, `comboConfigMode.ts`,
  `headers.ts`, `upstreamHeaders.ts` (अस्वीकृति सूची), `mcpScopes.ts`,
...
- `contracts/` — npm पर उपलब्ध कराए गए सार्वजनिक API अनुबंध।
- `types/` — साझा TS प्रकार।
- `utils/` — `circuitBreaker.ts`, `apiAuth.ts`, `apiKey.ts`, `apiKeyPolicy.ts`,
  ...
  `a11yAudit.ts`, साथ ही `services/`, `network/`,
  `middleware/`, `schemas/`, `hooks/`, `components/` के अंतर्गत डैशबोर्ड हुक/कंपोनेंट।
...
प्रोसेसिंग, एक्ज़ीक्यूटर, ट्रांसलेटर, सेवाओं, ट्रांसफ़ॉर्मर और MCP सर्वर का प्रबंधन करता है।
...
├── tsconfig.json
├── types.d.ts
├── config/                 प्रोवाइडर रजिस्ट्री, हेडर प्रोफ़ाइल, पहचान, …
├── handlers/               अनुरोध हैंडलर (चैट, एम्बेडिंग, ऑडियो, इमेज, …)
├── executors/              108 प्रोवाइडर-विशिष्ट HTTP एक्ज़ीक्यूटर
├── translator/             प्रारूप रूपांतरण (OpenAI ↔ Claude ↔ Gemini ↔ Cursor ↔ Kiro)
...
| `usageExtractor.ts`     | अपस्ट्रीम स्ट्रीम से टोकन संख्या निकालना                               |
| `responseSanitizer.ts`  | प्रोवाइडर-विशिष्ट अवांछित सामग्री हटाना                                |
| `responseTranslator.ts` | प्रोवाइडर प्रतिक्रिया और ट्रांसलेटर लेयर के बीच संयोजक                 |

### 4.2 `open-sse/executors/`

108 प्रोवाइडर एक्ज़ीक्यूटर, जिनमें से प्रत्येक `BaseExecutor` (`base.ts`) को विस्तारित करता है:
...
> नोट: यहाँ सूचीबद्ध नहीं किए गए प्रोवाइडर को जेनेरिक
> OpenAI-संगत एक्ज़ीक्यूटर का उपयोग करके `default.ts` द्वारा सेवा दी जाती है। संपूर्ण प्रोवाइडर कैटलॉग (355 प्रोवाइडर)
...
- `responsesTransformer.ts` — `TransformStream`-आधारित Responses API ↔ Chat
  Completions कन्वर्टर (`responses/` रूट कैच-ऑल द्वारा उपयोग किया जाता है)।

### 4.5 `open-sse/services/`

मुख्य विशेषताएँ (पूरी सूची `open-sse/services/` के अंतर्गत है):
...
| लचीलापन           | `accountFallback.ts` (कूलडाउन + लॉकआउट), `errorClassifier.ts`, `emergencyFallback.ts`, `rateLimitManager.ts`, `rateLimitSemaphore.ts`, `accountSemaphore.ts`, `accountSelector.ts`                                                                |
...
| संपीड़न           | `compression/` — पूर्ण संपीड़न इंजन वायरिंग                                                                                                                                                                                                       |
...
- `server.ts` में **110 अद्वितीय टूल** वायर किए गए हैं (`schemas/tools.ts` में 45 कैनोनिकल +
...
- रनटाइम पर **33 स्कोप** लागू किए जाते हैं — आधार सूची `src/shared/constants/mcpScopes.ts` में है, जबकि पूरा सेट प्रत्येक टूल मॉड्यूल द्वारा घोषित स्कोप्स का यूनियन है।
...
`providerHeaderProfiles.ts`), प्रत्येक फ़ॉर्मेट के लिए मॉडल रजिस्ट्रियाँ (`audioRegistry.ts`,
`embeddingRegistry.ts`, `imageRegistry.ts`, `moderationRegistry.ts`,
`musicRegistry.ts`, `rerankRegistry.ts`, `searchRegistry.ts`, `videoRegistry.ts`),
आइडेंटिटी हेल्पर्स (`codexIdentity.ts`, `codexInstructions.ts`,
`anthropicHeaders.ts`, `antigravityUpstream.ts`, `antigravityModelAliases.ts`,
`cliFingerprints.ts`, `toolCloaking.ts`, `defaultThinkingSignature.ts`),
क्रेडेंशियल हेल्पर्स (`credentialLoader.ts`, `codexClient.ts`), और क्लाउड
एडेप्टर्स (`azureAi.ts`, `bedrock.ts`, `datarobot.ts`, `glmProvider.ts`,
...
| डायरेक्टरी                                           | प्रकार                                                                                                      |
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `tests/unit/`                                        | Node नेटिव टेस्ट रनर के माध्यम से यूनिट टेस्ट (1821 फ़ाइलें, साथ ही `api/`, `auth/`, `authz/` सबडायरेक्टरी) |
| `tests/integration/`                                 | क्रॉस-मॉड्यूल + DB-स्थिति टेस्ट                                                                             |
| `tests/e2e/`                                         | Playwright UI टेस्ट                                                                                         |
| `tests/e2e/protocol-clients.test.ts`                 | MCP/A2A प्रोटोकॉल e2e                                                                                       |
| `tests/translator/`                                  | अनुवादक-विशिष्ट टेस्ट                                                                                       |
| `tests/security/`                                    | सुरक्षा रिग्रेशन                                                                                            |
| `tests/load/`                                        | लोड / स्ट्रेस टेस्ट                                                                                         |
| `tests/golden-set/`                                  | अनुवादक रिग्रेशन के लिए संदर्भ आउटपुट                                                                       |
...
| -------------------------------------------------------- | --------------------------------------------------------------------- |
| `npm run test:unit`                                      | Node टेस्ट रनर के माध्यम से सभी `tests/unit/*.test.ts` (समवर्तीता 10) |
...
| `npm run test:coverage`                                  | कवरेज गेट (पंक्तियाँ/स्टेटमेंट/फ़ंक्शन/ब्रांच ≥60%)                   |
| `node --import tsx/esm --test tests/unit/<file>.test.ts` | एकल फ़ाइल निष्पादन                                                    |

---

## 8. `scripts/`

उद्देश्य के अनुसार 6 उप-फ़ोल्डरों में व्यवस्थित।
...
[RESILIENCE_GUIDE.md](./RESILIENCE_GUIDE.md) और
[CLAUDE.md](../../CLAUDE.md) में समर्पित अनुभाग देखें।
...
1. `src/shared/constants/providers.ts` में पंजीकृत करें (लोड होते समय Zod द्वारा सत्यापित)।
2. यदि कस्टम लॉजिक आवश्यक हो, तो `open-sse/executors/` में एक एक्ज़ीक्यूटर जोड़ें
   (`BaseExecutor` को विस्तारित करें)।
3. यदि वह OpenAI प्रारूप का उपयोग नहीं करता है, तो `open-sse/translator/` में एक ट्रांसलेटर जोड़ें।
4. यदि वह OAuth-आधारित है, तो `src/lib/oauth/providers/` और
   `src/lib/oauth/services/` के अंतर्गत कॉन्फ़िगरेशन जोड़ें।
5. मॉडलों को `open-sse/config/providerRegistry.ts` में पंजीकृत करें (या
   `open-sse/config/` के अंतर्गत प्रारूप-विशिष्ट रजिस्ट्री में)।
6. `tests/unit/` के अंतर्गत परीक्षण लिखें।

### नया API रूट जोड़ें

1. `src/app/api/your-route/route.ts` बनाएँ।
2. इस पैटर्न का पालन करें: CORS → Zod बॉडी सत्यापन → प्रमाणीकरण → हैंडलर को प्रत्यायोजन।
3. यदि अनुरोध की संरचना नई है: Zod स्कीमा को `src/shared/validation/schemas.ts` में जोड़ें।
4. यदि यह केवल प्रबंधन के लिए है: पथ को `src/shared/constants/publicApiRoutes.ts`
   में जोड़ें (सार्वजनिक API सतह के लिए निषेध-सूची)।
5. `tests/unit/` के अंतर्गत परीक्षण जोड़ें।
6. `docs/reference/API_REFERENCE.md` और `docs/openapi.yaml` को अपडेट करें।

### नया DB मॉड्यूल जोड़ें

1. `src/lib/db/yourModule.ts` बनाएँ और `./core.ts` से `getDbInstance()` आयात करें।
2. अपने डोमेन के लिए CRUD फ़ंक्शन निर्यात करें।
3. यदि नई तालिकाएँ हैं: `src/lib/db/migrations/` के अंतर्गत क्रमिक संख्या वाली,
   पुनरावृत्ति-सुरक्षित और ट्रांज़ैक्शनल माइग्रेशन जोड़ें।
4. आयातकर्ता `@/lib/db/yourModule` से सीधे आयात का उपयोग करते हैं (कोई बैरल नहीं — पुरानी `localDb.ts` पुनः-निर्यात परत हटा दी गई थी)।
5. `tests/unit/` के अंतर्गत परीक्षण जोड़ें।

### नया MCP टूल जोड़ें

1. `open-sse/mcp-server/tools/` के अंतर्गत टूल की परिभाषा जोड़ें (या
   `open-sse/mcp-server/schemas/tools.ts` को विस्तारित करें)।
2. `src/shared/constants/mcpScopes.ts` में उपयुक्त स्कोप निर्दिष्ट करें।
3. टूल को `open-sse/mcp-server/server.ts` में पंजीकृत करें।
4. `open-sse/mcp-server/__tests__/` के अंतर्गत परीक्षण जोड़ें।
5. [MCP-SERVER.md](../frameworks/MCP-SERVER.md) को अपडेट करें।

### नया A2A कौशल जोड़ें

[A2A-SERVER.md § नया कौशल जोड़ना](../frameworks/A2A-SERVER.md) देखें। कौशल
...
- **कोड शैली**: 2-स्पेस इंडेंट, दोहरे उद्धरण-चिह्न, 100 वर्ण चौड़ाई, सेमीकोलन,
  `es5` ट्रेलिंग कॉमा — `lint-staged` के माध्यम से Prettier द्वारा लागू।
- **आयात**: बाहरी → आंतरिक (`@/`, `@omniroute/open-sse`) → सापेक्ष।
- **नामकरण**: फ़ाइलें `camelCase` या `kebab-case`, कॉम्पोनेंट `PascalCase`,
  कॉन्स्टेंट `UPPER_SNAKE`।
- **ESLint**: `no-eval`, `no-implied-eval`, `no-new-func` = हर जगह `error`;
  `no-explicit-any` = `open-sse/` और `tests/` में `warn`, अन्यत्र `error`।
- **TypeScript**: `strict: false` (विरासती स्थिति)। क्रॉस-मॉड्यूल सीमाओं के लिए
  अनुमानित प्रकारों की तुलना में स्पष्ट प्रकारों को प्राथमिकता दें।
- **डेटाबेस**: रूट या हैंडलर में कभी भी रॉ SQL न लिखें — हमेशा
  `src/lib/db/` मॉड्यूल के माध्यम से जाएँ। कभी बैरल-आयात न करें — विशिष्ट `src/lib/db/*` मॉड्यूल का सीधे उपयोग करें।
- **DB-एंटिटी टाइपिंग (#3512)**: DB तालिका के रो आकार को लिखने या पढ़ने वाले फ़ंक्शन को
  उस तालिका के कॉलम को 1:1 प्रतिबिंबित करने वाला नामित TS इंटरफ़ेस लेना/लौटाना चाहिए,
  न कि कॉल साइट पर `any` या कोई इनलाइन अनाम प्रकार। इंटरफ़ेस को फ़ंक्शन के पास रखें
  (उदाहरण के लिए, `saveRequestUsage` के ऊपर `src/lib/usage/usageHistory.ts` में
  `export interface UsageEntry`), जब अलग-अलग राइटर रो को क्रमिक रूप से भरते हों,
  तो अलग-अलग फ़ील्ड को वैकल्पिक/नल-योग्य रखें, और ऐसे फ़ील्ड के लिए `any` के बजाय
  `unknown` को प्राथमिकता दें जिसका आकार कॉलर के अनुसार बदलता है (फ़ील्ड पर इसे
  प्रलेखित करें, उदाहरण के लिए `UsageEntry.tokens` रॉ प्रदाता-आकार वाले उपयोग और
  सामान्यीकृत आकार, दोनों को स्वीकार करता है)। जब इस तरह किसी फ़ाइल की `any` संख्या
  शून्य हो जाए, तो उसे `check:any-budget:t11` अनुमति-सूची
  (`scripts/check/check-t11-any-budget.mjs`, `maxAny: 0`) में जोड़ें, ताकि वह
  वापस न बढ़ सके। यह प्रथम-खंड परिपाटी है — व्यापक "कोई अनाम `any` नहीं" सफ़ाई
  शेष कोडबेस में पुनरावृत्तीय रूप से की जाती है।
- **त्रुटियाँ**: विशिष्ट त्रुटि प्रकारों के साथ try/catch का उपयोग करें, pino संदर्भ के साथ लॉग करें। SSE स्ट्रीम में
  त्रुटियों को कभी भी चुपचाप न दबाएँ; क्लीनअप के लिए अबॉर्ट सिग्नल का उपयोग करें।
- **सुरक्षा**: कभी भी `eval()` / `new Function()` / अप्रत्यक्ष eval का उपयोग न करें। सभी
  इनपुट को Zod से सत्यापित करें। संग्रहीत क्रेडेंशियल एन्क्रिप्ट रखें (AES-256-GCM)।
  `src/shared/constants/upstreamHeaders.ts` की निषेध-सूची को
  सैनिटाइज़ेशन/सत्यापन परत के अनुरूप रखें।
- **कमिट**: Conventional Commits — `feat(scope): subject`। अनुमत स्कोप:
  `db`, `sse`, `oauth`, `dashboard`, `api`, `cli`, `docker`, `ci`, `mcp`,
  `a2a`, `memory`, `skills`।
- **ब्रांच**: प्रीफ़िक्स `feat/`, `fix/`, `refactor/`, `docs/`, `test/`,
  `chore/`। कभी भी सीधे `main` में कमिट न करें।
- **Husky**: प्री-कमिट में `lint-staged` + `check:docs-sync` +
  `check:any-budget:t11` चलते हैं; प्री-पुश में `check:any-budget:t11` + `check:tracked-artifacts` चलते हैं (त्वरित गेट; `test:unit` को शामिल नहीं करते)।

---

## 12. कठोर नियम (CLAUDE.md से)

1. सीक्रेट्स या क्रेडेंशियल्स कभी कमिट न करें।
2. कभी भी बैरल-इंपोर्ट न करें — सीधे विशिष्ट `src/lib/db/*` मॉड्यूल का उपयोग करें।
3. कभी भी `eval()` / `new Function()` / अप्रत्यक्ष eval का उपयोग न करें।
4. कभी भी सीधे `main` में कमिट न करें।
5. रूट्स में कभी भी रॉ SQL न लिखें — हमेशा `src/lib/db/` मॉड्यूल के माध्यम से जाएँ।
6. SSE स्ट्रीम्स में त्रुटियों को कभी भी चुपचाप अनदेखा न करें।
7. इनपुट्स को हमेशा Zod स्कीमा के साथ वैलिडेट करें।
8. प्रोडक्शन कोड बदलते समय हमेशा टेस्ट शामिल करें।
9. कवरेज ≥ 60% (स्टेटमेंट्स, लाइन्स, फ़ंक्शंस, ब्रांचेज़) रहना चाहिए।

---

## 13. यह भी देखें

- [ARCHITECTURE.md](./ARCHITECTURE.md) — उच्च-स्तरीय आर्किटेक्चर और मॉड्यूल की
  ज़िम्मेदारियाँ।
- [API_REFERENCE.md](../reference/API_REFERENCE.md) — सार्वजनिक + प्रबंधन API संदर्भ।
- [FEATURES.md](../guides/FEATURES.md) — फ़ीचर मैट्रिक्स और वर्ज़न की प्रमुख विशेषताएँ।
- [RESILIENCE_GUIDE.md](./RESILIENCE_GUIDE.md) — सर्किट ब्रेकर, कूलडाउन और
  लॉकआउट का गहन विवरण।
- [AUTO-COMBO.md](../routing/AUTO-COMBO.md) — Auto Combo स्कोरिंग और रणनीतियाँ।
- [MCP-SERVER.md](../frameworks/MCP-SERVER.md) — संपूर्ण MCP टूल कैटलॉग + ट्रांसपोर्ट्स।
...
- [CLAUDE.md](../../CLAUDE.md) — Claude Code के लिए रेपो नियम (ऊपर दिए गए कई
  कन्वेंशंस का प्रामाणिक स्रोत)।
- [AGENTS.md](../../AGENTS.md) — एजेंट्स द्वारा उपयोग किया जाने वाला अधिक विस्तृत आर्किटेक्चर संदर्भ।
