...
Guia completo para configurar provedores, criar combos, integrar ferramentas de CLI e implantar o OmniRoute.

---

## Sumário

- [Visão Geral dos Preços](#-pricing-at-a-glance)
- [Casos de Uso](#-use-cases)
- [Configuração de Provedores](#-provider-setup)
- [Integração com CLI](#-cli-integration)
- [Implantação](#-deployment)
- [Modelos Disponíveis](#-available-models)
- [Recursos Avançados](#-advanced-features)
- [Roteamento Automático (Configuração Zero)](#-auto-routing-zero-config)
- [Integração com MCP e A2A](#-mcp--a2a-integration)
- [Sistema de Skills](#-skills-system)
- [Sistema de Memória](#-memory-system)
- [Webhooks](#-webhooks)
- [Agentes na Nuvem](#-cloud-agents)
- [Gerenciamento Programático](#-programmatic-management)
- [CLI Interna](#-internal-cli)
- [Aplicativo para Desktop (Electron)](#-desktop-application-electron)
...
### Caso 1: "Tenho uma assinatura do Claude Pro"
...
Para adicionar em massa conexões de chaves de API a partir de um arquivo CSV ou JSON, use **Painel → Provedores → Importar de arquivo**. As colunas são posicionais (`provider,name,apiKey,baseUrl,priority`); `provider` já deve existir como um provedor gerenciado ou um nó compatível. Consulte [Importar provedores de um arquivo CSV ou JSON](../providers/CSV-IMPORT.md).
...
1. Cadastre-se: [Baidu AI Cloud Qianfan](https://cloud.baidu.com/product/wenxinworkshop)
...
### Exemplo 2: Somente gratuitos (Custo zero)
...
**Usando o OmniRoute como provedor do Cursor** (o OmniRoute chama o Cursor como upstream): prefira
...
### Implantação com PM2 (Pouca memória)
...
#### Aliases de modelo com curingas
...
O roteador automático está totalmente descrito em [AUTO-COMBO.md](../routing/AUTO-COMBO.md) — incluindo como ajustar os pesos de pontuação, adicionar provedores à lista de bloqueio e inspecionar as decisões de roteamento em **Painel → Auto Combo**.
...
### Conectar o Cursor / Continue / MCP do VS Code

Use a URL SSE `http://localhost:20128/api/mcp/sse` e uma chave de API Bearer gerada em **Painel → Chaves de API**.

### Escopos

Atualmente, o MCP define 32 escopos nomeados. Cada chave Bearer pode ser limitada a escopos específicos — consulte [MCP-SERVER.md](../frameworks/MCP-SERVER.md) para obter o inventário oficial de escopos e ferramentas e [A2A-SERVER.md](../frameworks/A2A-SERVER.md) para o esquema JSON-RPC.
...
Gerencie as memórias em **Dashboard → Memory** (pesquisar, editar, exportar, limpar). A interface HTTP (`/api/memory/*`) permite que agentes enviem e consultem fatos de forma programática — consulte [MEMORY.md](../frameworks/MEMORY.md).
...
Consulte [API_REFERENCE.md](../reference/API_REFERENCE.md) para ver o catálogo completo de endpoints e os esquemas de solicitação/resposta.
