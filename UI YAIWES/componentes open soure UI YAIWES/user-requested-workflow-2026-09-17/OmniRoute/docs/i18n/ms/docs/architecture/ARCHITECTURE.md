...
```mermaid
flowchart LR
    subgraph Clients[Klien Pembangun]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Klien tersuai yang serasi dengan OpenAI]
        BROWSER[Papan Pemuka Pelayar]
    end

    subgraph Router[Proses Setempat OmniRoute]
        API[API Keserasian V1\n/v1/*]
        DASH[Papan Pemuka + API Pengurusan\n/api/*]
        CORE[Teras SSE + Terjemahan\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(jadual penggunaan + artifak log)]
    end

    subgraph Upstreams[Penyedia Huluan]
        P1[Penyedia OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Penyedia Kunci API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Nod Serasi\nSerasi dengan OpenAI / Serasi dengan Anthropic]
    end

    subgraph Cloud[Penyegerakan Awan Pilihan]
        CLOUD[Titik Akhir Penyegerakan Awan\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    ...
    participant Auth as Pemilih Kelayakan
    participant Exec as Pelaksana Penyedia
    participant Prov as Penyedia Huluan
    participant Stream as Penterjemah Strim
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: huraikan/selesaikan model atau kombo

    alt Model kombo
        Chat->>Chat: ulang proses model kombo (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: akaun aktif + token/kunci API

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: kesan format sumber
    Core->>Core: terjemahkan permintaan kepada format sasaran
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: panggilan API huluan
    Prov-->>Exec: respons SSE/JSON
    Exec-->>Core: respons + metadata

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: token yang dikemas kini
        Core->>Exec: cuba semula permintaan
    end

    Core->>Stream: terjemahkan/normalkan strim kepada format klien
    Stream-->>Client: cebisan SSE / respons JSON

    Stream->>Usage: ekstrak penggunaan + simpan sejarah/log
```
...
```mermaid
flowchart TD
    A[Rentetan model masuk] --> B{Adakah nama gabungan?}
    B -- Ya --> C[Muatkan jujukan model gabungan]
    B -- Tidak --> D[Laluan model tunggal]

    C --> E[Cuba model N]
    E --> F[Selesaikan penyedia/model]
    D --> F

    F --> G[Pilih kelayakan akaun]
    G --> H{Kelayakan tersedia?}
    H -- Tidak --> I[Kembalikan penyedia tidak tersedia]
    H -- Ya --> J[Laksanakan permintaan]

    J --> K{Berjaya?}
    K -- Ya --> L[Kembalikan respons]
    K -- Tidak --> M{Ralat layak sandaran?}

    M -- Tidak --> N[Kembalikan ralat]
    M -- Ya --> O[Tandakan akaun tidak tersedia untuk tempoh bertenang]
    O --> P{Akaun lain untuk penyedia?}
    P -- Ya --> G
    P -- Tidak --> Q{Dalam gabungan dengan model seterusnya?}
    Q -- Ya --> E
...
sequenceDiagram
    autonumber
    participant UI as UI Papan Pemuka
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Pelayan Pengesahan Penyedia
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Pelaksana Penyedia

    UI->>OAuth: GET kebenaran atau kod peranti
    OAuth->>ProvAuth: cipta aliran pengesahan/peranti
    ProvAuth-->>OAuth: URL pengesahan atau muatan kod peranti
    OAuth-->>UI: data aliran

    UI->>OAuth: POST pertukaran atau tinjauan
    OAuth->>ProvAuth: pertukaran/tinjauan token
    ProvAuth-->>OAuth: token akses/penyegaran
    OAuth->>DB: createProviderConnection(data oauth)
    OAuth-->>UI: berjaya + id sambungan

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: sahkan kelayakan / penyegaran pilihan
    Exec-->>Test: maklumat token yang sah atau disegarkan
    Test->>DB: kemas kini status/token/ralat
    Test-->>UI: hasil pengesahan
```
...
sequenceDiagram
    autonumber
    participant UI as UI Halaman Titik Akhir
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Penyegerakan Awan Luaran
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: tetapkan cloudEnabled=true
    Sync->>DB: pastikan kunci API wujud
    Sync->>Cloud: POST /sync/{machineId} (penyedia/alias/gabungan/kunci)
    Cloud-->>Sync: hasil penyegerakan
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: didayakan + status pengesahan

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: data jauh
    Sync->>DB: kemas kini token/status tempatan yang lebih baharu
    Sync-->>UI: disegerakkan

    UI->>Sync: POST action=disable
    Sync->>DB: tetapkan cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: tukar ANTHROPIC_BASE_URL kembali kepada setempat (jika perlu)
    Sync-->>UI: dinyahdayakan
```
...
```mermaid
flowchart LR
    subgraph LocalHost[Hos Pembangun]
        CLI[Alat CLI]
        Browser[Pelayar Papan Pemuka]
    end

    subgraph ContainerOrProcess[Masa Jalan OmniRoute]
        Next[Pelayan Next.js\nPORT=20128]
        Core[Teras SSE + Pelaksana]
        MainDB[(storage.sqlite)]
        UsageDB[(jadual penggunaan + artifak log)]
    end

    subgraph External[Perkhidmatan Luaran]
        Providers[Penyedia AI]
        SyncCloud[Perkhidmatan Penyegerakan Awan]
    end
