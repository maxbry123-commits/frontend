...
![Mtiririko wa ombi (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Wateja wa Wasanidi Programu]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Wateja maalum wanaooana na OpenAI]
        BROWSER[Dashibodi ya Kivinjari]
    end

    subgraph Router[Mchakato wa Ndani wa OmniRoute]
        API[API ya Uoanifu ya V1\n/v1/*]
        DASH[Dashibodi + API ya Usimamizi\n/api/*]
        CORE[SSE + Kiini cha Utafsiri\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(majedwali ya matumizi + vizalia vya kumbukumbu)]
    end

    subgraph Upstreams[Watoa Huduma wa Juu]
        P1[Watoa Huduma wa OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Watoa Huduma wa Ufunguo wa API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Nodi Zinazooana\nZinazooana na OpenAI / zinazooana na Anthropic]
    end

    subgraph Cloud[Usawazishaji wa Hiari wa Wingu]
        CLOUD[Endpointi ya Usawazishaji wa Wingu\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as Kiteja cha CLI/SDK
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as Kisuluhishi cha Modeli
    participant Auth as Kiteuzi cha Kitambulisho
    participant Exec as Kitekelezaji cha Mtoa Huduma
    participant Prov as Mtoa Huduma wa Juu
    participant Stream as Kitafsiri cha Mtiririko
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: changanua/suluhisha modeli au mseto

    alt Modeli mseto
        Chat->>Chat: pitia modeli za mseto (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: akaunti inayotumika + tokeni/ufunguo wa api

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: tambua umbizo la chanzo
    Core->>Core: tafsiri ombi hadi umbizo lengwa
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: mwito wa API wa juu
    Prov-->>Exec: jibu la SSE/JSON
    Exec-->>Core: jibu + metadata

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: tokeni zilizosasishwa
        Core->>Exec: jaribu ombi tena
    end

    Core->>Stream: tafsiri/sawazisha mtiririko hadi umbizo la kiteja
    Stream-->>Client: vipande vya SSE / jibu la JSON

    Stream->>Usage: toa matumizi + hifadhi historia/kumbukumbu
```
...
```mermaid
flowchart TD
    A[Mfuatano wa modeli unaoingia] --> B{Ni jina la combo?}
    B -- Ndiyo --> C[Pakia mfuatano wa modeli za combo]
    B -- Hapana --> D[Njia ya modeli moja]

    C --> E[Jaribu modeli N]
    E --> F[Tambua mtoa huduma/modeli]
    D --> F

    F --> G[Chagua vitambulisho vya akaunti]
    G --> H{Vitambulisho vinapatikana?}
    H -- Hapana --> I[Rejesha kuwa mtoa huduma hapatikani]
    H -- Ndiyo --> J[Tekeleza ombi]

    J --> K{Imefaulu?}
    K -- Ndiyo --> L[Rejesha jibu]
    K -- Hapana --> M{Hitilafu inaruhusu urejeaji?}

    M -- Hapana --> N[Rejesha hitilafu]
    M -- Ndiyo --> O[Weka akaunti kuwa haipatikani kwa kipindi cha kusubiri]
    O --> P{Kuna akaunti nyingine ya mtoa huduma?}
    P -- Ndiyo --> G
    P -- Hapana --> Q{Iko kwenye combo yenye modeli inayofuata?}
    Q -- Ndiyo --> E
    Q -- Hapana --> R[Rejesha kuwa zote hazipatikani]
```
...
sequenceDiagram
    autonumber
    participant UI as Kiolesura cha Dashibodi
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Seva ya Uthibitishaji ya Mtoa Huduma
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Kitekelezaji cha Mtoa Huduma

    UI->>OAuth: GET idhini au msimbo wa kifaa
    OAuth->>ProvAuth: unda mtiririko wa uthibitishaji/kifaa
    ProvAuth-->>OAuth: URL ya uthibitishaji au data ya msimbo wa kifaa
    OAuth-->>UI: data ya mtiririko

    UI->>OAuth: POST ubadilishaji au upigaji kura
    OAuth->>ProvAuth: ubadilishaji/upigaji kura wa tokeni
    ProvAuth-->>OAuth: tokeni za ufikiaji/kuonyesha upya
    OAuth->>DB: createProviderConnection(data ya oauth)
    OAuth-->>UI: mafanikio + kitambulisho cha muunganisho

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: thibitisha vitambulisho / uonyeshaji upya wa hiari
    Exec-->>Test: maelezo ya tokeni halali au iliyoonyeshwa upya
    Test->>DB: sasisha hali/tokeni/hitilafu
    Test-->>UI: matokeo ya uthibitishaji
```
...
sequenceDiagram
    autonumber
    participant UI as Kiolesura cha Ukurasa wa Endpoint
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Usawazishaji wa Wingu la Nje
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: weka cloudEnabled=true
    Sync->>DB: hakikisha ufunguo wa API upo
    Sync->>Cloud: POST /sync/{machineId} (watoa huduma/majina mbadala/combo/funguo)
    Cloud-->>Sync: matokeo ya usawazishaji
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: imewashwa + hali ya uthibitishaji

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: data ya mbali
    Sync->>DB: sasisha tokeni/hali mpya zaidi za ndani
    Sync-->>UI: imesawazishwa

    UI->>Sync: POST action=disable
    Sync->>DB: weka cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: rudisha ANTHROPIC_BASE_URL kuwa ya ndani (ikihitajika)
    Sync-->>UI: imezimwa
```
