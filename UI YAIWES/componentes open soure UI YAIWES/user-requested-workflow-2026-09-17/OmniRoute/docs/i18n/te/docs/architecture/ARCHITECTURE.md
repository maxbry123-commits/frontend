...
![3-పొరల స్థితిస్థాపకత నమూనా](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[డెవలపర్ క్లయింట్లు]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[కస్టమ్ OpenAI-అనుకూల క్లయింట్లు]
        BROWSER[బ్రౌజర్ డాష్బోర్డ్]
    end

    subgraph Router[OmniRoute స్థానిక ప్రాసెస్]
        API[V1 అనుకూలత API\n/v1/*]
        DASH[డాష్బోర్డ్ + నిర్వహణ API\n/api/*]
        CORE[SSE + అనువాద కోర్\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(వినియోగ పట్టికలు + లాగ్ ఆర్టిఫాక్ట్లు)]
    end

    subgraph Upstreams[అప్స్ట్రీమ్ ప్రొవైడర్లు]
        P1[OAuth ప్రొవైడర్లు\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API కీ ప్రొవైడర్లు\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[అనుకూల నోడ్లు\nOpenAI-అనుకూల / Anthropic-అనుకూల]
    end

    subgraph Cloud[ఐచ్ఛిక క్లౌడ్ సింక్]
        CLOUD[క్లౌడ్ సింక్ ఎండ్పాయింట్\nNEXT_PUBLIC_CLOUD_URL]
    end
...
```mermaid
flowchart TD
    A[ఇన్కమింగ్ మోడల్ స్ట్రింగ్] --> B{కాంబో పేరా?}
    B -- అవును --> C[కాంబో మోడళ్ల క్రమాన్ని లోడ్ చేయి]
    B -- కాదు --> D[ఒకే మోడల్ మార్గం]

    C --> E[మోడల్ Nని ప్రయత్నించు]
    E --> F[ప్రొవైడర్/మోడల్ను పరిష్కరించు]
    D --> F

    F --> G[ఖాతా క్రెడెన్షియల్స్ను ఎంచుకో]
    G --> H{క్రెడెన్షియల్స్ అందుబాటులో ఉన్నాయా?}
    H -- కాదు --> I[ప్రొవైడర్ అందుబాటులో లేదని చూపించు]
    H -- అవును --> J[అభ్యర్థనను అమలు చేయి]

    J --> K{విజయవంతమైందా?}
    K -- అవును --> L[ప్రతిస్పందనను అందించు]
    K -- కాదు --> M{ఫాల్బ్యాక్కు అర్హమైన లోపమా?}

    M -- కాదు --> N[లోపాన్ని అందించు]
    M -- అవును --> O[ఖాతాను కూల్డౌన్ కోసం అందుబాటులో లేనిదిగా గుర్తించు]
    O --> P{ప్రొవైడర్కు మరో ఖాతా ఉందా?}
    P -- అవును --> G
    P -- కాదు --> Q{తదుపరి మోడల్తో కాంబోలో ఉందా?}
    Q -- అవును --> E
    Q -- కాదు --> R[అన్నీ అందుబాటులో లేవని చూపించు]
```
...
sequenceDiagram
    autonumber
    participant UI as డ్యాష్బోర్డ్ UI
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as ప్రొవైడర్ ప్రమాణీకరణ సర్వర్
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as ప్రొవైడర్ ఎగ్జిక్యూటర్

    UI->>OAuth: GET authorize లేదా device-code
    OAuth->>ProvAuth: ప్రమాణీకరణ/పరికర ప్రవాహాన్ని సృష్టించు
    ProvAuth-->>OAuth: ప్రమాణీకరణ URL లేదా పరికర కోడ్ పేలోడ్
    OAuth-->>UI: ప్రవాహ డేటా

    UI->>OAuth: POST exchange లేదా poll
    OAuth->>ProvAuth: టోకెన్ మార్పిడి/పోలింగ్
    ProvAuth-->>OAuth: యాక్సెస్/రిఫ్రెష్ టోకెన్లు
    OAuth->>DB: createProviderConnection(oauth డేటా)
    OAuth-->>UI: విజయం + కనెక్షన్ id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: క్రెడెన్షియల్స్ను ధ్రువీకరించు / అవసరమైతే రిఫ్రెష్ చేయి
    Exec-->>Test: చెల్లుబాటు స్థితి లేదా రిఫ్రెష్ చేసిన టోకెన్ సమాచారం
    Test->>DB: స్థితి/టోకెన్లు/లోపాలను నవీకరించు
    Test-->>UI: ధ్రువీకరణ ఫలితం
```
...
sequenceDiagram
    autonumber
    participant UI as ఎండ్పాయింట్ పేజీ UI
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as బాహ్య క్లౌడ్ సింక్
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=trueగా సెట్ చేయి
    Sync->>DB: API కీ ఉందని నిర్ధారించు
    Sync->>Cloud: POST /sync/{machineId} (ప్రొవైడర్లు/అలియాస్లు/కాంబోలు/కీలు)
    Cloud-->>Sync: సింక్ ఫలితం
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: ప్రారంభించబడింది + ధ్రువీకరణ స్థితి

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: రిమోట్ డేటా
    Sync->>DB: కొత్త లోకల్ టోకెన్లు/స్థితిని నవీకరించు
    Sync-->>UI: సింక్ చేయబడింది

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=falseగా సెట్ చేయి
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URLను తిరిగి లోకల్కు మార్చు (అవసరమైతే)
    Sync-->>UI: నిలిపివేయబడింది
```
...
```mermaid
flowchart LR
    subgraph LocalHost[డెవలపర్ హోస్ట్]
        CLI[CLI సాధనాలు]
        Browser[డ్యాష్బోర్డ్ బ్రౌజర్]
    end

    subgraph ContainerOrProcess[OmniRoute రన్టైమ్]
        Next[Next.js సర్వర్\nPORT=20128]
        Core[SSE కోర్ + ఎగ్జిక్యూటర్లు]
        MainDB[(storage.sqlite)]
        UsageDB[(వినియోగ పట్టికలు + లాగ్ ఆర్టిఫాక్ట్లు)]
    end

    subgraph External[బాహ్య సేవలు]
        Providers[AI ప్రొవైడర్లు]
        SyncCloud[క్లౌడ్ సింక్ సేవ]
    end
