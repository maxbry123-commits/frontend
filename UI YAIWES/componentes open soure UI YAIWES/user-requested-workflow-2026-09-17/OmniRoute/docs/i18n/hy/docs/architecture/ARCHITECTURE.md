...
![Հարցման մշակման շղթա (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)

> Սկզբնաղբյուր՝ [diagrams/request-pipeline.mmd](../diagrams/request-pipeline.mmd)

![Դիմակայունության եռաշերտ մոդել](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Մշակողների հաճախորդներ]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[OpenAI-ի հետ համատեղելի հատուկ հաճախորդներ]
        BROWSER[Բրաուզերի կառավարման վահանակ]
    end

    subgraph Router[OmniRoute-ի տեղային գործընթաց]
        API[V1 համատեղելիության API\n/v1/*]
        DASH[Կառավարման վահանակ + կառավարման API\n/api/*]
        CORE[SSE + փոխակերպման միջուկ\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(օգտագործման աղյուսակներ + մատյանների արտեֆակտներ)]
    end

    subgraph Upstreams[Վերին մակարդակի մատակարարներ]
        P1[OAuth մատակարարներ\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API բանալիով մատակարարներ\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Համատեղելի հանգույցներ\nOpenAI-ի հետ համատեղելի / Anthropic-ի հետ համատեղելի]
    end

    subgraph Cloud[Ընտրովի ամպային համաժամացում]
        CLOUD[Ամպային համաժամացման վերջնակետ\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as CLI/SDK հաճախորդ
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as Մոդելի որոշիչ
    participant Auth as Հավաստագրերի ընտրիչ
    participant Exec as Մատակարարի կատարիչ
    participant Prov as Վերին մակարդակի մատակարար
    participant Stream as Հոսքի փոխակերպիչ
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: վերլուծել/որոշել մոդելը կամ համակցությունը

    alt Համակցված մոդել
        Chat->>Chat: հերթով մշակել համակցված մոդելները (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: ակտիվ հաշիվ + թոքեններ/api բանալի

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: հայտնաբերել սկզբնաղբյուրի ձևաչափը
    Core->>Core: փոխակերպել հարցումը նպատակային ձևաչափի
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: կանչ դեպի վերին մակարդակի API
    Prov-->>Exec: SSE/JSON պատասխան
    Exec-->>Core: պատասխան + մետատվյալներ

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: թարմացված թոքեններ
        Core->>Exec: կրկին կատարել հարցումը
    end

    Core->>Stream: փոխակերպել/նորմալացնել հոսքը հաճախորդի ձևաչափի
    Stream-->>Client: SSE հատվածներ / JSON պատասխան

    Stream->>Usage: հանել օգտագործման տվյալները + պահպանել պատմությունը/մատյանը
```
...
```mermaid
flowchart TD
    A[Մուտքային մոդելի տող] --> B{Կոմբոյի անո՞ւն է}
    B -- Այո --> C[Բեռնել կոմբոյի մոդելների հաջորդականությունը]
    B -- Ոչ --> D[Մեկ մոդելի ուղի]

    C --> E[Փորձել N մոդելը]
    E --> F[Որոշել մատակարարին/մոդելը]
    D --> F

    F --> G[Ընտրել հաշվի հավատարմագրերը]
    G --> H{Հավատարմագրերը հասանելի՞ են}
    H -- Ոչ --> I[Վերադարձնել՝ մատակարարն անհասանելի է]
    H -- Այո --> J[Կատարել հարցումը]

    J --> K{Հաջողվե՞ց}
    K -- Այո --> L[Վերադարձնել պատասխանը]
    K -- Ոչ --> M{Սխալը թույլ տալի՞ս է պահուստային անցում}

    M -- Ոչ --> N[Վերադարձնել սխալը]
    M -- Այո --> O[Հաշիվը նշել որպես ժամանակավորապես անհասանելի]
    O --> P{Մատակարարի համար կա՞ այլ հաշիվ}
    P -- Այո --> G
    P -- Ոչ --> Q{Կոմբոյում կա՞ հաջորդ մոդել}
    Q -- Այո --> E
    Q -- Ոչ --> R[Վերադարձնել՝ բոլորը հասանելի չեն]
```
...
sequenceDiagram
    autonumber
    participant UI as Կառավարման վահանակի UI
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Մատակարարի նույնականացման սերվեր
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Մատակարարի կատարիչ

    UI->>OAuth: GET authorize կամ device-code
    OAuth->>ProvAuth: ստեղծել նույնականացման/սարքի հոսք
    ProvAuth-->>OAuth: նույնականացման URL կամ սարքի կոդի տվյալներ
    OAuth-->>UI: հոսքի տվյալներ

    UI->>OAuth: POST exchange կամ poll
    OAuth->>ProvAuth: տոկենի փոխանակում/հարցում
    ProvAuth-->>OAuth: մուտքի/թարմացման տոկեններ
    OAuth->>DB: createProviderConnection(oauth տվյալներ)
    OAuth-->>UI: հաջողություն + կապի id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: վավերացնել հավատարմագրերը / անհրաժեշտության դեպքում թարմացնել
    Exec-->>Test: վավեր կամ թարմացված տոկենի տվյալներ
    Test->>DB: թարմացնել կարգավիճակը/տոկենները/սխալները
    Test-->>UI: վավերացման արդյունք
```
...
sequenceDiagram
    autonumber
    participant UI as Վերջնակետի էջի UI
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Արտաքին ամպային համաժամացում
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: սահմանել cloudEnabled=true
    Sync->>DB: համոզվել, որ API բանալին գոյություն ունի
    Sync->>Cloud: POST /sync/{machineId} (մատակարարներ/այլանուններ/կոմբոներ/բանալիներ)
    Cloud-->>Sync: համաժամացման արդյունք
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: միացված է + ստուգման կարգավիճակ

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: հեռակա տվյալներ
    Sync->>DB: թարմացնել ավելի նոր տեղային տոկենները/կարգավիճակը
    Sync-->>UI: համաժամացված է

    UI->>Sync: POST action=disable
    Sync->>DB: սահմանել cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: վերադարձնել ANTHROPIC_BASE_URL-ը տեղային հասցեին (անհրաժեշտության դեպքում)
    Sync-->>UI: անջատված է
```
...
```mermaid
flowchart LR
    subgraph LocalHost[Մշակողի հոսթ]
        CLI[CLI գործիքներ]
        Browser[Կառավարման վահանակի դիտարկիչ]
    end

    subgraph ContainerOrProcess[OmniRoute գործարկման միջավայր]
        Next[Next.js սերվեր\nPORT=20128]
        Core[SSE միջուկ + կատարիչներ]
        MainDB[(storage.sqlite)]
        UsageDB[(օգտագործման աղյուսակներ + մատյանի արտեֆակտներ)]
    end

    subgraph External[Արտաքին ծառայություններ]
        Providers[AI մատակարարներ]
        SyncCloud[Ամպային համաժամացման ծառայություն]
    end
