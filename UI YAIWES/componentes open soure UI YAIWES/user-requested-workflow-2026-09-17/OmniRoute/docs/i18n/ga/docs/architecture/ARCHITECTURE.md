...
```mermaid
flowchart LR
    subgraph Clients[Cliaint Forbróra]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Cliaint shaincheaptha atá comhoiriúnach le OpenAI]
        BROWSER[Deais Brabhsálaí]
    end

    subgraph Router[Próiseas Logánta OmniRoute]
        API[API Comhoiriúnachta V1\n/v1/*]
        DASH[Deais + API Bainistíochta\n/api/*]
        CORE[Croílár SSE + Aistriúcháin\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(táblaí úsáide + déantáin loga)]
    end

    subgraph Upstreams[Soláthraithe Réamhtheachtacha]
        P1[Soláthraithe OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Soláthraithe Eochracha API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Nóid Chomhoiriúnacha\nComhoiriúnach le OpenAI / comhoiriúnach le Anthropic]
    end

    subgraph Cloud[Sioncronú Roghnach Néalríomhaireachta]
        CLOUD[Críochphointe Sioncronaithe Néalríomhaireachta\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as Cliant CLI/SDK
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as Réiteoir Samhla
    participant Auth as Roghnóir Dintiúr
    participant Exec as Feidhmitheoir Soláthraí
    participant Prov as Soláthraí Réamhtheachtach
    participant Stream as Aistritheoir Srutha
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: parsáil/réitigh samhail nó teaglaim

    alt Samhail teaglaim
        Chat->>Chat: atriall trí shamhlacha teaglaim (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: cuntas gníomhach + comharthaí/eochair API

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: braith formáid na foinse
    Core->>Core: aistrigh an t-iarratas go dtí an spriocfhormáid
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: glao API réamhtheachtach
    Prov-->>Exec: freagra SSE/JSON
    Exec-->>Core: freagra + meiteashonraí

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: comharthaí nuashonraithe
        Core->>Exec: atriail an t-iarratas
    end

    Core->>Stream: aistrigh/normalú an sruth go formáid an chliaint
    Stream-->>Client: smutáin SSE / freagra JSON

    Stream->>Usage: bain úsáid amach + marthanaigh stair/loga
```
...
```mermaid
flowchart TD
    A[Teaghrán samhla isteach] --> B{An ainm teaglama é?}
    B -- Is ea --> C[Luchtaigh seicheamh samhlacha an teaglama]
    B -- Ní hea --> D[Conair aon samhla]

    C --> E[Bain triail as samhail N]
    E --> F[Réitigh soláthraí/samhail]
    D --> F

    F --> G[Roghnaigh dintiúir chuntais]
    G --> H{Dintiúir ar fáil?}
    H -- Níl --> I[Cuir ar ais nach bhfuil an soláthraí ar fáil]
    H -- Tá --> J[Rith an t-iarratas]

    J --> K{D'éirigh leis?}
    K -- D'éirigh --> L[Cuir an freagra ar ais]
    K -- Níor éirigh --> M{Earráid atá incháilithe do chúltaca?}

    M -- Níl --> N[Cuir an earráid ar ais]
    M -- Tá --> O[Marcáil nach bhfuil an cuntas ar fáil le linn na tréimhse feithimh]
    O --> P{Cuntas eile don soláthraí?}
    P -- Tá --> G
    P -- Níl --> Q{I dteaglaim a bhfuil samhail eile ina diaidh?}
    Q -- Tá --> E
    Q -- Níl --> R[Cuir ar ais nach bhfuil ceann ar bith ar fáil]
```
...
sequenceDiagram
    autonumber
    participant UI as Comhéadan Úsáideora an Deais
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Freastalaí Fíordheimhnithe an tSoláthraí
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Seiceadóir an tSoláthraí

    UI->>OAuth: GET authorize nó device-code
    OAuth->>ProvAuth: cruthaigh sreabhadh fíordheimhnithe/gléis
    ProvAuth-->>OAuth: URL fíordheimhnithe nó pálasta cóid gléis
    OAuth-->>UI: sonraí an tsreafa

    UI->>OAuth: POST exchange nó poll
    OAuth->>ProvAuth: malartú/vótaíocht comhartha
    ProvAuth-->>OAuth: comharthaí rochtana/athnuachana
    OAuth->>DB: createProviderConnection(sonraí oauth)
    OAuth-->>UI: rath + aitheantas ceangail

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: bailíochtaigh dintiúir / athnuachan roghnach
    Exec-->>Test: faisnéis faoi chomhartha bailí nó athnuaite
...
sequenceDiagram
    autonumber
    participant UI as Comhéadan Úsáideora Leathanach an Chríochphointe
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Sioncronú Seachtrach Néalríomhaireachta
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: socraigh cloudEnabled=true
    Sync->>DB: cinntigh go bhfuil eochair API ann
    Sync->>Cloud: POST /sync/{machineId} (soláthraithe/ailiasanna/teaglamaí/eochracha)
    ...
    Sync->>DB: nuashonraigh comharthaí/stádas áitiúil níos nuaí
    Sync-->>UI: sioncronaithe

    UI->>Sync: POST action=disable
    Sync->>DB: socraigh cloudEnabled=false
...
```mermaid
flowchart LR
    subgraph LocalHost[Óstríomhaire an Fhorbróra]
        CLI[Uirlisí CLI]
        Browser[Brabhsálaí an Deais]
    end

    subgraph ContainerOrProcess[Timpeallacht Rite OmniRoute]
        Next[Freastalaí Next.js\nPORT=20128]
        Core[Croílár SSE + Forghníomhaithe]
        MainDB[(storage.sqlite)]
        UsageDB[(táblaí úsáide + déantáin loga)]
    end

    subgraph External[Seirbhísí Seachtracha]
        Providers[Soláthraithe IS]
        SyncCloud[Seirbhís Sioncronaithe Néalríomhaireachta]
    end
