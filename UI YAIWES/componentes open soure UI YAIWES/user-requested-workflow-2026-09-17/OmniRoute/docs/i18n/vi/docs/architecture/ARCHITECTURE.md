...
![Mô hình khả năng phục hồi 3 lớp](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Ứng dụng khách dành cho nhà phát triển]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Ứng dụng khách tùy chỉnh tương thích với OpenAI]
        BROWSER[Bảng điều khiển trên trình duyệt]
    end

    subgraph Router[Tiến trình cục bộ OmniRoute]
        API[API tương thích V1\n/v1/*]
        DASH[Bảng điều khiển + API quản lý\n/api/*]
        CORE[Lõi SSE + chuyển đổi\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(bảng dữ liệu sử dụng + thành phần nhật ký)]
    end

    subgraph Upstreams[Nhà cung cấp thượng nguồn]
        P1[Nhà cung cấp OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Nhà cung cấp khóa API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Nút tương thích\nTương thích với OpenAI / tương thích với Anthropic]
    end

    subgraph Cloud[Đồng bộ đám mây tùy chọn]
        CLOUD[Điểm cuối đồng bộ đám mây\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as Máy khách CLI/SDK
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as Bộ phân giải mô hình
    participant Auth as Bộ chọn thông tin xác thực
    participant Exec as Bộ thực thi nhà cung cấp
    participant Prov as Nhà cung cấp thượng nguồn
    participant Stream as Bộ chuyển đổi luồng
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: phân tích/phân giải mô hình hoặc tổ hợp

    alt Mô hình tổ hợp
        Chat->>Chat: lặp qua các mô hình trong tổ hợp (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: tài khoản đang hoạt động + token/khóa API

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: phát hiện định dạng nguồn
    Core->>Core: chuyển đổi yêu cầu sang định dạng đích
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: gọi API thượng nguồn
    Prov-->>Exec: phản hồi SSE/JSON
    Exec-->>Core: phản hồi + siêu dữ liệu

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: token đã cập nhật
        Core->>Exec: thử lại yêu cầu
    end

    Core->>Stream: chuyển đổi/chuẩn hóa luồng sang định dạng máy khách
    Stream-->>Client: các đoạn SSE / phản hồi JSON

    Stream->>Usage: trích xuất dữ liệu sử dụng + lưu trữ lịch sử/nhật ký
```
...
```mermaid
flowchart TD
    A[Chuỗi model đầu vào] --> B{Là tên combo?}
    B -- Có --> C[Tải chuỗi model của combo]
    B -- Không --> D[Luồng model đơn]

    C --> E[Thử model N]
    E --> F[Phân giải provider/model]
    D --> F

    F --> G[Chọn thông tin xác thực tài khoản]
    G --> H{Có thông tin xác thực?}
    H -- Không --> I[Trả về provider không khả dụng]
    H -- Có --> J[Thực thi yêu cầu]

    J --> K{Thành công?}
    K -- Có --> L[Trả về phản hồi]
    K -- Không --> M{Lỗi đủ điều kiện dự phòng?}

    M -- Không --> N[Trả về lỗi]
    M -- Có --> O[Đánh dấu tài khoản không khả dụng trong thời gian chờ]
    O --> P{Còn tài khoản khác cho provider?}
    P -- Có --> G
    P -- Không --> Q{Đang trong combo và còn model tiếp theo?}
    Q -- Có --> E
    Q -- Không --> R[Trả về tất cả đều không khả dụng]
```
...
sequenceDiagram
    autonumber
    participant UI as Giao diện bảng điều khiển
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Máy chủ xác thực của provider
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Bộ thực thi provider

    UI->>OAuth: GET authorize hoặc device-code
    OAuth->>ProvAuth: tạo luồng xác thực/thiết bị
    ProvAuth-->>OAuth: URL xác thực hoặc payload mã thiết bị
    OAuth-->>UI: dữ liệu luồng

    UI->>OAuth: POST exchange hoặc poll
    OAuth->>ProvAuth: trao đổi/thăm dò token
    ProvAuth-->>OAuth: access/refresh token
    OAuth->>DB: createProviderConnection(dữ liệu oauth)
    OAuth-->>UI: thành công + ID kết nối

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: xác thực thông tin đăng nhập / tùy chọn làm mới
    Exec-->>Test: thông tin token hợp lệ hoặc đã làm mới
    Test->>DB: cập nhật trạng thái/token/lỗi
    Test-->>UI: kết quả xác thực
```
...
sequenceDiagram
    autonumber
    participant UI as Giao diện trang Endpoint
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Dịch vụ đồng bộ đám mây bên ngoài
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: đặt cloudEnabled=true
    Sync->>DB: đảm bảo API key tồn tại
    Sync->>Cloud: POST /sync/{machineId} (provider/alias/combo/key)
    Cloud-->>Sync: kết quả đồng bộ
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: đã bật + trạng thái xác minh

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: dữ liệu từ xa
    Sync->>DB: cập nhật token/trạng thái cục bộ mới hơn
    Sync-->>UI: đã đồng bộ

    UI->>Sync: POST action=disable
    Sync->>DB: đặt cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: chuyển ANTHROPIC_BASE_URL trở lại cục bộ (nếu cần)
    Sync-->>UI: đã tắt
```
...
```mermaid
erDiagram
    SETTINGS ||--o{ PROVIDER_CONNECTION : kiểm_soát
    PROVIDER_NODE ||--o{ PROVIDER_CONNECTION : hỗ_trợ_nhà_cung_cấp_tương_thích
    PROVIDER_CONNECTION ||--o{ USAGE_ENTRY : phát_sinh_mức_sử_dụng
...
```mermaid
flowchart LR
    subgraph LocalHost[Máy Phát triển]
        CLI[Công cụ CLI]
        Browser[Trình duyệt Bảng điều khiển]
    end

    subgraph ContainerOrProcess[Thời gian chạy OmniRoute]
        Next[Máy chủ Next.js\nPORT=20128]
        Core[Lõi SSE + Bộ thực thi]
        MainDB[(storage.sqlite)]
        UsageDB[(bảng mức sử dụng + thành phần nhật ký)]
    end

    subgraph External[Dịch vụ Bên ngoài]
        Providers[Nhà cung cấp AI]
        SyncCloud[Dịch vụ Đồng bộ Đám mây]
    end
