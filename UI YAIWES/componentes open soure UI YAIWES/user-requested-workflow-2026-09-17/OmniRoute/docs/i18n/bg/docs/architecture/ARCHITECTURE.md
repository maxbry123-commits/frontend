...
![3-слоен модел за устойчивост](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Клиенти за разработчици]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Персонализирани клиенти, съвместими с OpenAI]
        BROWSER[Табло за управление в браузъра]
    end

    subgraph Router[Локален процес OmniRoute]
        API[API за съвместимост V1\n/v1/*]
        DASH[Табло за управление + API за управление\n/api/*]
        CORE[Ядро за SSE + преобразуване\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(таблици за използването + артефакти от регистрационните файлове)]
    end

    subgraph Upstreams[Външни доставчици]
        P1[Доставчици с OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Доставчици с API ключове\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Съвместими възли\nСъвместими с OpenAI / съвместими с Anthropic]
    end

    subgraph Cloud[Незадължителна облачна синхронизация]
        CLOUD[Крайна точка за облачна синхронизация\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as CLI/SDK клиент
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as Разрешаване на модели
    participant Auth as Избор на идентификационни данни
    participant Exec as Изпълнител на доставчика
    participant Prov as Външен доставчик
    participant Stream as Преобразувател на поток
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: анализиране/разрешаване на модел или комбинация

    alt Комбиниран модел
        Chat->>Chat: обхождане на комбинираните модели (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: активен акаунт + токени/API ключ

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: откриване на изходния формат
    Core->>Core: преобразуване на заявката в целевия формат
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: заявка към външен API
    Prov-->>Exec: SSE/JSON отговор
    Exec-->>Core: отговор + метаданни

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: актуализирани токени
        Core->>Exec: повторен опит за изпълнение на заявката
    end

    Core->>Stream: преобразуване/нормализиране на потока към клиентския формат
    Stream-->>Client: SSE фрагменти / JSON отговор

    Stream->>Usage: извличане на данните за използването + съхраняване на хронологията/регистрационния файл
```
...
```mermaid
flowchart TD
    A[Входящ низ на модел] --> B{Име на комбо ли е?}
    B -- Да --> C[Зареждане на последователността от модели в комбото]
    B -- Не --> D[Път за единичен модел]

    C --> E[Опит с модел N]
    E --> F[Определяне на доставчик/модел]
    D --> F

    F --> G[Избор на идентификационни данни за акаунт]
    G --> H{Налични ли са идентификационни данни?}
    H -- Не --> I[Връщане на недостъпен доставчик]
    H -- Да --> J[Изпълнение на заявката]

    J --> K{Успешно ли е?}
    K -- Да --> L[Връщане на отговора]
    K -- Не --> M{Грешката позволява ли резервен вариант?}

    M -- Не --> N[Връщане на грешката]
    M -- Да --> O[Маркиране на акаунта като недостъпен за период на изчакване]
    O --> P{Има ли друг акаунт за доставчика?}
    P -- Да --> G
    P -- Не --> Q{Има ли следващ модел в комбото?}
    Q -- Да --> E
    Q -- Не --> R[Връщане на резултат, че всички са недостъпни]
```
...
sequenceDiagram
    autonumber
    participant UI as Потребителски интерфейс на таблото
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Сървър за удостоверяване на доставчика
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Изпълнител на доставчика

    UI->>OAuth: GET за оторизация или код на устройство
    OAuth->>ProvAuth: създаване на поток за удостоверяване/устройство
    ProvAuth-->>OAuth: URL за удостоверяване или данни за код на устройство
    OAuth-->>UI: данни за потока

    UI->>OAuth: POST за обмен или периодична проверка
    OAuth->>ProvAuth: обмен/периодична проверка на токен
    ProvAuth-->>OAuth: токени за достъп/обновяване
    OAuth->>DB: createProviderConnection(oauth данни)
    OAuth-->>UI: успех + идентификатор на връзката

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: проверка на идентификационните данни / незадължително обновяване
    Exec-->>Test: валидна информация или информация за обновен токен
    Test->>DB: актуализиране на състояние/токени/грешки
    Test-->>UI: резултат от проверката
```
...
sequenceDiagram
    autonumber
    participant UI as Потребителски интерфейс на страницата на крайната точка
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Външна облачна синхронизация
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: задаване на cloudEnabled=true
    Sync->>DB: гарантиране, че съществува API ключ
    Sync->>Cloud: POST /sync/{machineId} (доставчици/псевдоними/комбинации/ключове)
    Cloud-->>Sync: резултат от синхронизирането
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: активирано + състояние на проверката

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: отдалечени данни
    Sync->>DB: актуализиране на по-новите локални токени/състояние
    Sync-->>UI: синхронизирано

    UI->>Sync: POST action=disable
    Sync->>DB: задаване на cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: връщане на ANTHROPIC_BASE_URL към локалния адрес (ако е необходимо)
    Sync-->>UI: деактивирано
```
...
```mermaid
flowchart LR
    subgraph LocalHost[Хост на разработчика]
        CLI[CLI инструменти]
        Browser[Браузър за таблото]
    end

    subgraph ContainerOrProcess[Среда за изпълнение на OmniRoute]
        Next[Next.js сървър\nPORT=20128]
        Core[SSE ядро + изпълнители]
        MainDB[(storage.sqlite)]
        UsageDB[(таблици за употреба + артефакти от регистъра)]
    end

    subgraph External[Външни услуги]
        Providers[Доставчици на ИИ]
        SyncCloud[Облачна услуга за синхронизация]
    end
