...
- [نگاهی سریع به قیمتگذاری](#-pricing-at-a-glance)
- [موارد استفاده](#-use-cases)
- [راهاندازی ارائهدهنده](#-provider-setup)
- [یکپارچهسازی CLI](#-cli-integration)
- [استقرار](#-deployment)
- [مدلهای موجود](#-available-models)
- [قابلیتهای پیشرفته](#-advanced-features)
- [مسیریابی خودکار (بدون پیکربندی)](#-auto-routing-zero-config)
- [یکپارچهسازی MCP و A2A](#-mcp--a2a-integration)
- [سیستم مهارتها](#-skills-system)
- [سیستم حافظه](#-memory-system)
- [وبهوکها](#-webhooks)
- [عاملهای ابری](#-cloud-agents)
- [مدیریت برنامهنویسیشده](#-programmatic-management)
- [CLI داخلی](#-internal-cli)
- [برنامه دسکتاپ (Electron)](#-desktop-application-electron)
...
1. ثبتنام کنید: [Zhipu AI](https://open.bigmodel.cn)
...
1. ثبتنام کنید: [MiniMax](https://www.minimax.io)
...
1. اشتراک تهیه کنید: [Moonshot AI](https://platform.kimi.ai?aff=omniroute)
2. کلید API را دریافت کنید → داشبورد → افزودن کلید API
...
1. ثبتنام کنید: [Baidu AI Cloud Qianfan](https://cloud.baidu.com/product/wenxinworkshop)
...
[`docs/providers/CURSOR-DOCKER.md`](../providers/CURSOR-DOCKER.md) مراجعه کنید.
...
برای مرجع کامل متغیرهای محیطی، به [README](../README.md) مراجعه کنید.
...
<details>
<summary><b>مشاهده همه مدلهای موجود</b></summary>
...
**راهبردهای پیشرفته ترکیبی و خودکار** (برای هر ترکیب یا از طریق پیشوندهای `auto/*` قابلپیکربندی هستند — [AUTO-COMBO.md](../routing/AUTO-COMBO.md) را ببینید):
...
| **هوش مصنوعی** | بودجه تفکر (عبور بدون تغییر / حذف خودکار / سفارشی / تطبیقی — به [THINKING_BUDGET.md](./THINKING_BUDGET.md) مراجعه کنید)، پرامپت سیستمی سراسری، آمار کش پرامپت           |
...
مهلتهای زمانی مقصدهای ترکیب بهطور پیشفرض از مهلت زمانی درخواست فعلی به ارث برده میشوند. فقط زمانی از **مهلت زمانی مقصد
(ثانیه)** در پیشفرضهای ترکیب یا یک ترکیب خاص استفاده کنید که لازم است محدودیت کوتاهتری برای هر مقصد،
بازگشت به مقصد جایگزین را سریعتر فعال کند.

بهینهسازیهای بدون تأخیر ترکیب اختیاری هستند. **بهینهسازیهای بدون تأخیر** را غیرفعال نگه دارید تا
این قابلیتهای کاهش تأخیر از رقابت همزمان مقصدهای جایگزین، رد کردن مقصدها بر اساس سابقه
TTFT یا فشردهسازی درخواستهای جایگزین جلوگیری کنند؛ فعالسازی آن اجازه میدهد پوشش ریسک پیکربندیشده، رد کردن پیشبینانه بر اساس TTFT
و فشردهسازی پیشدستانه درخواست جایگزین، صحت مسیریابی/درخواست را در ازای کاهش
تأخیر انتهایی کمتر کنند.
...
مسیریاب خودکار بهطور کامل در [AUTO-COMBO.md](../routing/AUTO-COMBO.md) توضیح داده شده است — از جمله نحوه تنظیم وزنهای امتیازدهی، قراردادن ارائهدهندگان در فهرست سیاه و بررسی تصمیمهای مسیریابی در **داشبورد ← ترکیب خودکار**.
...
MCP در حال حاضر 32 دامنه نامگذاریشده تعریف میکند. هر کلید Bearer را میتوان به دامنههای مشخصی محدود کرد — برای فهرست مرجع دامنهها و ابزارها به [MCP-SERVER.md](../frameworks/MCP-SERVER.md) و برای طرحواره JSON-RPC به [A2A-SERVER.md](../frameworks/A2A-SERVER.md) مراجعه کنید.
...
مرجع کامل: [SKILLS.md](../frameworks/SKILLS.md).
...
حافظهها را در **داشبورد ← حافظه** مدیریت کنید (جستوجو، ویرایش، برونبری و پاکسازی). رابط HTTP (`/api/memory/*`) به عاملها اجازه میدهد واقعیتها را بهصورت برنامهنویسیشده ارسال و جستوجو کنند — به [MEMORY.md](../frameworks/MEMORY.md) مراجعه کنید.
...
طرحواره کامل در [WEBHOOKS.md](../frameworks/WEBHOOKS.md).
...
مرجع کامل: [CLOUD_AGENT.md](../frameworks/CLOUD_AGENT.md).
...
برای مشاهده فهرست کامل نقاط پایانی و طرحوارههای درخواست/پاسخ، به [API_REFERENCE.md](../reference/API_REFERENCE.md) مراجعه کنید.
...
| `OMNIROUTE_MEMORY_MB` | `512`   | محدودیت heap در Node.js (64–16384 MB) |
