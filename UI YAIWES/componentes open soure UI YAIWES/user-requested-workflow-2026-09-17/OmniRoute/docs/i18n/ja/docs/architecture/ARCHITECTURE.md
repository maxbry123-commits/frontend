...
![3層レジリエンスモデル](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[開発者向けクライアント]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[カスタムOpenAI互換クライアント]
        BROWSER[ブラウザダッシュボード]
    end

    subgraph Router[OmniRouteローカルプロセス]
        API[V1互換API\n/v1/*]
        DASH[ダッシュボード + 管理API\n/api/*]
        CORE[SSE + 変換コア\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(使用量テーブル + ログアーティファクト)]
    end

    subgraph Upstreams[アップストリームプロバイダー]
        P1[OAuthプロバイダー\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[APIキープロバイダー\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[互換ノード\nOpenAI互換 / Anthropic互換]
    end

    subgraph Cloud[オプションのクラウド同期]
...
sequenceDiagram
    autonumber
    participant Client as CLI/SDK クライアント
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as モデルリゾルバー
    participant Auth as 認証情報セレクター
    participant Exec as プロバイダーエグゼキューター
    participant Prov as アップストリームプロバイダー
    participant Stream as ストリームトランスレーター
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: モデルまたはコンボを解析/解決

    alt コンボモデル
        Chat->>Chat: コンボモデルを反復処理 (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: アクティブなアカウント + トークン/API キー

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: ソース形式を検出
    Core->>Core: リクエストをターゲット形式に変換
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: アップストリーム API 呼び出し
    Prov-->>Exec: SSE/JSON レスポンス
    Exec-->>Core: レスポンス + メタデータ
    ...
    Core->>Stream: ストリームをクライアント形式に変換/正規化
    Stream-->>Client: SSE チャンク / JSON レスポンス

    Stream->>Usage: 使用量を抽出 + 履歴/ログを永続化
```
...
```mermaid
flowchart TD
    A[受信したモデル文字列] --> B{コンボ名か？}
    B -- はい --> C[コンボのモデルシーケンスを読み込む]
    B -- いいえ --> D[単一モデルのパス]
    ...
    F --> G[アカウント認証情報を選択]
    G --> H{認証情報を利用できるか？}
    H -- いいえ --> I[プロバイダー利用不可を返す]
    H -- はい --> J[リクエストを実行]

    J --> K{成功したか？}
    K -- はい --> L[レスポンスを返す]
    K -- いいえ --> M{フォールバック可能なエラーか？}

    M -- いいえ --> N[エラーを返す]
    M -- はい --> O[アカウントをクールダウン期間中の利用不可としてマーク]
    O --> P{同じプロバイダーに別のアカウントがあるか？}
    P -- はい --> G
    P -- いいえ --> Q{コンボ内に次のモデルがあるか？}
    Q -- はい --> E
    Q -- いいえ --> R[すべて利用不可を返す]
```
...
sequenceDiagram
    ...
    participant ProvAuth as プロバイダー認証サーバー
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as プロバイダーエグゼキューター

    UI->>OAuth: GET authorize または device-code
    OAuth->>ProvAuth: 認証/デバイスフローを作成
    ProvAuth-->>OAuth: 認証 URL またはデバイスコードのペイロード
    OAuth-->>UI: フローデータ

    UI->>OAuth: POST exchange または poll
    OAuth->>ProvAuth: トークンの交換/ポーリング
    ProvAuth-->>OAuth: アクセス/リフレッシュトークン
    OAuth->>DB: createProviderConnection(oauth data)
    OAuth-->>UI: 成功 + 接続 ID

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: 認証情報を検証 / 必要に応じて更新
    Exec-->>Test: 有効、または更新されたトークン情報
    Test->>DB: ステータス/トークン/エラーを更新
    Test-->>UI: 検証結果
```
...
sequenceDiagram
    autonumber
    participant UI as エンドポイントページ UI
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as 外部クラウド同期
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=true に設定
    Sync->>DB: API キーが存在することを確認
    Sync->>Cloud: POST /sync/{machineId}（プロバイダー/エイリアス/コンボ/キー）
    Cloud-->>Sync: 同期結果
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: 有効化済み + 検証ステータス

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: リモートデータ
    Sync->>DB: より新しいローカルトークン/ステータスを更新
    Sync-->>UI: 同期済み

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=false に設定
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URL をローカルに戻す（必要な場合）
    Sync-->>UI: 無効化済み
```
...
```mermaid
flowchart LR
    subgraph LocalHost[開発者ホスト]
        CLI[CLIツール]
        Browser[ダッシュボードブラウザー]
    end

    subgraph ContainerOrProcess[OmniRouteランタイム]
        Next[Next.jsサーバー\nPORT=20128]
        Core[SSEコア + エグゼキューター]
        MainDB[(storage.sqlite)]
        UsageDB[(使用量テーブル + ログ成果物)]
    end

    subgraph External[外部サービス]
        Providers[AIプロバイダー]
        SyncCloud[クラウド同期サービス]
    end
