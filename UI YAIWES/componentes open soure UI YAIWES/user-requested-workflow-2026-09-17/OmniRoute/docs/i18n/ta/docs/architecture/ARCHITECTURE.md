...
```mermaid
flowchart LR
    subgraph Clients[டெவலப்பர் கிளையன்ட்கள்]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[தனிப்பயன் OpenAI-இணக்கமான கிளையன்ட்கள்]
        BROWSER[உலாவி டாஷ்போர்டு]
    end

    subgraph Router[OmniRoute உள்ளூர் செயல்முறை]
        API[V1 இணக்கத்தன்மை API\n/v1/*]
        DASH[டாஷ்போர்டு + மேலாண்மை API\n/api/*]
        CORE[SSE + மொழிபெயர்ப்பு மையம்\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(பயன்பாட்டு அட்டவணைகள் + பதிவு கலைப்பொருட்கள்)]
    end

    subgraph Upstreams[அப்ஸ்ட்ரீம் வழங்குநர்கள்]
        P1[OAuth வழங்குநர்கள்\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API விசை வழங்குநர்கள்\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[இணக்கமான நோடுகள்\nOpenAI-இணக்கமான / Anthropic-இணக்கமான]
    end

    subgraph Cloud[விருப்பத் தேர்வான கிளவுட் ஒத்திசைவு]
        CLOUD[கிளவுட் ஒத்திசைவு முடிவுப்புள்ளி\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as CLI/SDK கிளையண்ட்
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as மாடல் தீர்மானிப்பான்
    participant Auth as நற்சான்றுத் தேர்வி
    participant Exec as வழங்குநர் இயக்கி
    participant Prov as மேல்நிலை வழங்குநர்
    participant Stream as ஸ்ட்ரீம் மொழிமாற்றி
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: மாடல் அல்லது combo-வைப் பகுத்து/தீர்மானிக்கவும்

    alt Combo மாடல்
        Chat->>Chat: combo மாடல்களைச் சுழன்று செயலாக்கவும் (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: செயலில் உள்ள கணக்கு + token-கள்/api விசை

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: மூல வடிவமைப்பைக் கண்டறியவும்
    Core->>Core: கோரிக்கையை இலக்கு வடிவமைப்புக்கு மாற்றவும்
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: மேல்நிலை API அழைப்பு
    Prov-->>Exec: SSE/JSON பதில்
    Exec-->>Core: பதில் + metadata

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: புதுப்பிக்கப்பட்ட token-கள்
        Core->>Exec: கோரிக்கையை மீண்டும் முயற்சிக்கவும்
    end

    Core->>Stream: ஸ்ட்ரீமை கிளையண்ட் வடிவமைப்புக்கு மாற்றி/இயல்பாக்கவும்
    Stream-->>Client: SSE துண்டுகள் / JSON பதில்

    Stream->>Usage: பயன்பாட்டைப் பிரித்தெடுத்து + வரலாறு/பதிவை நிலையாகச் சேமிக்கவும்
```
...
```mermaid
flowchart TD
    A[உள்வரும் மாடல் சரம்] --> B{காம்போ பெயரா?}
    B -- ஆம் --> C[காம்போ மாடல்களின் வரிசையை ஏற்று]
    B -- இல்லை --> D[ஒற்றை மாடல் பாதை]

    C --> E[மாடல் N-ஐ முயற்சிக்கவும்]
    E --> F[வழங்குநர்/மாடலைத் தீர்மானிக்கவும்]
    D --> F

    F --> G[கணக்கு நற்சான்றுகளைத் தேர்ந்தெடுக்கவும்]
    G --> H{நற்சான்றுகள் கிடைக்கின்றனவா?}
    H -- இல்லை --> I[வழங்குநர் கிடைக்கவில்லை எனத் திருப்பியனுப்பு]
    H -- ஆம் --> J[கோரிக்கையைச் செயல்படுத்து]

    J --> K{வெற்றியா?}
    K -- ஆம் --> L[பதிலைத் திருப்பியனுப்பு]
    K -- இல்லை --> M{மாற்று வழிக்குத் தகுதியான பிழையா?}

    M -- இல்லை --> N[பிழையைத் திருப்பியனுப்பு]
    M -- ஆம் --> O[கணக்கைத் தற்காலிகமாகக் கிடைக்காததாகக் குறிக்கவும்]
    O --> P{வழங்குநருக்கு வேறொரு கணக்கு உள்ளதா?}
    P -- ஆம் --> G
    P -- இல்லை --> Q{அடுத்த மாடலுடன் காம்போவில் உள்ளதா?}
    Q -- ஆம் --> E
    Q -- இல்லை --> R[அனைத்தும் கிடைக்கவில்லை எனத் திருப்பியனுப்பு]
```
...
sequenceDiagram
    autonumber
    participant UI as கட்டுப்பாட்டுப் பலகை UI
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as வழங்குநர் அங்கீகாரச் சேவையகம்
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as வழங்குநர் செயலாக்கி

    UI->>OAuth: GET அங்கீகரிப்பு அல்லது சாதனக் குறியீடு
    OAuth->>ProvAuth: அங்கீகார/சாதன ஓட்டத்தை உருவாக்கு
    ProvAuth-->>OAuth: அங்கீகார URL அல்லது சாதனக் குறியீட்டுத் தரவுத்தொகுப்பு
    OAuth-->>UI: ஓட்டத் தரவு

    UI->>OAuth: POST பரிமாற்றம் அல்லது வாக்கெடுப்பு
    OAuth->>ProvAuth: டோக்கன் பரிமாற்றம்/வாக்கெடுப்பு
    ProvAuth-->>OAuth: அணுகல்/புதுப்பிப்பு டோக்கன்கள்
    OAuth->>DB: createProviderConnection(oauth தரவு)
    OAuth-->>UI: வெற்றி + இணைப்பு id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: நற்சான்றுகளைச் சரிபார் / விருப்பப் புதுப்பிப்பு
    Exec-->>Test: செல்லுபடியாகும் அல்லது புதுப்பிக்கப்பட்ட டோக்கன் தகவல்
    Test->>DB: நிலை/டோக்கன்கள்/பிழைகளைப் புதுப்பி
    Test-->>UI: சரிபார்ப்பு முடிவு
```
...
sequenceDiagram
    autonumber
    participant UI as இறுதிப்புள்ளிப் பக்க UI
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as வெளிப்புற கிளவுட் ஒத்திசைவு
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=true என அமை
    Sync->>DB: API விசை இருப்பதை உறுதிசெய்
    Sync->>Cloud: POST /sync/{machineId} (வழங்குநர்கள்/மாற்றுப்பெயர்கள்/காம்போக்கள்/விசைகள்)
    Cloud-->>Sync: ஒத்திசைவு முடிவு
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: இயக்கப்பட்டது + சரிபார்ப்பு நிலை

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: தொலைநிலைத் தரவு
    Sync->>DB: புதிய உள்ளூர் டோக்கன்கள்/நிலையைப் புதுப்பி
    Sync-->>UI: ஒத்திசைக்கப்பட்டது

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=false என அமை
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URL-ஐ மீண்டும் உள்ளகத்திற்கு மாற்று (தேவைப்பட்டால்)
    Sync-->>UI: முடக்கப்பட்டது
```
