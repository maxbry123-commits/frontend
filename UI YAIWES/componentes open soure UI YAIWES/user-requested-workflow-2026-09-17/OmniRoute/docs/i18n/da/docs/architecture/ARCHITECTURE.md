...
![Resiliensmodel med 3 lag](../diagrams/exported/resilience-3layers.svg)
...
sequenceDiagram
    ...
    participant Model as Modelopløser
    participant Auth as Legitimationsvælger
    participant Exec as Udbyderudfører
    participant Prov as Opstrømsudbyder
    participant Stream as Streamoversætter
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: fortolk/løs model eller kombination

    alt Kombinationsmodel
        Chat->>Chat: gennemløb kombinationsmodeller (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: aktiv konto + tokens/API-nøgle

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: registrer kildeformat
    Core->>Core: oversæt anmodning til målformat
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: opstrøms-API-kald
    Prov-->>Exec: SSE/JSON-svar
    Exec-->>Core: svar + metadata

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: opdaterede tokens
        Core->>Exec: prøv anmodningen igen
    end

    Core->>Stream: oversæt/normaliser stream til klientformat
    Stream-->>Client: SSE-fragmenter/JSON-svar

    Stream->>Usage: udtræk forbrug + gem historik/log
```
...
```mermaid
flowchart TD
    A[Indgående modelstreng] --> B{Er det et kombinationsnavn?}
    B -- Ja --> C[Indlæs kombinationens modelsekvens]
    B -- Nej --> D[Sti for enkeltmodel]

    C --> E[Prøv model N]
    E --> F[Find udbyder/model]
    D --> F

    F --> G[Vælg kontolegitimationsoplysninger]
    G --> H{Er legitimationsoplysninger tilgængelige?}
    H -- Nej --> I[Returner, at udbyderen ikke er tilgængelig]
    H -- Ja --> J[Udfør anmodning]

    J --> K{Lykkedes det?}
    K -- Ja --> L[Returner svar]
    K -- Nej --> M{Fejl, der tillader reserve?}

    M -- Nej --> N[Returner fejl]
    M -- Ja --> O[Markér konto som utilgængelig i nedkølingsperioden]
    O --> P{En anden konto til udbyderen?}
    P -- Ja --> G
    P -- Nej --> Q{I en kombination med en efterfølgende model?}
    Q -- Ja --> E
    Q -- Nej --> R[Returner, at alle er utilgængelige]
```
...
sequenceDiagram
    autonumber
    participant UI as Dashboardbrugerflade
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Udbyderens godkendelsesserver
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Udbyderkørsel

    UI->>OAuth: GET-godkendelse eller enhedskode
    OAuth->>ProvAuth: opret godkendelses-/enhedsflow
    ProvAuth-->>OAuth: godkendelses-URL eller enhedskodedata
    OAuth-->>UI: flowdata

    UI->>OAuth: POST-udveksling eller forespørgsel
    OAuth->>ProvAuth: tokenudveksling/-forespørgsel
    ProvAuth-->>OAuth: adgangs-/fornyelsestokens
    OAuth->>DB: createProviderConnection(oauth-data)
    OAuth-->>UI: gennemført + forbindelses-id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: valider legitimationsoplysninger / valgfri fornyelse
    Exec-->>Test: gyldige eller fornyede tokenoplysninger
    Test->>DB: opdater status/tokens/fejl
    Test-->>UI: valideringsresultat
```
...
sequenceDiagram
    autonumber
    participant UI as Slutpunktsidebrugerflade
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Ekstern cloudsynkronisering
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: indstil cloudEnabled=true
    Sync->>DB: sørg for, at der findes en API-nøgle
    Sync->>Cloud: POST /sync/{machineId} (udbydere/aliasser/kombinationer/nøgler)
    Cloud-->>Sync: synkroniseringsresultat
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: aktiveret + bekræftelsesstatus

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: eksterne data
    Sync->>DB: opdater nyere lokale tokens/status
    Sync-->>UI: synkroniseret

    UI->>Sync: POST action=disable
    Sync->>DB: indstil cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: skift ANTHROPIC_BASE_URL tilbage til lokal (hvis nødvendigt)
    Sync-->>UI: deaktiveret
```
