...
![Pipeline ng request (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)

> Source: [diagrams/request-pipeline.mmd](../diagrams/request-pipeline.mmd)

![3-layer na modelo ng resilience](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Mga Developer Client]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Mga custom na OpenAI-compatible client]
        BROWSER[Dashboard ng Browser]
    end

    subgraph Router[Lokal na Proseso ng OmniRoute]
        API[V1 Compatibility API\n/v1/*]
        DASH[Dashboard + Management API\n/api/*]
        CORE[SSE + Translation Core\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(mga talahanayan ng paggamit + mga log artifact)]
    end

    subgraph Upstreams[Mga Upstream Provider]
        P1[Mga OAuth Provider\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[Mga API Key Provider\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Mga Compatible Node\nOpenAI-compatible / Anthropic-compatible]
    end

    subgraph Cloud[Opsyonal na Cloud Sync]
...
sequenceDiagram
    ...
    participant Model as Resolver ng Model
    participant Auth as Tagapili ng Credential
    participant Exec as Executor ng Provider
    participant Prov as Upstream Provider
    participant Stream as Tagapagsalin ng Stream
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: i-parse/i-resolve ang model o combo

    alt Combo model
        Chat->>Chat: ulitin sa mga combo model (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: aktibong account + mga token/api key

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: tukuyin ang source format
    Core->>Core: isalin ang request sa target format
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: tawag sa upstream API
    Prov-->>Exec: tugon na SSE/JSON
    Exec-->>Core: tugon + metadata

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: mga na-update na token
        Core->>Exec: subukang muli ang request
    end

    Core->>Stream: isalin/i-normalize ang stream sa format ng client
    Stream-->>Client: mga SSE chunk / tugong JSON

    Stream->>Usage: kunin ang paggamit + i-persist ang history/log
```
...
```mermaid
flowchart TD
    A[Papasok na string ng modelo] --> B{Pangalan ba ng combo?}
    B -- Oo --> C[I-load ang pagkakasunod-sunod ng mga modelo sa combo]
    B -- Hindi --> D[Landas ng iisang modelo]

    C --> E[Subukan ang modelong N]
    E --> F[Tukuyin ang provider/model]
    D --> F

    F --> G[Piliin ang mga kredensyal ng account]
    G --> H{May available na mga kredensyal?}
    H -- Wala --> I[Ibalik na hindi available ang provider]
    H -- Mayroon --> J[Isagawa ang request]

    J --> K{Tagumpay?}
    K -- Oo --> L[Ibalik ang response]
    K -- Hindi --> M{Error na kwalipikado para sa fallback?}

    M -- Hindi --> N[Ibalik ang error]
    M -- Oo --> O[Markahan ang account bilang hindi available sa panahon ng cooldown]
    O --> P{May iba pang account para sa provider?}
    P -- Oo --> G
    P -- Wala --> Q{Nasa combo na may kasunod na modelo?}
    Q -- Oo --> E
    Q -- Hindi --> R[Ibalik na hindi available ang lahat]
```
...
sequenceDiagram
    autonumber
    participant UI as UI ng Dashboard
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Auth Server ng Provider
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Executor ng Provider

    UI->>OAuth: GET authorize o device-code
    OAuth->>ProvAuth: gumawa ng auth/device flow
    ProvAuth-->>OAuth: auth URL o payload ng device code
    OAuth-->>UI: data ng flow

    UI->>OAuth: POST exchange o poll
    OAuth->>ProvAuth: token exchange/poll
    ProvAuth-->>OAuth: mga access/refresh token
    OAuth->>DB: createProviderConnection(oauth data)
    OAuth-->>UI: tagumpay + connection id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: i-validate ang mga kredensyal / opsyonal na pag-refresh
    Exec-->>Test: valid o na-refresh na impormasyon ng token
    Test->>DB: i-update ang status/mga token/mga error
    Test-->>UI: resulta ng validation
```
...
sequenceDiagram
    autonumber
    participant UI as UI ng Endpoint Page
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Panlabas na Cloud Sync
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: itakda ang cloudEnabled=true
    Sync->>DB: tiyaking may API key
    Sync->>Cloud: POST /sync/{machineId} (mga provider/alias/combo/key)
    Cloud-->>Sync: resulta ng sync
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: naka-enable + status ng verification

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: remote na data
    Sync->>DB: i-update ang mas bagong mga lokal na token/status
    Sync-->>UI: na-sync

    UI->>Sync: POST action=disable
    Sync->>DB: itakda ang cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ibalik ang ANTHROPIC_BASE_URL sa lokal (kung kinakailangan)
    Sync-->>UI: naka-disable
```
...
```mermaid
flowchart LR
    subgraph LocalHost[Host ng Developer]
        CLI[Mga CLI Tool]
        Browser[Browser ng Dashboard]
    end

    subgraph ContainerOrProcess[OmniRoute Runtime]
        Next[Next.js Server\nPORT=20128]
        Core[SSE Core + Mga Executor]
        MainDB[(storage.sqlite)]
        UsageDB[(mga talahanayan ng paggamit + mga artifact ng log)]
    end

    subgraph External[Mga Panlabas na Serbisyo]
        Providers[Mga AI Provider]
        SyncCloud[Serbisyo ng Cloud Sync]
    end
