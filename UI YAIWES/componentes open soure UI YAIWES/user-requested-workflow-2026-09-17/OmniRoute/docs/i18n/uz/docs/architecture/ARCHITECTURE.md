...
![Soʻrov konveyeri (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)

> Manba: [diagrams/request-pipeline.mmd](../diagrams/request-pipeline.mmd)

![3 qatlamli bardoshlilik modeli](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Dasturchi Klientlari]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[OpenAI bilan moslashtirilgan maxsus klientlar]
        BROWSER[Brauzer Boshqaruv Paneli]
    end

    subgraph Router[OmniRoute Mahalliy Jarayoni]
        API[V1 Moslik API’si\n/v1/*]
        DASH[Boshqaruv Paneli + Boshqaruv API’si\n/api/*]
        CORE[SSE + Tarjima Yadrosi\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(foydalanish jadvallari + jurnal artefaktlari)]
    end

    subgraph Upstreams[Yuqori Oqim Provayderlari]
        P1[OAuth Provayderlari\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API Kaliti Provayderlari\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Mos Tugunlar\nOpenAI bilan moslashtirilgan / Anthropic bilan moslashtirilgan]
    end

    subgraph Cloud[Ixtiyoriy Bulutli Sinxronlash]
        CLOUD[Bulutli Sinxronlash Yakuniy Nuqtasi\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    autonumber
    participant Client as CLI/SDK mijozi
    participant Route as /api/v1/chat/completions
    participant Chat as src/sse/handlers/chat
    participant Core as open-sse/handlers/chatCore
    participant Model as Modelni aniqlovchi
    participant Auth as Hisob maʼlumotlarini tanlovchi
    participant Exec as Provayder bajaruvchisi
    participant Prov as Yuqori oqim provayderi
    participant Stream as Oqim tarjimoni
    participant Usage as usageDb

    Client->>Route: POST /v1/chat/completions
    Route->>Chat: handleChat(request)
    Chat->>Model: model yoki kombinatsiyani tahlil qilish/aniqlash

    alt Kombinatsiya modeli
        Chat->>Chat: kombinatsiya modellarini ketma-ket koʻrib chiqish (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: faol hisob + tokenlar/api kaliti

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: manba formatini aniqlash
    Core->>Core: soʻrovni maqsadli formatga oʻgirish
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: yuqori oqim API chaqiruvi
    Prov-->>Exec: SSE/JSON javobi
    Exec-->>Core: javob + metamaʼlumotlar

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: yangilangan tokenlar
        Core->>Exec: soʻrovni qayta yuborish
    end

    Core->>Stream: oqimni mijoz formatiga oʻgirish/meʼyorlashtirish
    Stream-->>Client: SSE qismlari / JSON javobi

    Stream->>Usage: foydalanish maʼlumotlarini ajratib olish + tarix/jurnalni saqlash
```
...
```mermaid
flowchart TD
    A[Kiruvchi model satri] --> B{Combo nomimi?}
    B -- Ha --> C[Combo modellari ketma-ketligini yuklash]
    B -- Yoʻq --> D[Yagona model yoʻli]

    C --> E[N-modelni sinab koʻrish]
    E --> F[Provayder/modelni aniqlash]
    D --> F

    F --> G[Hisob qaydnomasi hisob maʼlumotlarini tanlash]
    G --> H{Hisob maʼlumotlari mavjudmi?}
    H -- Yoʻq --> I[Provayder mavjud emasligini qaytarish]
    H -- Ha --> J[Soʻrovni bajarish]

    J --> K{Muvaffaqiyatli boʻldimi?}
    K -- Ha --> L[Javobni qaytarish]
    K -- Yoʻq --> M{Xato zaxira oʻtish uchun mosmi?}

    M -- Yoʻq --> N[Xatoni qaytarish]
    M -- Ha --> O[Hisob qaydnomasini kutish davrida mavjud emas deb belgilash]
    O --> P{Provayder uchun boshqa hisob qaydnomasi bormi?}
    P -- Ha --> G
    P -- Yoʻq --> Q{Keyingi modelga ega combo ichidami?}
    Q -- Ha --> E
    Q -- Yoʻq --> R[Hammasi mavjud emasligini qaytarish]
```
...
sequenceDiagram
    autonumber
    participant UI as Boshqaruv paneli interfeysi
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Provayderning autentifikatsiya serveri
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Provayder ijrochisi

    UI->>OAuth: GET orqali avtorizatsiya yoki qurilma kodini olish
    OAuth->>ProvAuth: autentifikatsiya/qurilma jarayonini yaratish
    ProvAuth-->>OAuth: autentifikatsiya URL manzili yoki qurilma kodi maʼlumotlari
    OAuth-->>UI: jarayon maʼlumotlari

    UI->>OAuth: POST orqali almashtirish yoki soʻrov holatini tekshirish
    OAuth->>ProvAuth: tokenni almashtirish/soʻrov holatini tekshirish
    ProvAuth-->>OAuth: kirish/yangilash tokenlari
    OAuth->>DB: createProviderConnection(OAuth maʼlumotlari)
    OAuth-->>UI: muvaffaqiyat + ulanish identifikatori

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: hisob maʼlumotlarini tekshirish / ixtiyoriy yangilash
    Exec-->>Test: yaroqli yoki yangilangan token maʼlumotlari
    Test->>DB: holat/tokenlar/xatolarni yangilash
    Test-->>UI: tekshiruv natijasi
```
...
sequenceDiagram
    autonumber
    participant UI as Endpoint sahifasi interfeysi
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Tashqi bulutli sinxronlash
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: cloudEnabled=true qiymatini oʻrnatish
    Sync->>DB: API kaliti mavjudligini taʼminlash
    Sync->>Cloud: POST /sync/{machineId} (provayderlar/aliaslar/combolar/kalitlar)
    Cloud-->>Sync: sinxronlash natijasi
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: yoqildi + tekshiruv holati

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: masofaviy maʼlumotlar
    Sync->>DB: yangiroq mahalliy tokenlar/holatni yangilash
    Sync-->>UI: sinxronlandi

    UI->>Sync: POST action=disable
    Sync->>DB: cloudEnabled=false qiymatini oʻrnatish
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: ANTHROPIC_BASE_URL manzilini mahalliy manzilga qaytarish (zarur boʻlsa)
    Sync-->>UI: oʻchirildi
```
...
```mermaid
flowchart LR
    subgraph LocalHost[Dasturchi xosti]
        CLI[CLI vositalari]
        Browser[Boshqaruv paneli brauzeri]
    end

    subgraph ContainerOrProcess[OmniRoute ish muhiti]
        Next[Next.js serveri\nPORT=20128]
        Core[SSE yadrosi + ijrochilar]
        MainDB[(storage.sqlite)]
        UsageDB[(foydalanish jadvallari + jurnal artefaktlari)]
    end

    subgraph External[Tashqi xizmatlar]
        Providers[AI provayderlari]
        SyncCloud[Bulutli sinxronlash xizmati]
    end
