...
func TestResolver_RealWorldScenario(t *testing.T) {
	...
	coordinatorFinder := newFinder(tmpDir, serviceregistry.ServiceNameCoordinator, true)
	...
	filename := instanceFilePath(tmpDir, string(serviceregistry.ServiceNameCoordinator), coordinator1.ID)
	...
	filename = instanceFilePath(tmpDir, string(serviceregistry.ServiceNameCoordinator), coordinator2.ID)
...
func TestResolver_Members_CacheConcurrency(t *testing.T) {
	...
	results := make(chan []serviceregistry.HostInfo, numGoroutines)
