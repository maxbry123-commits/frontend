...
class FakeLedger:
    ...
    def mark_pending(self, effect_id, payload_sha256):
        self.pending_calls += 1
        self.records[effect_id] = EffectRecord("pending", payload_sha256)

    def commit(self, effect_id, payload_sha256, output_sha256):
        self.commit_calls += 1
        self.records[effect_id] = EffectRecord("committed", payload_sha256, output_sha256)


class CorruptReadbackLedger(FakeLedger):
    def __init__(self):
        super().__init__()
        self.corrupt_pending = False

    def mark_pending(self, effect_id, payload_sha256):
        super().mark_pending(effect_id, payload_sha256)
        if self.corrupt_pending:
            self.records[effect_id] = EffectRecord("pending", "0" * 64)
...
class ProductRecoveryTests(unittest.TestCase):
    ...
    def test_cancel_after_pending_preserves_restartable_pending_state(self):
        payload = b"payload"
        ledger = FakeLedger()
        ledger.records["effect-1"] = EffectRecord("pending", hashlib.sha256(payload).hexdigest())
        sink = IdempotentSink()
        outcome = resume_effect(
            effect_id="effect-1",
            payload=payload,
    ...
    def test_same_effect_id_with_different_payload_fails_closed_before_delivery(self):
        original = b"original"
        ledger = FakeLedger()
        ledger.records["effect-1"] = EffectRecord(
            "pending", hashlib.sha256(original).hexdigest()
        )
        sink = IdempotentSink()
        with self.assertRaisesRegex(RecoveryControlPlaneError, "effect_payload_mismatch"):
            resume_effect(
                effect_id="effect-1",
                payload=b"different",
                ledger=ledger,
    ...
    def test_pending_ledger_write_must_read_back_exact_payload_hash(self):
        ledger = CorruptReadbackLedger()
        ledger.corrupt_pending = True
        sink = IdempotentSink()
        with self.assertRaisesRegex(RecoveryControlPlaneError, "effect_payload_mismatch"):
            resume_effect(
                effect_id="effect-1",
