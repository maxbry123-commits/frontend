...
type StaticRegistry struct {
	coordinators []serviceregistry.HostInfo
}
...
func NewStaticRegistry(addresses []string) (*StaticRegistry, error) {
	hosts := make([]serviceregistry.HostInfo, 0, len(addresses))
	...
	for _, addr := range addresses {
		...
		hosts = append(hosts, serviceregistry.HostInfo{
...
func (r *StaticRegistry) Register(_ context.Context, _ serviceregistry.ServiceName, _ serviceregistry.HostInfo) error {
...
func (r *StaticRegistry) GetServiceMembers(_ context.Context, name serviceregistry.ServiceName) ([]serviceregistry.HostInfo, error) {
	if name == serviceregistry.ServiceNameCoordinator {
...
func (r *StaticRegistry) UpdateStatus(_ context.Context, _ serviceregistry.ServiceName, _ serviceregistry.ServiceStatus) error {
	// Static registry doesn't support status updates
	return nil
}

// Ensure StaticRegistry implements ServiceRegistry
var _ serviceregistry.ServiceRegistry = (*StaticRegistry)(nil)
