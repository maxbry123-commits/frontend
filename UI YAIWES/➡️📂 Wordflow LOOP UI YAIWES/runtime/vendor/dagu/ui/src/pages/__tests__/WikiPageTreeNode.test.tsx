...
import { WikiPageTreeNodeResponseType } from '@/api/v1/schema';
import WikiPageTreeNode from '../wiki/components/WikiPageTreeNode';

function makeFileNode(workspace?: string | null) {
  const select = vi.fn();
  const activate = vi.fn();
  return {
    id: workspace ? `${workspace}/docs/deploy` : 'docs/deploy',
    data: {
      id: workspace ? `${workspace}/docs/deploy` : 'docs/deploy',
      name: 'deploy',
      title: 'deploy',
      type: WikiPageTreeNodeResponseType.file,
      workspace,
    },
    isEditing: false,
    isSelected: false,
...
describe('WikiPageTreeNode', () => {
  ...
  it('opens rename with the node workspace and visible Wiki page path', async () => {
    const user = userEvent.setup();
