...
class EffectLedger(Protocol):
    ...
    def mark_pending(self, effect_id: str) -> None: ...

    def commit(self, effect_id: str, output_sha256: str) -> None: ...
...
def resume_effect(
    *,
    ...
    """Resume an effect with a stable idempotency key and fail-closed ledger.

    If a disconnect happens after delivery but before commit, the ledger remains
    ``pending``. On restart the exact same ``effect_id`` is sent again; the
    effect adapter must deduplicate that idempotency key and report
    ``duplicate=True``. A committed effect is never sent again.
    """
    ...
    if record is not None:
        ...
        if record.status != "pending" or record.output_sha256 is not None:
            raise RecoveryControlPlaneError("invalid_effect_record")
    else:
        try:
            if cancelled():
    ...
    try:
        ledger.commit(effect_id, output_sha256)
    except Exception as exc:
