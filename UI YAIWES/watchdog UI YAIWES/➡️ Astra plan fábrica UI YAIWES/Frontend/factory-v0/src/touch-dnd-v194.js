// YAIWES Factory V1.9.4 — touch/pen gesture arbitration.
// Copies V1.9.3 helpers. Does not import touch-dnd-v193.js (that module auto-mounts).
// Does not mutate touch-dnd-v192.js or touch-dnd-v193.js.
// Producer segment: do not wire the integrator candidate from this file.

export const TOUCH_DND_VERSION = '1.9.4';
...
export function mountTouchDndV194(options = {}) {
  ...
  if (!force && (existingMark === 'v192' || existingMark === 'v193' || existingMark === 'v194')) {
    return { ok: false, reason: 'ALREADY_MOUNTED', version: TOUCH_DND_VERSION, mounted: existingMark };
  }
  if (force && globalThis.__YAIWES_TOUCH_DND_V194__?.destroy) {
    globalThis.__YAIWES_TOUCH_DND_V194__.destroy();
  }
  ...
  function cancel(source) {
    if (session?.source === source) {
      session.node?.classList?.remove?.('is-pointer-moving');
      session = null;
    }
  }
  ...
  if (doc.documentElement) doc.documentElement.dataset.touchDnd = 'v194';
  ...
  const api = {
    ...
    destroy() {
      abort.abort();
      session = null;
      if (doc.documentElement?.dataset?.touchDnd === 'v194') delete doc.documentElement.dataset.touchDnd;
      if (globalThis.__YAIWES_TOUCH_DND_V194__ === api) delete globalThis.__YAIWES_TOUCH_DND_V194__;
    },
  };
  globalThis.__YAIWES_TOUCH_DND_V194__ = api;
  return { ok: true, ...api };
}

if (typeof document !== 'undefined') {
  mountTouchDndV194({ auto: true });
}
