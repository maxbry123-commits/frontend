...
const shotDir='test-results/f-ui-060';
test.beforeAll(async()=>{await fs.mkdir(shotDir,{recursive:true});});

test('HF Jobs UI syncs run/status/logs/complete/cancel without secrets', async ({page})=>{
  const jobs=new Map();
  let seq=0;
  await page.route('https://hf-jobs-proxy.test/**', async route=>{
    ...
    if(path.endsWith('/run')){
      const id=`job-${++seq}`;jobs.set(id,{stage:'RUNNING',logs:'boot'});
      return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({job_id:id,stage:'RUNNING',token:'server-secret'})});
    }
    const match=path.match(/\/jobs\/([^/]+)(?:\/(logs|cancel))?$/);
    if(!match)return route.fulfill({status:404,body:'{}'});
    const id=decodeURIComponent(match[1]);const action=match[2];const state=jobs.get(id)||{stage:'UNKNOWN',logs:''};
    ...
    state.stage='COMPLETED';state.logs='boot\ndone';jobs.set(id,state);
    return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({job_id:id,stage:'COMPLETED'})});
  });
  ...
  await page.evaluate(()=>localStorage.setItem('yaiwes-factory-config-v13',JSON.stringify({
    ...
    hfJobs:{url:'https://hf-jobs-proxy.test/api',secretRef:'HF_JOBS_TOKEN'}
  })));
  await page.reload();
  ...
  const firstId=await page.locator('#hf-job-status').getAttribute('data-hf-job-id');
  expect(firstId).toBe('job-1');
  await page.screenshot({path:`${shotDir}/01-running.png`,fullPage:true});
  ...
  await page.screenshot({path:`${shotDir}/02-completed.png`,fullPage:true});
  ...
  await page.screenshot({path:`${shotDir}/03-canceled.png`,fullPage:true});
