...
const CYCLES = 50;
const URL = '/index-v19.html';
const PROJECT_KEY = 'yaiwes-factory-project-v19';

async function boot(page) {
  await page.goto(URL, { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveTitle(/YAIWES UI Factory V1\.9/);
...
async function touchDragNode(page) {
  ...
  await expect.poll(() => node.evaluate(el => ({ left: el.style.left, top: el.style.top }))).not.toEqual(before);
  await page.reload({ waitUntil: 'domcontentloaded' });
  await expect(page.locator('[data-node]')).toHaveCount(1);
}

test.describe('YAIWES Factory V1.9 candidate stability', () => {
  test('50 candidate cycles: editor, scroll, controls and reload recovery', async ({ page }, testInfo) => {
    ...
    for (let cycle = 1; cycle <= CYCLES; cycle += 1) {
      ...
      console.log(`FACTORY_V1_9_LOCAL_CYCLE=${cycle}/${CYCLES} PASS`);
    }
  });

  test('mobile touch move and single contextual scroll', async ({ page }, testInfo) => {
