...
const SELECTOR = "[data-kind='panel']";
...
async function revealCard(page) {
  ...
  await expect(toggle, 'mobile shell must expose Biblioteca toggle to reach [data-kind=panel]').toBeVisible();
  await toggle.click();
  await expect(card).toBeVisible();
}

test.describe('F-FE-114 ELEMENT-PANEL control QA', () => {
  test('panel card selects preview then Insertar adds Panel and persists', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('[data-preview-meta]')).toContainText('panel');
    await expect(insert).toHaveAttribute('data-selected-kind', 'panel');
    ...
    await expect(page.locator('[data-node] strong')).toHaveText('Panel');
    await expect(page.locator('[data-node] small')).toContainText('panel');
    ...
    expect(created.item).toMatchObject({ kind: 'panel', label: 'Panel' });
    expect(created.selectedId).toBe(created.item.id);
    expect(created.storedCount).toBe(1);
    expect(created.storedKind).toBe('panel');
    expect(created.storedLabel).toBe('Panel');
    ...
    expect(afterReload).toEqual({ count: 1, kind: 'panel', label: 'Panel' });
    ...
    console.log(`F_FE_114_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, kind: 'panel', project: testInfo.project.name })}`);
  });
});
