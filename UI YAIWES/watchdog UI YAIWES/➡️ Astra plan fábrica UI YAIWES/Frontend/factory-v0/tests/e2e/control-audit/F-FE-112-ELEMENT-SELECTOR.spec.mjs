...
const SELECTOR = "[data-kind='selector']";
...
async function revealCard(page) {
  ...
  await expect(toggle, 'mobile shell must expose Biblioteca toggle to reach [data-kind=selector]').toBeVisible();
  await toggle.click();
  await expect(card).toBeVisible();
}

test.describe('F-FE-112 ELEMENT-SELECTOR control QA', () => {
  test('selector card selects preview then Insertar adds Selector and persists', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('[data-preview-meta]')).toContainText('selector');
    await expect(insert).toHaveAttribute('data-selected-kind', 'selector');
    ...
    await expect(page.locator('[data-node] strong')).toHaveText('Selector');
    await expect(page.locator('[data-node] small')).toContainText('selector');
    ...
    expect(created.item).toMatchObject({ kind: 'selector', label: 'Selector' });
    expect(created.selectedId).toBe(created.item.id);
    expect(created.storedCount).toBe(1);
    expect(created.storedKind).toBe('selector');
    expect(created.storedLabel).toBe('Selector');
    ...
    expect(afterReload).toEqual({ count: 1, kind: 'selector', label: 'Selector' });
    ...
    console.log(`F_FE_112_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, kind: 'selector', project: testInfo.project.name })}`);
  });
});
