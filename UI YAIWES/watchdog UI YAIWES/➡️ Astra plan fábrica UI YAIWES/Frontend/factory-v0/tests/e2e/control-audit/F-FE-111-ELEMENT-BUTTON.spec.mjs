...
const SELECTOR = "[data-kind='button']";
...
async function boot(page) {
  ...
  await revealButtonCard(page);
  return { pageErrors, failedRequests };
}

async function revealButtonCard(page) {
  const card = page.locator(SELECTOR);
  if (await card.isVisible()) return;
  const toggle = page.locator('[data-workspace-toggle="left"]');
  await expect(toggle, 'mobile shell must expose Biblioteca toggle to reach [data-kind=button]').toBeVisible();
  await toggle.click();
  await expect(card).toBeVisible();
}

test.describe('F-FE-111 ELEMENT-BUTTON control QA', () => {
  test('button card selects preview then Insertar adds Boton and persists', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('[data-preview-meta]')).toContainText('button');
    await expect(insert).toHaveAttribute('data-selected-kind', 'button');
    ...
    await expect(page.locator('[data-node] strong')).toHaveText('Botón');
    await expect(page.locator('[data-node] small')).toContainText('button');
    ...
    expect(created.item).toMatchObject({ kind: 'button', label: 'Botón' });
    expect(created.selectedId).toBe(created.item.id);
    expect(created.storedCount).toBe(1);
    expect(created.storedKind).toBe('button');
    expect(created.storedLabel).toBe('Botón');
    ...
    expect(afterReload).toEqual({ count: 1, kind: 'button', label: 'Botón' });
    ...
    console.log(`F_FE_111_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, kind: 'button', project: testInfo.project.name })}`);
  });
});
