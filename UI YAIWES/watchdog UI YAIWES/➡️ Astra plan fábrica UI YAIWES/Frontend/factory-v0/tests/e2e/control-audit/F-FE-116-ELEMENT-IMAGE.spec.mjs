...
const SELECTOR = "[data-kind='image']";
...
async function revealCard(page) {
  ...
  await expect(toggle, 'mobile shell must expose Biblioteca toggle to reach [data-kind=image]').toBeVisible();
  await toggle.click();
  await expect(card).toBeVisible();
}

test.describe('F-FE-116 ELEMENT-IMAGE control QA', () => {
  test('image card selects preview then Insertar adds Imagen and persists', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('[data-preview-title]')).toHaveText('Imagen');
    await expect(page.locator('[data-preview-meta]')).toContainText('image');
    await expect(insert).toHaveAttribute('data-selected-kind', 'image');
    ...
    await expect(page.locator('[data-node] strong')).toHaveText('Imagen');
    await expect(page.locator('[data-node] small')).toContainText('image');
    ...
    expect(created.item).toMatchObject({ kind: 'image', label: 'Imagen' });
    expect(created.selectedId).toBe(created.item.id);
    expect(created.storedCount).toBe(1);
    expect(created.storedKind).toBe('image');
    expect(created.storedLabel).toBe('Imagen');
    ...
    expect(afterReload).toEqual({ count: 1, kind: 'image', label: 'Imagen' });
    ...
    console.log(`F_FE_116_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, kind: 'image', project: testInfo.project.name })}`);
  });
});
