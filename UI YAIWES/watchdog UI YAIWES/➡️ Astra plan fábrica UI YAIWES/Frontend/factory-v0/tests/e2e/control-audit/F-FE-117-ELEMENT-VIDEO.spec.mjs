...
const SELECTOR = "[data-kind='video']";
...
async function revealCard(page) {
  ...
  await expect(toggle, 'mobile shell must expose Biblioteca toggle to reach [data-kind=video]').toBeVisible();
  await toggle.click();
  await expect(card).toBeVisible();
}

test.describe('F-FE-117 ELEMENT-VIDEO control QA', () => {
  test('video card selects preview then Insertar adds Video and persists', async ({ page }, testInfo) => {
    ...
    console.log(`F_FE_117_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, kind: 'video', project: testInfo.project.name })}`);
  });
});
