...
test.describe('F-FE-095 STEP-2 control QA', () => {
  test('clicking [data-step=2] activates step 2 and persists across reload', async ({ page }) => {
    const { pageErrors, failedRequests } = await boot(page, { clearStorage: true });
    const step2 = page.locator(SELECTOR);
    ...
    await expect(page.locator('#step-title')).toHaveText('Componer');
    await expect(page.locator('#context-count')).toHaveText('2/5');
    ...
    await activateStep(page, SELECTOR);
    await expect(step2).toHaveClass(/active/);
    await expect(page.locator("[data-step='3']")).not.toHaveClass(/active/);
    await expect(page.locator('#step-kicker')).toHaveText('PASO 2');
    await expect(page.locator('#step-title')).toHaveText('Componer');
    await expect(page.locator('#context-count')).toHaveText('2/5');
    expect(await page.evaluate(() => globalThis.__YAIWES_FACTORY_V19__.getState().step)).toBe(2);
    ...
    console.log(`F_FE_095_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, step: afterReload })}`);
  });
});
