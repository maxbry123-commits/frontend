...
async function revealCard(page) {
  ...
  await expect(toggle, 'mobile shell must expose Biblioteca toggle to reach [data-kind=page]').toBeVisible();
  await toggle.click();
  await expect(card).toBeVisible();
}

test.describe('F-FE-115 ELEMENT-PAGE control QA', () => {
  test('page card selects preview then Insertar adds Pagina and persists', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('[data-preview-title]')).toHaveText('Página');
    await expect(page.locator('[data-preview-meta]')).toContainText('page');
    await expect(insert).toHaveAttribute('data-selected-kind', 'page');
    ...
    await expect(page.locator('[data-node] strong')).toHaveText('Página');
    ...
    expect(afterReload).toEqual({ count: 1, kind: 'page', label: 'Página' });
    ...
    console.log(`F_FE_115_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, kind: 'page', project: testInfo.project.name })}`);
  });
});
