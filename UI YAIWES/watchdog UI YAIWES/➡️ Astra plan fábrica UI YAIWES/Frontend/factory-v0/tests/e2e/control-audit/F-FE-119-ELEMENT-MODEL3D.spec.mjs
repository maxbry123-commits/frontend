...
const SELECTOR = "[data-kind='model3d']";
...
async function revealCard(page) {
  ...
  if (!(await card.isVisible())) {
    ...
    await expect(toggle, 'mobile shell must expose Biblioteca toggle to reach [data-kind=model3d]').toBeVisible();
    await toggle.click();
  }
...
test.describe('F-FE-119 ELEMENT-MODEL3D control QA', () => {
  test('model3d card selects preview then Insertar adds 3D and persists', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('[data-preview-title]')).toHaveText('3D');
    await expect(page.locator('[data-preview-meta]')).toContainText('model3d');
    await expect(insert).toHaveAttribute('data-selected-kind', 'model3d');
    ...
    await expect(page.locator('[data-node] strong')).toHaveText('3D');
    await expect(page.locator('[data-node] small')).toContainText('model3d');
    ...
    expect(created.item).toMatchObject({ kind: 'model3d', label: '3D' });
    expect(created.selectedId).toBe(created.item.id);
    expect(created.storedCount).toBe(1);
    expect(created.storedKind).toBe('model3d');
    expect(created.storedLabel).toBe('3D');
    ...
    expect(afterReload).toEqual({ count: 1, kind: 'model3d', label: '3D' });
    ...
    console.log(`F_FE_119_PASS=${JSON.stringify({ candidate: CANDIDATE, selector: SELECTOR, kind: 'model3d', project: testInfo.project.name })}`);
  });
});
