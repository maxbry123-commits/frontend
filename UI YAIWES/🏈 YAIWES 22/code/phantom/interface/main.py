...
def validate_environment() -> None:  # noqa: PLR0912, PLR0915
    ...
    phantom_llm = Config.get("phantom_llm")
    uses_phantom_models = phantom_llm and phantom_llm.startswith("phantom/")

    if not phantom_llm:
        missing_required_vars.append("PHANTOM_LLM")

    has_base_url = uses_phantom_models or any(
        [
    ...
    if missing_required_vars:
        ...
        error_text.append("\nExample setup:\n", style="white")
        if uses_phantom_models:
            error_text.append("export PHANTOM_LLM='phantom/gpt-5'\n", style="dim white")
        else:
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Phantom Multi-Agent Cybersecurity Penetration Testing Tool",
